...
## 写用例的两条硬纪律（本项目踩出来的，违反会造成假绿或归不到账的红）

### 1. 🔴「断言某个副作用不该发生」的用例，落点必须打 `tmp_path`——判据是红态而非绿态

写 `assert not <某文件>.exists()` 这类"越界不该落盘"的断言时，**被断言的那个路径必须落在 `tmp_path` 下**，不得用 `/tmp/xxx` 这类全局固定路径。

**为什么**：这种用例在**绿的时候**两种写法看不出区别；差别只在**它真的红的那一次**。用全局路径时，一旦有人做验红（故意关掉防线，看用例会不会红），文件会**真的被写出来并留在磁盘上**——此后即使生产代码已还原，该用例**仍然红**，得手工 `rm` 才复绿。

**实证**（`P-S8-20`，Sprint 8）：`tests/test_sprint3_b2.py` 断言 `not Path("/tmp/evil.py").exists()`，开发代理做验红时该文件真被写出，还原代码后用例仍红、`sha256sum -c` 双 OK 但测试不绿，**白排查一轮**才发现是残留。⇒ 已改为 `tmp_path / "outside" / "evil.py"`（语义完全一致：绝对路径 + 不在工作区内 + 断言未落盘，**但落点随用例隔离**）。

⚠ **判据在红态**：审这类用例时不要只看它现在绿不绿，要问「**它红的那一次会不会污染磁盘**」。

### 2. 🔴 报告纪律 D-1：会被后续动作作废的基线值，必须注明取值时刻或归档前重取

凡在测试报告里记 **md5 / sha / 行数 / 字节数 / 用例条数**这类**可被后续动作作废**的基线，必须二选一：**注明取值的确切时刻**，或**在归档前重取一次**。

**实证**：一份报告记下 fixture 的 md5 后，该 fixture 又被重新生成过一次，基线值没跟着更新 ⇒ 后人拿报告里那个 md5 去复核，会得出「fixture 被改过、契约破了」的**错误结论**，而契约本身其实好好的。**坏的不是契约，是纸面证据。**
