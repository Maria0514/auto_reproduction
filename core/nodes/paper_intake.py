...
from core.state import GlobalState, NodeError, PaperMeta
...
PAPER_META_SCHEMA: Dict[str, Any] = {
    ...
    "properties": {
        ...
        "title_zh": {
            "type": ["string", "null"],
            "description": "论文标题的中文翻译（Optional 备份字段，给中文 UI / reporting 消费）。",
        },
        "abstract_zh": {
            "type": ["string", "null"],
            "description": "论文摘要的中文翻译（Optional 备份字段）。",
        },
        "tldr_zh": {
            "type": ["string", "null"],
            "description": "论文 TL;DR 的中文翻译（Optional 备份字段）。",
        },
    },
...
# 该段落字节稳定，论文间共享，拼接在 _INTAKE_SYSTEM_PROMPT 主体之后；paper_intake
# 当前无动态尾部段落（user_input 走 HumanMessage 通道），故直接追加在主体末尾。
_LANGUAGE_POLICY_SECTION_INTAKE = """--- 输出语言策略 ---
请在 <result> JSON 中按以下规则填写字段语言：
- title / abstract / tldr：保留论文原文语言（通常为英文，事实层，禁止翻译）；
- title_zh / abstract_zh / tldr_zh：上述三字段的中文翻译（备份字段，给中文 UI / reporting 消费，避免中英混杂）；
- arxiv_id / authors / categories / keywords / github_url / pdf_url / publish_date：英文/原文事实层，禁止翻译；
- notes：中文自由文本 + 英文机器标签（如 WARNING: non-CS category）。
"""
...
def _build_intake_system_prompt(context: Dict[str, Any]) -> str:
    ...
    Sprint 2：主体 `_INTAKE_SYSTEM_PROMPT` 冻结（一个字节都不改），在其后追加
    字节稳定的 `_LANGUAGE_POLICY_SECTION_INTAKE` 常量段落（架构 §4.5 方案 A）。
    """
...
def _build_paper_meta(result: Dict[str, Any], fallback_arxiv_id: str) -> PaperMeta:
    ...
    return PaperMeta(
        ...
        pdf_url=pdf_url,
        title_zh=_coerce_optional_str(result.get("title_zh")),
        abstract_zh=_coerce_optional_str(result.get("abstract_zh")),
        tldr_zh=_coerce_optional_str(result.get("tldr_zh")),
    )
...
def _backfill_zh_fields(
    ...
    """LLM 漏写 *_zh 字段时回退为原文主字段值并标记 degraded（架构 §2.6.3）。

    沿用 BUG-S1-02 / BUG-S1-03 治理范式：非静默——触发回退时写 degraded NodeError +
    打 WARNING 日志。**严禁引入二次 LLM 翻译调用**（PRD §4.7.4 硬约束），仅做原文兜底。

    Args:
        meta: 待回填的 PaperMeta（原地修改）。
        degraded_nodes: degraded 节点列表（原地去重追加）。
        node_errors: NodeError 列表（原地追加一条 degraded 记录）。

    Returns:
        True 表示触发了至少一项回退（调用方据此判断是否标记 degraded）。
    """
    fell_back = False
    ...
    if fell_back:
        ...
        node_errors.append(
            make_node_error(
                NODE_NAME,
                "degraded",
                "LLM 漏写中文字段 *_zh，已回退原文主字段值（未触发二次翻译）",
                None,
            )
        )
        logger.warning(
            "[%s] *_zh 字段缺失回退为原文主字段（避免静默吞错，未触发二次 LLM 翻译）",
            NODE_NAME,
        )
    return fell_back
...
def _map_intake_result(
    ...
    # Sprint 2：LLM 漏写 *_zh 中文字段时回退原文主字段值（非静默 + degraded 标记）。
    _backfill_zh_fields(paper_meta, degraded_nodes, node_errors)
