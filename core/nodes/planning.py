...
REPRODUCTION_PLAN_SCHEMA: Dict[str, Any] = {
    ...
    "properties": {
        ...
        # S5-05（P9，架构 sp5 §7.5）：定性预期 array 形态——description 必填 + trend 可选，
        # 不再是 {metric: number} 数值映射（禁编造数值，数值对比只留论文 baseline vs 复现值）。
        "expected_results": {
            "type": "array",
            "items": {
                "type": "object",
        ...
        # S5-01（P9，架构 sp5 §5 / §7.1）：所需外部凭证声明，可选属性（map 缺失回填 []）；
        # 每项恰含 purpose_key / purpose 两键，绝不含凭证值本身。
...
# S5-09 术语治理（P8，架构 sp5 §7.9 / AC-S5-19）：尾部独立静态段落，跨论文字节级恒定。
# 独立常量便于测试断言（CP-1.5-3 尾部段落常量断言）；严禁混入任何论文级动态变量。
_PLANNING_TERMINOLOGY_SECTION = """
【术语与措辞约束】
- 机器可读字段（JSON 内的枚举值，如 code_strategy 的 "use_repo" / "hybrid" / "from_scratch"，
  以及 required_credentials 的 purpose_key）必须保留枚举/约定原值，不要翻译、不要改写。
- 面向用户的可读文本（plan_summary、purpose、expected_results 的 description、
  execution_steps 的 expected_output 等）一律使用通俗中文表述。
...
5. execution_steps（step_name / command / expected_output 三元组列表）：可执行的步骤序列，
   ...
   - 如需多命令，可用 `&&`（前一条成功才继续）；
   - 避免管道 `|`、重定向 `>`、命令替换 `$(...)`、后台 `&` 等无法在无 shell 环境生效的语法。
   【执行落点：命令一律相对代码目录书写】
   - 系统已把执行的工作目录设为**本次论文的代码目录**（复现代码就写在那里），
     所有命令一律**相对代码目录**书写；不要写绝对路径——代码目录在规划阶段还不存在。
   - 需要用到参考仓库的源码时，用 `pip install -e <参考仓库路径>` 把它装成可导入的包
     （仓库路径取自上下文 selected_repo.local_path），装完在代码目录下直接 import 即可；
     **不要进入仓库目录**——参考仓库是多篇论文共用的只读参考物，
     不接收任何复现代码与复现产物。
   - 实验产出统一落在代码目录下的 `outputs/` 目录里（指标汇总、图、中间结果都放这里）。
   【计划只规定"跑什么"，写代码是代码生成环节的职责】
   - **不得生成"先写一个占位文件、再运行该占位文件"这类步骤**：无论写进去的是占位符
     还是真实实现，这个**步骤形态本身**就越权了。复现代码由代码生成环节统一产出，
     计划只负责规定跑哪个脚本、带什么参数。
6. expected_results + required_credentials + estimated_time + deliverables：
   - expected_results（定性预期列表，**严禁编造具体数值**）：每项形如
     {"description": str, "trend": {"metric": str, "greater": str, "lesser": str} 或 null}：
     - description：中文定性描述（如「loss 应收敛」「指标应接近论文报告量级」）；
     - trend（可选）：可机验的相对趋势——metric 为指标名，greater / lesser 为对比的
       实验组名，语义是「预期 greater 组的该指标高于 lesser 组」；无可比组时置 null；
     - **禁止**输出 {指标名: 数值} 形式的数值预期，禁止凭空捏造任何具体数字；
       论文真实 baseline 数字已由论文分析环节提取，**不要**在计划中复述。
   - required_credentials（所需外部凭证/资源声明列表）：逐项声明复现真实运行所依赖的
     外部凭证，每项恰含两键 {"purpose_key": str, "purpose": str}：
     - purpose_key 命名约定：Git 私有仓库凭证用 "git_credential:<host>"
       （如 "git_credential:github.com"）；HuggingFace token 用 "hf_token"；
       API key 等环境变量类凭证用通用约定 "env:<ENV_VAR>"（如 "env:OPENAI_API_KEY"）；
     - purpose：给用户看的中文用途说明（如「论文方法依赖真实 LLM 调用」）；
...
- 单次重规划最多克隆 3 个用户提供仓库。
""" + REPO_QUALITY_SCORING_SECTION + _PLANNING_TERMINOLOGY_SECTION
...
def _coerce_expected_results(value: Any) -> List[Dict[str, Any]]:
    """expected_results 规整为 List[Dict]（S5-05 新形态；缺失回填 []，带值透传）。

    - list：dict 项透传（description / trend 形态）；字符串项包装为 {"description": str}
      （与 _coerce_step_list 的降级包装同范式）；其余脏项过滤；
    - 缺失 / 旧 {metric: number} dict 形态 / 其它类型：回填 []——旧数值映射属被
      S5-05 明令禁止的编造数值形态，不带进新链路（架构 sp5 §7.5）。
    """
...
def _coerce_credential_list(value: Any) -> List[Dict[str, str]]:
    """required_credentials 规整为 List[Dict]（S5-01；缺失回填 []，带值透传 dict 项）。

    空列表属合法——论文可能不依赖外部凭证；不列入 _missing_core_fields、
    不做工具历史回填（架构 sp5 §7.1）。
    """
    if not isinstance(value, list):
        return []
...
def _build_reproduction_plan(
    ...
    return ReproductionPlan(
        ...
        # S5-05 / S5-01（sp5）：两字段缺失回填 []（required_credentials 可为空属合法）。
        expected_results=_coerce_expected_results(result.get("expected_results")),
        required_credentials=_coerce_credential_list(result.get("required_credentials")),
