...
覆盖 dev-plan §667-672 的 5 条核心场景：
    ...
    2. 修复循环上限（AC-S3-03，上限取 MAX_FIX_LOOP_COUNT）：连续可修复失败 → fix_loop_count 自增至上限拦截 → interrupt#2；
...
# ===========================================================================
# 场景 2：修复循环上限（AC-S3-03，上限取 MAX_FIX_LOOP_COUNT）
# ===========================================================================


def test_f2_e2e_2_fix_loop_upper_limit_then_interrupt(monkeypatch, tmp_path):
    """场景 2（AC-S3-03）：连续可修复失败 → fix_loop_count 自增至上限拦截 → interrupt#2。

    每轮 sandbox 返回可修复失败（ModuleNotFoundError=import 可修复）。真实图路由：
    execution(失败) → retry_coding → coding(fake) → execution(失败) → … 达 MAX_FIX_LOOP_COUNT
    回合后 fix_loop_count 触顶 → 不再回 coding → await self-loop → interrupt#2 暂停。
    """
...
class TestRealChainE2E:
    ...
    # ===================================================================
    # real-2 修复循环上限（AC-S3-03，上限值取 MAX_FIX_LOOP_COUNT）
    # ===================================================================
    def test_real_2_fix_loop_upper_limit_three(self, monkeypatch, tmp_path):
        ...
        fix_loop_count 自增至 MAX_FIX_LOOP_COUNT 拦截 → interrupt#2 暂停。

        真实链路：上游真实 + 真实 coding 修复回合（每轮真实 LLM 读失败反馈再写代码）。
        mock 边界：sandbox 每轮注入同一可修复失败（ModuleNotFoundError=import 可修复），驱动
        execution 真实错误分类 → retry_coding 回边 → coding(真实) → … 达 MAX_FIX_LOOP_COUNT 回合后触顶。
