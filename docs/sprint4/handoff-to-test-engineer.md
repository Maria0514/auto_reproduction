...
| # | 项 | 验证目标 | 前置 | 预估时长 | 预估配额 | 建议跑数 |
|---|---|---|---|---|---|---|
| 1 | `real_s4_1`（三 interrupt 串行真实链路） | AC-S4-13 转正 | 凭证已具备（.env） | ~3-8 min/次 | 1 条真实链路 LLM token；deepxiv 近零（靶 HippoRAG 缓存） | 3~5 次（LLM 服从度依赖） |
| 2 | `real_s4_2`（真凭证注入 sandbox） | AC-S4-08 转正 | 需 Maria 提供 `SP4_E2E_PRIVATE_REPO_URL` + `SP4_E2E_GIT_TOKEN` | <2 min | **零 LLM/deepxiv 配额** | 1 次 |
| 3 | `real_s4_3`（真实 agent 复述脱敏） | AC-S4-11 真实 LLM 侧转正 | 仅 LLM 凭证 | ~5-10 轮 LLM 调用/次 | LLM token | 3~5 次 |
| 4 | coding Prompt Cache 命中率回归 | CP-G3-1：R_after ≥ sp3 F3 基线 0.9008 × 0.95 | `SPIKE_F3_AUTHORIZED=1` | 5 min 内连跑 ×3 | 3 次 coding prompt LLM 调用 | 1 组 |
| 5 | execution Prompt Cache 命中率首测 | 新 prompt 建基线（无历史对照） | 同上 | 同上 | 3 次 execution prompt LLM 调用 | 1 组 |
| 6 | （建议首跑）sp3 `real-1` smoke | fail-fast 验凭证/deepxiv 可达 | 凭证已具备 | ~3 min | 最省 | 1 次 |
...
| AC-S4-11 | 全链路无凭证明文 | CP-A3-3 / CP-C1-4 / CP-E1-3 / CP-E3-4 / CP-D2-2 / CP-G2-2（哨兵 grep 四落点；GlobalState 投影 BUG-S4-G2-01 修复后转正；checkpoint DB = ADJ-S4-G2-02 已知限制 characterization 锚定） | ✅ mock grep；真实 agent 复述→清单#3 |
| AC-S4-12 | 日志脱敏 | CP-A3-3/5（caplog）+ G2-01 修复补 warning 日志 mask | ✅ caplog |
| AC-S4-13 | 三 interrupt 互不干扰 | CP-G2-1（mock LLM + 真实 build_graph + SqliteSaver，#1→#3→#2 串行，3 连跑） | ✅ mock；真实链路→清单#1 |
...
| 新增 `docs/sprint4/test-reports/*` | ✅（a1-a2-d1 / a3-d2 / b1 / b2 / cef / e3-e4 / g1 / g2） |
...
- **工具 schema 稳定前缀**：`request_user_input` docstring 逐字节锚定
  ...
  常量同批更新），机制未破。
- **命中率回归**：脚本 `scripts/spike_coding_prompt_cache.py` 就绪，
  `SPIKE_F3_AUTHORIZED=1` 双保险闸门实证生效（不设即拒跑）。真跑 = 清单 #4/#5，
  守门线 R_after ≥ 0.9008 × 0.95 = 0.8558。
