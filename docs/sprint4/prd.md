...
| **预算扣减** | wrapper 唯一扣减点（`react_base.py` L889-894，现状即如此） | 改造后**自动获得同一扣减点**（这正是 must-fix-2 被简化的机理，见 §6.2）（**落地注记 2026-07-05：已被推翻，最终为编排层在 `_map_execution_result` 显式回写，见 §6.2 落地注记**） |
...
> **[落地注记 2026-07-05] 最终形态 = 手写编排 + 内嵌裸 `create_react_subgraph`（非 `_make_react_wrapper`），预算扣减由编排层在 `_map_execution_result` 显式回写（不存在"自动扣减"）；见架构 §3/§4 与 dev-plan §7.2 差异 2/3。本段保留立项时表述，不作实现依据。**
...
- **路线丙让 execution 变成 ReAct agent（`_make_react_wrapper` 生成）→ 自动走唯一预算扣减点**。代码实证：`_make_react_wrapper._wrapper` L889-894——
  ...
  > **[落地注记 2026-07-05] 本段"自动走唯一预算扣减点"已被架构推翻：最终形态 = 手写编排 + 内嵌裸 `create_react_subgraph`（非 `_make_react_wrapper`），预算扣减由编排层在 `_map_execution_result` 显式回写（不存在"自动扣减"，也不再需要担心 wrapper 双重扣减）；见架构 §3/§4 与 dev-plan §7.2 差异 2/3。本段保留立项时表述，不作实现依据。**
...
- **execution agent**：**完整复现执行**——跑 `execution_steps`（可能是长时训练/评估）、`collect_artifacts` 收产物、`_parse_metrics` 解析指标、`_classify_execution` 分类、B 档判定（**口径注记 2026-07-05：B 档"exit 全 0"按 `_effective_runs` 有效执行视图判定——同命令 argv 精确匹配取最后一次；见架构 §3.2 注记与 commit 86c5325**）。这是"真正的复现"，成本高、有副作用、产出是最终交付依据。
- **边界红线（防两 agent 干重叠的事）**：
...
| AC-S4-04 | execution ReAct 化后自动走 `_make_react_wrapper` 预算扣减（或等价的 round 扣减），`retry_budget_remaining` 按实际 round 递减，不双重扣减（**落地注记 2026-07-05：落地为"等价的 round 扣减"分支——编排层在 `_map_execution_result` 显式回写而非 wrapper 自动扣减，见 §6.2 落地注记**） | mock：断言一次 execution 后 `retry_budget_remaining` 递减 = round 数 |
