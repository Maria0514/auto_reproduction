...
- **一致点**：敏感值**都不进 state/checkpoint**（**勘误 2026-07-06/ADJ-S4-G2-02**：「不进 GlobalState 业务字段」成立；「不进 checkpoint」不成立——子图 messages 与 `__resume__` 通道帧级明文为已知接受限制，见 §6.5 checkpoint 行）。LLM api_key 走 env，用户凭证走 `.secrets`——两者都在 GlobalState 之外。
...
| 落点 | 保证 |
|---|---|
| `request_user_input` 返回 | 敏感值作为 ToolMessage 内容进入**子图私有 messages**，不进 GlobalState（子图 messages 不回灌，见 §9.3）（勘误 2026-07-06/ADJ-S4-G2-02：子图 messages 随 checkpoint 帧落库，「不进 GlobalState」成立、「不进 checkpoint」不成立，见本表 checkpoint 行） |
...
| checkpoint（SqliteSaver） | ~~因 state 不含敏感值，checkpoint 天然不含~~（**勘误 2026-07-06/ADJ-S4-G2-02 裁决 (a)**）：root-ns GlobalState 通道不含敏感值（BUG-S4-G2-01 修复后）；子图 messages 与 `__resume__` 通道帧级明文为已知接受限制（威胁面与 `.secrets` 等价；缓解 = `checkpoints.db*` gitignore + 0600 权限对齐；升级触发条件见 G3 handoff） |
...
| R-S4-05 | 敏感值泄漏进 state/logs/report/checkpoint | 高 | 敏感值旁路 state（§6.5）；mask_value 统一脱敏落点（§9.4）；AC-S4-11/12 grep + caplog 断言；checkpoint 帧级明文 = ADJ-S4-G2-02 已知限制（裁决 (a)，0600 对齐 + gitignore 兜底，2026-07-06） |
