...
| **改动** | `core/errors.py` | S4-06 | `ErrorCategory` 无需改（细分类是 execution 本地 Enum）；新增凭证关键字表（execution 本地） |
...
from core.secrets_store import lookup_secret, remember_secret, mask_value
...
见 §6（Q-E1）完整设计。此处仅列职责：`lookup_secret(purpose_key) -> Optional[str]`（只读 .secrets）、`remember_secret(purpose_key, value, is_sensitive)`（0600 落盘）、`load_all_secrets() -> Dict[str, str]`（execution/coding 编排层启动时读入内存注入 extra_env）、`mask_value(text, secrets) -> str`（日志/报告脱敏 filter）。
...
| `execution_result.logs` / `run_command` stdout | 落盘/回 state 前经 `mask_value(text, load_all_secrets())` 脱敏（§9.3） |
...
| **零改动** | — | `core/graph.py`（节点名/边/self-loop 不变）、`core/react_base.py`（预算扣减点不动）、`core/checkpointer.py`、`core/nodes/planning.py`、reporting、sp1/sp2 既有节点 | 无 |
