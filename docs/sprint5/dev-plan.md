...
**自测检查点**：
- [ ] CP-0.1-1 spike 用例跑通，coding / execution 两路径传播结论各自明确落档（是/否 + 证据）
- [ ] CP-0.1-2 事件元数据实证：`langgraph_node` 可取、tool 入参可摘要、`on_llm_end` 文本可截断（T-S5-4-1 消费前提）
- [ ] CP-0.1-3 结论报告归档 `docs/sprint5/test-reports/`；若回退：R-B1 档位判定 + 勘误注记写入本计划 §7
...
**自测检查点**：
- [ ] CP-0.2-1 case④ kind 分发三态断言：planning 形态 → review；dev_loop_failure / user_input_request → 执行监控页（AC-S5-16）
- [ ] CP-0.2-2 case④bis/④ter：controller state 序列驱动断言 coding/execution → 监控页、reporting∧report_path 非空 → 报告页（progress 与监控页双通道各证一次，AC-S5-15 mock 部分）
- [ ] CP-0.2-3 case⑥bis：mock 空 report_path ∧ is_finished → 失败/降级卡片渲染 + autorefresh 未注册（AC-S5-17）
- [ ] CP-0.2-4 `is_finished` 单测（运行中/interrupted/完成三态）；受影响既有 UI 路由用例适配（R-8，只换断言目标不弱化语义，逐条记录）
- [ ] CP-0.2-5 回归样本 thread `task-9208a1a4b4f5` 手动验证路由可达报告页（只读，与测试工程师协作，AC-S5-15 手动部分）
...
- [ ] CP-1.1-2 账本边界断言：`REACT_MAX_ROUNDS_EXECUTION_CAP == MAX_DEV_LOOP_LLM_CALLS // 2`（30==60/2）∧ `CAP > FLOOR > 0`（AC-S5-12 常量面）
- [ ] CP-1.1-3 既有常量基线零改动（`MAX_TOTAL_LLM_CALLS==120` / `MAX_DEV_LOOP_LLM_CALLS==60` / `REACT_MAX_ROUNDS_CODING==12` 等；git diff 实证纯追加）
...
**自测检查点**：
- [ ] CP-1.2-1 `GlobalState.__annotations__` / `ReproductionPlan` / `ExecutionResult` 含全部新字段且类型正确；`create_initial_state` 默认值正确
- [ ] CP-1.2-2 must-fix-1 grep 双证沿用：三 List 字段（node_errors/degraded_nodes/fix_loop_history）与全部新字段均无 `Annotated`/`operator.add`
- [ ] CP-1.2-3 全量非 e2e 回归零退化（新字段不破坏既有初始化/反序列化；旧 checkpoint state 快照 `.get()` 读通）
...
- [ ] CP-1.3-2 主体字节级一致守门更新：两篇不同论文去尾部动态段后 `==` 新常量（含 `_CODING_HONESTY_SECTION` 进稳定前缀）；段内禁动态变量审查断言
- [ ] CP-1.3-3 mock map 断言：`<result>` 带/不带 simulation_notice → state 落值/回填 None（AC-S5-04 state 部分）；`_map_coding_result` 3 参签名与既有断言零回归
...
**自测检查点**：
- [ ] CP-1.4-1 主体字节级一致断言更新（CP-E2-1 同款）+ 主体无 "max_rounds=10" 数字残留断言 + step_index 说明存在
- [ ] CP-1.4-2 `run_in_sandbox` docstring 字节稳定 + `step_index` 缺省 -1 向后兼容（不带参调用不炸、台账记 -1）
- [ ] CP-1.4-3 prepare payload 含 `env_info` 且 JSON 合法（sort_keys/ensure_ascii=False/禁 str(dict)，BUG-S1-02 自查）
- [ ] CP-1.4-4 HumanMessage 动态上下文含预算数字键且 sort_keys 幂等；`interaction_tools.request_user_input` docstring 零字节改动断言（CP-B1-5 守门沿用）
...
**自测检查点**：
- [ ] CP-1.5-1 prompt 断言：禁编造数值文案 + required_credentials 声明指令 + 尾部术语约束段存在（AC-S5-01/09/19 prompt 部分）
- [ ] CP-1.5-2 schema 断言：expected_results 为 array 形态、**无 {metric: number} 数值映射**（AC-S5-09 schema 部分）；required_credentials 两键约束
- [ ] CP-1.5-3 主体字节级一致断言更新（CP-B3-10 同款：两论文去尾部后 == 常量）+ 尾部段落常量断言 + 禁动态变量审查
- [ ] CP-1.5-4 map 回填断言：`<result>` 缺两字段 → 回填 `[]` 不炸；带值 → 透传
...
**自测检查点**：
- [ ] CP-1.6-1 §10.2 prompt 类断言全绿；适配清单落档（改哪条/为何/语义不降）
- [ ] CP-1.6-2 三主体禁动态变量审查断言 PASS；`request_user_input` docstring 零字节改动复核
- [ ] CP-1.6-3 全量非 e2e 回归零退化（对照开工基线）；在线维复采挂账记录进 T-S5-5-3 授权清单
...
**自测检查点**：
- [ ] CP-2.1-1 `env:OPENAI_API_KEY` → env 注入断言；非 `env:` 未知 key 行为与 sp4 一致（零回归）；`GIT_TERMINAL_PROMPT=0` 无条件带不回归
- [ ] CP-2.1-2 stash → `load_all_secrets` 合并可见、`.secrets` 文件不含（不落盘断言）、`mask_value` 覆盖该值（脱敏地基）
- [ ] CP-2.1-3 会话层与 `.secrets` 同 key 时覆盖语义确定（会话层优先，最后提交者胜）+ 模块重载模拟"进程重启即失"
- [ ] CP-2.1-4 既有六接口测试全绿零退化；本模块日志只打 purpose_key 无 value 明文（caplog 审计沿用 CP-A3-5 范式）
...
**自测检查点**：
- [ ] CP-2.2-1 缺凭证 → interrupt payload 五键契约断言（四键 + allow_degrade=True）；`.secrets`/会话层命中 → 零 interrupt 静默通过（AC-S5-02："interrupt 或降级标记二者必居其一、不存在静默绕过"）
- [ ] CP-2.2-2 resume 四分支：记住落盘 / 不记住进会话层（不落盘）/ degrade → `credential_degradations` 落 state 且该 key 不再拦 / 非法 resume（缺 value 非 degrade）→ WARNING 非静默
- [ ] CP-2.2-3 **单项串行幂等**：构造两缺失项 → 两次串行 interrupt、resume 值不串位、第二次重跑 missing 重算正确——**连跑 3 次一致**（interrupt 幂等类判据）
- [ ] CP-2.2-4 GraphBubbleUp 直通：gate interrupt 冒泡不被捕获（grep/AST 断言 gate 函数体无裸 except + mock 主图暂停实证）
- [ ] CP-2.2-5 wrapper 复合零回归：graph.py 零改动（git diff）、coding 既有用例（sp3 c1 系列 + sp4 c2 系列）全绿、`required_credentials==[]` 时 gate 零开销直通
...
**自测检查点**：
- [ ] CP-2.3-1 allow_degrade 有/无两态渲染分支断言（agent 路径 payload 无按钮）
- [ ] CP-2.3-2 点击降级 → resume 三键契约断言（value=""/remember=False/degrade=True 经 `resume_with` 透传）
- [ ] CP-2.3-3 既有 user_input_request 面板（提交/记住/敏感 password）零回归（AC-S5-03 mock e2e 三落点断言在 T-S5-5-2 收口）
...
**自测检查点**：
- [ ] CP-2.4-1 13 步计划 / 8 步执行 mock → planned=13/executed=8/completed 与 exit_code 一致/unexecuted_steps 5 条含 index+step_name（AC-S5-10 对账部分）
- [ ] CP-2.4-2 归属三级各证：step_index 优先 / 越界丢弃+WARNING / 无标签归一匹配兜底 / agent 改写命令（改路径补参）经归一仍归属 / 完全不匹配入 extra_commands
- [ ] CP-2.4-3 attribution_unavailable 保守断言：全零归属非空台账 → 不触发 incomplete_execution、原始命令如实保留（R-2）
- [ ] CP-2.4-4 steps_attempted 不参与判定（构造自报 13 台账 8 → 对账以台账为准）；注入哨兵 token → 对账字段无明文（脱敏出口②）
- [ ] CP-2.4-5 guard 幂等零扰动：interrupt#2 重跑路径复用已落盘 exec_result 含 step_reconciliation，无重算（批次纪律③）
...
**自测检查点**：
- [ ] CP-2.5-1 联动参数化断言：13 步→18 / 1 步→10（FLOOR）/ 40 步→30（CAP）/ 空计划→10（AC-S5-12 联动部分）
- [ ] CP-2.5-2 截断场景 mock（剧本跑满轮次）→ budget_truncated=True + INFO 日志 caplog；正常收尾 → False（判据两侧各证）
- [ ] CP-2.5-3 预算扣减落点 B 零回归：rounds_used 扣减语义、guard 重入零扣减（sp4 CP-E3-1 断言面复跑）；HumanMessage 预算数字与 helper 产出一致
- [ ] CP-2.5-4 exec_result 含 budget_truncated 随一次 commit 落盘（guard 复用含新字段）
...
**自测检查点**：
- [ ] CP-2.6-1 回归样本 outputs fixture → 三组全对齐（`evoskills_smoke` / `baselines/no_skill` / `baselines/self_generated`），组内 pass_rate 等顶层字段收编正确（AC-S5-20 解析部分）
- [ ] CP-2.6-2 容错：损坏 JSON / 非 dict 顶层 / 深层嵌套只收顶层——容忍 + WARNING 不炸；无 outputs 目录 → `{}`
- [ ] CP-2.6-3 env_info 回读：mock prepare ToolMessage 带 env_info → 重建后 key_packages 非空（AC-S5-20 environment 部分）；失败 ToolMessage 过滤仍生效（BUG-S1-03 范式自查）
- [ ] CP-2.6-4 两构造点（含降级路径）新键齐全断言；旧 checkpoint 快照无新键 `.get()` 防御读通（R-5/R-6）
...
**自测检查点**：
- [ ] CP-3.1-1 回归样本 fixture 命中 ≥2 类：`skill_generator.py` 抄 verifier 关键词 → R1；`task_executor.py` baseline 0.0/0.3/0.1 写死 → R2/R3（AC-S5-05 审计部分）
- [ ] CP-3.1-2 干净 fixture（`tests/fixtures/clean_code_sample/`）**零命中**（AC-S5-06 误报红线）
- [ ] CP-3.1-3 三重豁免各一断言：评估器读答案不命中 / score 初始化后更新不命中 / return 引用入参不命中
- [ ] CP-3.1-4 每条 hit 证据三元组齐全可人工复核；snippet 含哨兵 token 时已脱敏；语法错误文件跳过 + caplog WARNING
- [ ] CP-3.1-5 纯函数断言：模块无 LLM 客户端 import、无 state 读写（确定性红线）
...
**自测检查点**：
- [ ] CP-3.2-1 reporting 返回含 honesty_audit 且与 `audit_code_dir` 产出一致；调用恰一次（mock call count）
- [ ] CP-3.2-2 code_output_dir 缺失/目录不存在 → honesty_audit=None、报告仍生成（降级容忍）
- [ ] CP-3.2-3 CP-C2-5 断言更新落地：返回键集合 = {report_path, current_step, honesty_audit}，node_errors/degraded_nodes 等 list 字段零触碰
...
**自测检查点**：
- [ ] CP-3.3-1 level 三值参数化：success=False→none / success=True+标注→engineering 禁 science / 全符合无标注→science（AC-S5-07 判定部分）
- [ ] CP-3.3-2 正交标注三来源各一 mock（simulation_notice / audit hits / 降级快照 / 未执行步骤 / budget_truncated 五路径），attribution_unavailable 不触发断言（AC-S5-03 第③落点判定面 + AC-S5-11）
- [ ] CP-3.3-3 trend 回验三态：greater/lesser 与 metrics_groups 相符→"符合"、相反→"不符"、组名失配→"未验证"；纯文本→"未验证"；旧 dict 形态→全"未验证"不崩
- [ ] CP-3.3-4 判定纯函数无 LLM（确定性红线断言，与 CP-3.1-5 同范式）
...
**自测检查点**：
- [ ] CP-3.4-1 措辞断言：engineering 场景全文无"复现成功"、含目标措辞语义；science 场景正常宣告（AC-S5-07 mock 两级各一）
- [ ] CP-3.4-2 回验节三态渲染 + 存在不符/未验证 → 整体结论不宣告完全成功（AC-S5-08 mock 三态）
- [ ] CP-3.4-3 对账节渲染（N/M + 清单 + 截断声明）+ 声明块三来源渲染（AC-S5-03 第③落点渲染面 / AC-S5-10/11/12 报告面）
- [ ] CP-3.4-4 对比表无 expected 列、多组按组填充"本次复现值"非空；嵌套指标无巨型 dict 字符串（AC-S5-09/20 渲染部分）
- [ ] CP-3.4-5 旧形态兼容：旧 dict expected_results + 无新键 exec_result（旧 checkpoint 快照）→ 报告生成不崩、回验全"未验证"
- [ ] CP-3.4-6 sp3/sp4 reporting 渲染断言适配清单落档（"✅ 复现成功"类旧断言由新措辞断言取代，只换不弱化——最终收口在 T-S5-5-1）
...
**自测检查点**：
- [ ] CP-3.5-1 `humanize` 单测：全 domain 采样 + 未知 domain/value 兜底文案（AC-S5-18）
- [ ] CP-3.5-2 页面渲染扫描断言：AppTest/文本断言核心页面无裸内部枚举（覆盖 §7.9 列举 domain）
- [ ] CP-3.5-3 裸露点扫描清单落档；state 字段零改动（git diff）；既有 UI 用例零退化
...
**自测检查点**：
- [ ] CP-3.6-1 两页渲染断言：路径展示区存在、值来自 state 字段、字段缺失时不崩（`.get()` 防御）
- [ ] CP-3.6-2 手动 happy path 走查记录（AC-S5-21，与测试工程师协作）
...
**自测检查点**：
- [ ] CP-4.1-1 事件格式断言：两采集点各产出 5 字段合规事件、text 截断长度（≤120/≤160）、seq 单调、kind 两值
- [ ] CP-4.1-2 封顶断言：写入 MAX+1 条 → 恰保留最新 MAX 条（AC-S5-14 封顶单测）
- [ ] CP-4.1-3 脱敏：注入哨兵 token 的工具参数/LLM 输出 → text 无明文（脱敏出口①）
- [ ] CP-4.1-4 handler 内部异常（如 metadata 缺失）→ 吞掉 + WARNING、主流程不炸；node 取不到回填 ""
...
**自测检查点**：
- [ ] CP-4.2-1 invoke / resume 两路径均带 callbacks（spy 断言）；per-thread 隔离（两 thread 事件不串）
- [ ] CP-4.2-2 端到端 mock：mock LLM 驱动主图 → coding/execution 子图内事件到达 handler（spike 结论在真实装配下复证，AC-S5-13 事件生成部分）
- [ ] CP-4.2-3 `get_activity_tail` 尾部 n 条快照语义（不可变、越界安全）
- [ ] CP-4.2-4 事件不进 checkpoint/state：跑完 mock 流程后 checkpoint DB 与 state 快照 grep 无活动流事件痕迹（AC-S5-14）+ 全量回归零退化（react_base 零字节改动 git diff 证；若回退档启用则以勘误注记为准）
...
**自测检查点**：
- [ ] CP-4.3-1 渲染断言：尾部 30 行、seq 递增顺序、等宽块形态；空流占位文案
- [ ] CP-4.3-2 既有监控页功能（interrupt 面板/日志区/case⑥ 系列）零回归（主控收口后跑该页全量用例）
- [ ] CP-4.3-3 手动 happy path 走查：真实任务运行中活动流滚动可感知（AC-S5-13 UI 部分，与测试工程师协作）
...
**自测检查点**：
- [ ] CP-5.1-1 §10.2 四类断言面逐条适配完成，适配清单落档（改哪条/为何/语义不降）
- [ ] CP-5.1-2 全量非 e2e 回归全绿（`.venv/bin/pytest -q -m "not e2e"`，对照开工基线零退化 + sp5 新增用例计数对账）
- [ ] CP-5.1-3 interrupt/幂等类连跑 3 次 0 flaky（gate 串行 / guard 复用 / 路由类）
...
**自测检查点**：
- [ ] CP-5.2-1 五条靶测 AC 全绿（fixture 副本驱动，原样本零写入实证）
- [ ] CP-5.2-2 AC-S5-03 三落点 mock e2e 串联 PASS
- [ ] CP-5.2-3 覆盖矩阵审计 PASS：AC-S5-01~21 逐条映射 CP 用例并全绿；手动/授权项缺口显式落档
...
1. **授权动作清单先行提交 Maria（一次授权、一次动作窗口）**：
   - ① Prompt Cache 在线维复采：`scripts/spike_coding_prompt_cache.py` / `scripts/spike_execution_prompt_cache.py` / `scripts/spike_prompt_cache_baseline.py` 复采命中率，记录新 R_baseline（旧基线因批次 1 前缀变更作废属预期），此后以新基线 × 0.95 守门；
...
**自测检查点**：
- [ ] CP-5.3-1 授权动作清单提交 Maria 并获明确授权（授权记录留档）；真跑合并一次窗口执行
- [ ] CP-5.3-2 Prompt Cache 新 R_baseline 落档 + ×0.95 守门断言更新（AC-S5-19 基线对照收口）
- [ ] CP-5.3-3 真实链路抽验结论归档 `docs/sprint5/test-reports/`（跑数/耗时）
- [ ] CP-5.3-4 handoff 完成、交付物与 §7 逐条一致；最终回归收口数字落档
