...
**版本**：**v1.2**（v1.0 = 2026-08-04 初稿；v1.1 = 同日回填 Maria 拍板 1/2；**v1.2 = 同日再回填架构师两项裁定 + Maria 加拍**：①**三档 + `_parse_metrics` 四函数整体删除**（推翻本计划默认取值 A）；②**execution 上下文注入迁入批次 2**（`T-S8-2-8b`）；③**Maria 加拍：coding 侧改词一并迁入批次 2**（`T-S8-2-1b`）⇒ **批次 1a 收敛为「只加不改行为」的 5 个任务**。连带同步见 §0.0 / §3.1 / §3.4 / §14.2 / §15 / §16；架构文档同批跟改至 **v2.1**）
**日期**：2026-08-04
**作者**：全栈开发工程师代理
...
**任务总数**：**39 个**（1a×**5** + 1b×5 + 2×**14** + 3×10 + 4×3 + 5×2）
**批次数**：**6 个**（1a / 1b / 2 / 3 / 4 / 5）
**检查点总数**：**300 个**（分布见 §13 CP 索引；收口门 CP 为各批总闸门。逐批小计：1a=32 / 1b=41 / 2=126 / 3=74 / 4=15 / 5=12）
...
⇒ 🔴 **v1.2 裁定 1 之后的账目：4 行移出（`_apply_no_metrics` / `_extract_metrics_block` / `_parse_metrics` / `_regex_scan_metrics`）+ 1 行换发（`_build_execution_result`）= 5 行动；冻结表 10 行 → 6 行。**
