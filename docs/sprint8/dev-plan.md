...
> 　⇒ 🔴 **现行汇总账（2026-08-08；数字取自 §13 实算表，逐行加总非沿用）：检查点 `331` 个**（逐批 **25 / 45 / `150` / 83 / 16 / 12**）、**任务 40 个**（批次 2 = **15**）、**落点勘误 ~~`38`~~ ~~`41`~~ ~~`44`~~ ~~`47`~~ ~~`49`~~ ~~`50`~~ `54` 条**（`P-S8-1` ~ **`P-S8-54`**，其中**实质性 ~~`16`~~ ~~`17`~~ ~~`18`~~ ~~`20`~~ ~~`22`~~ `26` 条**；
🔴 **2026-08-10 ~ 08-11 批次 2 执行期新增 ~~6~~ ~~9~~ ~~11~~ ~~12~~ `16` 条 `P-S8-39 ~ ~~P-S8-44~~ `P-S8-54``，见 §15.0e** —— `39~41` 由 `T-S8-2-2` 实现前核实挖出、`42~44` 由 `T-S8-2-1` 真跑全量回归暴露、**`45~47` 由 `T-S8-2-1b` 实做时挖出**（`45` 收网抓出第四处 `<METRICS>`；`46` 是两条各自正确的约束互相拉扯；`47` 超范围登记不治））、**冻结表 10 行 → `5` 行**。~~（作废旧账：勘误 `35` 条）~~⚠ **上一行（`:6`，2026-08-07 那轮）记的 `329` / `148` / `20 条` / 「10→4」全部是当时的账、已被本轮作废** —— 保留作留痕，**不要照它对账**。
...
**落点勘误**：🔴 **总数见文首「现行汇总账」权威行（本处 2026-08-09 起不复述数字）**；下列为编号构成。~~（作废旧账：15 / 20 / 25 / 34 / `35` 条）~~（§15；v1.0~v1.2 的 **P-S8-1 ~ P-S8-11**（11 条）+ v1.3 新增 **P-S8-12 ~ P-S8-15**（4 条，§15.0）+ 批次 1a 执行期 **P-S8-16 ~ P-S8-20**（5 条，§15.0b）+ 批次 1b 开工前置期 **`P-S8-21 ~ P-S8-25`**（5 条，§15.0c）+ **批次 1b 执行期 + 批次 2 开工前置期 `P-S8-26` ~ ~~`P-S8-35`~~ `P-S8-38`**（~~10 条~~ **`13` 条**，**§15.0d**，2026-08-08 ~ 08-09，含主控收口挖出的 `P-S8-35` 与批次 2 开工前置三组失真收口的 **`P-S8-36 ~ P-S8-38`**）+ 🔴 **批次 2 执行期 `P-S8-39 ~ ~~P-S8-44~~ `P-S8-54``**（~~6 条~~ ~~9 条~~ ~~11 条~~ ~~12 条~~ **`16` 条**，**§15.0e**，2026-08-10 ~ **08-11**；`39~41` 由 `T-S8-2-2` 实现前核实挖出、`42~44` 由 `T-S8-2-1` 真跑全量回归暴露、**`45~47` 由 `T-S8-2-1b` 实做时挖出、`48~49` 由 `T-S8-2-3` 实做时挖出、**`51~54` 由 `T-S8-2-4` 实做时挖出**；**其中 ~~3 条~~ `5` 条是同一份文档内部两处直接打架**）。实质性 ~~`14`~~ ~~`16`~~ ~~`17`~~ ~~`18`~~ ~~`20`~~ **`22` 条**⟦🔴 **2026-08-11 主控订正（第一次）**：本行原写 `17`，而**同一行的编号列表实为 18 个**（下列逐个可数），文首 `:10` 权威行写的也是 `18` ⇒ 上一轮新增两条实质性（`P-S8-40` / `P-S8-42`）时**编号加进来了、数字只推了一格**。🔴 **`P-S8-32` 同族又一次发作，且这次最刺眼——两个对不上的数就在同一行里**，连"跨章节漏跟"这个借口都没有。⟧⟦**同日第二次推数**：`T-S8-2-1b` 交付新增 `P-S8-45` / `P-S8-46` 两条实质性（`P-S8-47` 属超范围登记不治、不计入）⇒ `18` → **`20`**，编号列表同批补入，**本轮已按 §3.10 全文 grep 五个同源汇总口一并推**⟧：P-S8-3 / P-S8-4 / P-S8-5 / P-S8-9 / 🔴 **P-S8-12（架构未覆盖）** / 🔴 **P-S8-16（`AR-S8-15` 触发条）** / P-S8-18 / `P-S8-21` / `P-S8-22` / `P-S8-23` / `P-S8-24` / **`P-S8-27`** / **`P-S8-31`** / **`P-S8-35`** / 🔴 **`P-S8-36`（验钞口径，安全面）** / 🔴 **`P-S8-37`（`T-S8-2-8` 缺实现步骤）** / 🔴 **`P-S8-40`（`_collect_grouped_metrics` 存废，三处仍写「不删」）** / 🔴🔴 **`P-S8-42`（§3.3 变红面表漏三族，收口门自身失效）** / 🔴 **`P-S8-45`（三处 `<METRICS>` 实为四处，两道验收口都判绿）** / 🔴 **`P-S8-46`（「例子要具体」与「主体禁含论文级字面量」互相拉扯，初版当场被判红）** / 🔴 **`P-S8-48`（「零命中」按字符串判与留痕互斥，43 号同型复发）**⟦⚠ 此处刻意**不写成 `P-S8-43` 的编号格式** —— 本列表的长度要能被 `grep -oE "P-S8-[0-9]+"` 机械核验，括号内作说明用的编号会混进计数（本轮初版即因此数出 23 而非 22）⟧ / 🔴 **`P-S8-49`（新增枚举成员的隐式跨文件契约，四处同步）** / 🔴 **`P-S8-51`（架构 §1.3 的 docstring 原文与同节条款打架，照抄会写进一个反向操作）** / 🔴 **`P-S8-52`（★★命门的取证夹具里没有 messages、也没有档位字段）** / 🔴🔴 **`P-S8-53`（AC 交叉引用整代偏移 9 处，机械筛只抓到 3 处）** / 🔴 **`P-S8-54`（半维护的抄写口：编号跟改、中文数词三轮没改）**）⟦🔴 **2026-08-08 订正**：本行**上一轮（20 → 25）整个漏跟**，四个同源汇总口只改了另外三处 ⇒ 同一份文档里两个数并存了半天。已登记 **`P-S8-32`**（`P-S8-22` 同族复发）⟧⟦🔴 **2026-08-09 补充实测**：`P-S8-32` 说勘误总数有"**4 个**同源汇总口"，**实为 5 个** —— 它漏了**文首 `:9` 那行「现行汇总账」**（该行同样写着"落点勘误 N 条"）。本轮已**五处一并**推到 `38` 条 / 实质性 `16` 条⟧
...
- [ ] **CP-2.5-4** ★★**两个闸同时为真**（~~AC-S8-05④ + AC-S8-04~~ **`AC-S8-06④` + `AC-S8-05`**⟦🔴 **2026-08-11 主控订正（`P-S8-53`）**：抄自架构 `:457`，**两个号同时错了一位**，已同批就地订正；详见 §15.0e⟧，架构 §3.3 明令"两条一起才叫验完"）：**正向**——`read_code_file` 读参考仓库**成功**；**负向**——引用参考仓库路径作物证**不成立**
...
| **落点勘误** | ~~**`P-S8-1` ~ `P-S8-11`**~~ ~~**`P-S8-1` ~ `P-S8-20`**（共 **20** 条）~~ ~~**`P-S8-1` ~ `P-S8-25`**（共 **25** 条）~~ ~~**`P-S8-1` ~ `P-S8-34`**（共 **34** 条）~~ ~~**`P-S8-1` ~ `P-S8-35`**（共 **`35`** 条）~~ ~~⇒ **`P-S8-1` ~ `P-S8-38`**~~ ⇒ **`P-S8-1` ~ `P-S8-54`**（🔴 **总数见文首「现行汇总账」权威行，本行 2026-08-09 起不复述**；构成：§15 本表 11 + §15.0 四条 + §15.0b 五条 + §15.0c 五条 + §15.0d 十三条 + ~~**§15.0e 六条**~~ **§15.0e `16` 条**⟦🔴 **2026-08-11 主控订正（`P-S8-54`）**：本处的「六条」**三轮推数（6→9→11→12）一次都没跟改**，而同一括号里的编号上界每轮都改了 ⇒ **同一个抄写口里的两个数独立老化**。加总现为 11+4+5+5+13+16 = **54**，与文首 `:10` 权威行一致⟧） | 本文 §15，**与 sprint7 的裸 `P-N`（已用到 P-73）显式分段** |
...
### 15.0e 🔴 批次 2 执行期新增勘误（~~**6 条**~~ ~~**9 条**~~ ~~**11 条**~~ ~~**12 条**~~ **`16` 条**：`P-S8-39 ~ ~~P-S8-44~~ `P-S8-54``，2026-08-10 ~ **08-11**，起点 `HEAD=ac60379`；**`P-S8-39~41` 由 `T-S8-2-2` 实现前核实挖出，`P-S8-42~44` 由 `T-S8-2-1` 落盘后真跑全量回归暴露，`P-S8-45~47` 由 `T-S8-2-1b` 实做时挖出**）

> **纪律**（沿 §15.0b / §15.0d 先例）：实做时凡与文档记载对不上、或按文档做会漏做/做错的，一律落进本表并写清磁盘实测值，**不默默按文档写的做**。
> 🔴 **本表 ~~3~~ ~~6~~ ~~9~~ ~~11~~ ~~12~~ `16` 条里有 ~~2~~ ~~3~~ `5` 条（`P-S8-39` / `P-S8-40` / **`P-S8-43`** / **`P-S8-51`** / **`P-S8-54`**）是同一份文档内部两处直接打架** —— 不是"文档与磁盘不符"，是"**文档与自己不符**"。⚠ **这类失真比对不上磁盘更危险**：执行者读到哪一处全凭运气，而两处都在同一份 dev-plan 里、都带着权威口吻。⇒ **`T-S8-2-2` 这类"实现前核实"任务的射程必须包含"同文档内互证"，不能只做"文档 vs 磁盘"。**
