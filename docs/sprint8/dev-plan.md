...
> 🔴 **2026-08-07 收口订正（约 20 处，一次做完）**：①**编号换发** `AR-S8-13` → **`AR-S8-15`**（与架构 v2.2 已占用号撞车，裁定正身已落架构 **v2.5 §17**；含一处**悬空引用** `§6` → **§17.6 第 2 条**）；②**架构号段口径** `AR-S8-01~08` → `AR-S8-01~15` 四处；③**§12 新增第 17 条 `R-S8-42`**（TypedDict 声明与构造点原子同批）；④**`T-S8-1a-2` 注销后的全部连带跟改**（§2 总表 / 三处悬空依赖 / §3.1 / §3.2 依赖图 / §3.4 关键路径 / §4 与 §5 文件边界与前置）；⑤**全部汇总口一次对平**——任务 41→**40**、检查点 328→**329**（逐批 25/45/148/83/16/12）、勘误 15→**20**、批次 2 任务 14→**15**、冻结表「10→6 行」→**「10→4 行」**四处；⑥`P-S8-17` 第③条**改判为"无须再做"**（架构 v2.4 早已改过）、`P-S8-20` **改判为"已修复"**（测试工程师 2026-08-07 落盘）。**旧结论一律划删保留、不洗白。**
...
**检查点总数**：~~328 个~~ **329 个**（分布见 §13 CP 索引。逐批小计：~~1a=32 / 1b=41 / 2=144 / 3=83 / 4=16 / 5=12~~ ⇒ **1a=25 / 1b=45 / 2=148 / 3=83 / 4=16 / 5=12**）
> 🔴 **本轮实算算式（2026-08-07，逐行加总，非沿用旧数）**：`25 + 45 + 148 + 83 + 16 + 12 = 329`。
...
> ⚠ **还有 1 条待定**：`CP-2.8-18`（旧快照 `result.get("conclusion") or {}` → `{}`）按原文是「批次 2 开工时先核对 `CP-2.8-1 ~ 16` 是否已覆盖：**有则登记吸收、不新设；无则新设**」⇒ **若新设，批次 2 = 149、总数 = 330**。本轮按"尚未新设"记 329，**待批次 2 开工核对后回填**。

**落点勘误**：~~15 条~~ **20 条**（§15；v1.0~v1.2 的 **P-S8-1 ~ P-S8-11**（11 条）+ v1.3 新增 **P-S8-12 ~ P-S8-15**（4 条，§15.0）+ 批次 1a 执行期新增 **P-S8-16 ~ P-S8-20**（5 条，§15.0b）。实质性 7 条：P-S8-3 / P-S8-4 / P-S8-5 / P-S8-9 / 🔴 **P-S8-12（架构未覆盖）** / 🔴 **P-S8-16（`AR-S8-15` 触发条）** / P-S8-18）
...
2. 🔴 **冻结表逐行对账：~~10 行 → 6 行~~ ⇒ 10 行 → 4 行**（§3.4 A 表 **v1.3 版**，AC-S8-21①，**v1.3 跟改架构 v2.4 §16.6 之后**）：
   > ⚠ **本条原写「10 行 → 6 行」，是 v1.2 时期的账，2026-08-07 订正**：v1.3 追加 `_collect_grouped_metrics` **整体删除**（推翻架构 v2.1 §13 的"不删、不改"），而它**本来就在冻结表 10 行内** ⇒ **5 行移出 + 1 行换发 = 6 行动，表 10 行 → 4 行**（剩 `_completion_insufficient` / `_apply_incomplete_execution` / `_reconcile_steps` / `_audit_declared_steps`）。**§3.4 A 表已于 v1.3 切版，本处漏跟改 ⇒ 照原文对账会当场对不平。** 记账见 §15.0 **P-S8-14**。
...
- [ ] **CP-2.12-2** ★**冻结表逐行对账**（AC-S8-21①，~~v1.2 订正~~ **v1.3 订正，2026-08-07 补跟改**）：~~10 行 → 6 行（移出 4 + 换发 1 + 不变 5）~~ ⇒ **10 行 → 4 行**（**移出 5**：`_apply_no_metrics` / `_extract_metrics_block` / `_parse_metrics` / `_regex_scan_metrics` / **`_collect_grouped_metrics`** + **换发 1**：`_build_execution_result` + **不变 4**：`_completion_insufficient` / `_apply_incomplete_execution` / `_reconcile_steps` / `_audit_declared_steps`）；**逐行写明原因**；表结构未被整表删除、断言未被改成「不少于」
...
**自测检查点**：
- [ ] **CP-4.2-1** ★A 表逐行对平（~~v1.2 裁定 1 之后~~ **v1.3 跟改架构 v2.4 §16.6 之后；2026-08-07 补跟改**）：~~移出 4 行 + 换发 1 行 + 不变 5 行；冻结表 10 行 → 6 行~~ ⇒ **移出 5 行 + 换发 1 行 + 不变 4 行；冻结表 10 行 → 4 行**，**逐行原因可查**（⚠ 与 `CP-2.12-2` 同源失真，两处已一并订正，见 §15.0 P-S8-14）
...
17. 🔴 **`R-S8-42`：`TypedDict` 的键声明必须与它的构造点原子同批（同一次 commit）。** 加键（`[typeddict-item]`）、删键（`[typeddict-unknown-key]`）**双向适用**；**读侧不受此限**（`.get()` 读未声明键 mypy 不报）。⚠ **例外面**：`mypy.ini:150` 已为 `reporting.py` 压制 `typeddict-item` ⇒ **该文件的构造点缺键静态检查抓不到**，须靠人工与用例守。全裁见架构 `AR-S8-15` / §17.5。
    > **本 Sprint 的两处落地**：`success_criteria` 声明随 `T-S8-1b-2`（与 `planning.py` 两处构造点同批）、`conclusion` 声明随 `T-S8-2-8`（与 `_build_execution_result` 换发 + `:2908` 降级构造点 + `metrics`/`metrics_groups` 删键同批）。
    > 🔴 **须有验红活体证明，否则该纪律无门**（架构 §17.5）：`CP-1b.2-11` 追加一项「⑥ 把 `success_criteria` 从 `planning.py` 任一处构造点删掉 → `.venv/bin/mypy` **必红** `[typeddict-item]`」；`CP-2.8-*` 的验红条同款追加一项，针对 `execution.py:2908` 降级构造点。
    > ⚠ **本条与上方第 5 条口径冲突**（第 5 条仍写「`core/state.py` 批次 1a 一次收口」，是 `AR-S8-15` 之前的账）——**以本条为准**；第 5 条的跟改未在本轮授权范围内，已回报主控（见本轮交付说明"新发现"第 1 条）。

---

## 13. 检查点总览（CP 索引，共 ~~328~~ **329** 个）
...
| | **T-S8-2-1b** | CP-2.1b-1 ~ ~~9~~ **10** | ~~9~~ **10** | ★★改前必红（先建后改活体证明；**门在 1a 已建**）/ 字节零扰动（**v1.2 迁入**）/ **三件套闭合自证（`CP-2.1b-10`，由 `CP-1a.5-3b` 改判迁入，P-S8-19）** |
...
| | T-S8-2-12 | CP-2.12-1 ~ 8（含 2b / 2c） | 10 | ★冻结表 ~~**10→6 行**~~ **10→4 行**对账（**v1.3 账；F4 同源失真已一并订正**）/ 🔴 **e2e 哑弹 grep** / 验红总账 |
| | **批次 2 小计** | | ~~144~~ ~~145~~ **148** | **10+10+6+6+10+15+13+9+19+6+11+7+8+8+10 = 148**。两笔：**+1** = `CP-2.1b-10`（由 `CP-1a.5-3b` 改判迁入，P-S8-19）；**+3** = `CP-2.8-17/19/20`（由 `T-S8-1a-2` 迁入，`AR-S8-15`）。⚠ ~~"六个小计合计 31+41+145+83+16+12 = 328，本次是批内等量搬移，总账未动"~~ ⇒ **该结论只对第一笔成立**；第二笔是 **6 出 7 进的净增**，总账**已动**（328 → 329），实算见 §2 摘要行与本节末合计行 |
...
| | **合计** | | ~~328~~ **329** | 🔴 **实算：25 + 45 + 148 + 83 + 16 + 12 = 329**（2026-08-07 逐行加总）。⚠ **`CP-2.8-18` 若在批次 2 开工核对后新设 ⇒ 批次 2 = 149、合计 = 330**，届时须同步 §2 摘要行 / 本节标题 / 本行 / §14 编号速查**四处**，别只改一处 |
...
| 检查点 | `CP-1a.1-1` ~ `CP-5.2-4` | ~~共 300 个（逐批：1a=32 / 1b=41 / 2=126 / 3=74 / 4=15 / 5=12）~~ ⇒ 共 **329** 个（逐批：**1a=25 / 1b=45 / 2=148 / 3=83 / 4=16 / 5=12**，实算 `25+45+148+83+16+12=329`）。⚠ **本行原数同为 v1.2 时期的账**，与 §2 摘要行、§13 索引长期打架，2026-08-07 三处一次对平 |
| **落点勘误** | ~~**`P-S8-1` ~ `P-S8-11`**~~ ⇒ **`P-S8-1` ~ `P-S8-20`**（共 **20** 条：§15 本表 11 + §15.0 四条 + §15.0b 五条） | 本文 §15，**与 sprint7 的裸 `P-N`（已用到 P-73）显式分段** |
...
> 🔴 **口径订正（2026-08-07）：勘误现共 20 条，不是 11 条。** 本表 **11 条**（`P-S8-1 ~ P-S8-11`，v1.0~v1.2 落盘期）+ **§15.0 四条**（`P-S8-12 ~ P-S8-15`，v1.3 跟改期）+ **§15.0b 五条**（`P-S8-16 ~ P-S8-20`，批次 1a 执行期）= **20 条**，实质性 **7 条**：P-S8-3 / P-S8-4 / P-S8-5 / P-S8-9 / **P-S8-12** / **P-S8-16** / P-S8-18。
...
*（Sprint 8 开发计划 ~~**v1.2**~~ **v1.3（+ 2026-08-07 收口订正）** 完。~~**6 个批次 / 39 个任务 / 300 个检查点 / 11 条落点勘误 / 7 条新登记风险（R-S8-18~24）**~~ ⇒ **6 个批次 / 40 个任务 / 329 个检查点 / 20 条落点勘误（P-S8-1~20）/ 7 条新登记风险（R-S8-18~24）+ 1 条架构升格纪律（`R-S8-42`）** / 1 条新登记缺陷（BUG-S8-01，验收点 DA-S8-1）/ **4 项裁定全部结案（§16.A~D，🟢 零开放项）+ 架构文档同批跟改至 ~~v2.1~~ **v2.5**（§16.E + `AR-S8-15` / §17）**。** 本计划只写规格与检查点，**未写任何生产代码、未改 `docs/TODO.md`**（批次 1a 的实测数字由开发代理回填）。*
