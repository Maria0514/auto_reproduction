...
**版本**：**v1.1**（v1.0 = 2026-08-04 初稿；**v1.1 = 同日回填 Maria 两条拍板**：①AC-S8-14「可编辑」按只读展示验收；②三档解绑任务由批次 1a 迁至批次 2 首位、不可用窗口压进批次 2 内部。连带同步见 §0.0 / §3.3 / §14.1 / §16）
**日期**：2026-08-04
**作者**：全栈开发工程师代理
**状态**：草案（待 Maria 审阅后**逐批**授权执行）
**对应 PRD**：`docs/sprint8/prd.md` **v3.0**（S8-01 ~ S8-11 十一条全 P0；AC-S8-01 ~ AC-S8-25；R-S8-01 ~ R-S8-17；A-S8-01 ~ A-S8-10；Q-S8-01 ~ Q-S8-07）
**对应架构**：`docs/sprint8/architecture.md` **v2.0**（Q-S8-01 ~ Q-S8-09 九项裁决 + §12 文件级开发交接清单 + AR-S8-01 ~ AR-S8-08 + §14 跟改说明 + §15 护栏 3 落点）
...
> | **Maria 拍板** | 把解绑任务（原 `T-S8-1a-7`）**挪到批次 2 首位**，现编号 **`T-S8-2-1`** |
...
3. 🔴 **coding prompt 字节门（T-S8-1a-3）不可裁、不可顺延**。理由：它是 T-S8-1a-4 的**前置**——“先建后改”一旦变成“改完再建”，建出来的基线是照着改后字节写死的、**永远绿**，与自锁定形态等价（R-S7-41 / sprint7 P-27 两次实证）。**要么在改 prompt 之前建，要么这道门根本不该记进交付。**
...
### 1.6 🔴 BUG-S8-01：非 str `group` 被静默吞成主实验指标（本计划新登记，纯 bug 修复）
...
| **T-S8-1a-4** | S8-02 | coding 侧：三处 `<METRICS>` 清除 + 产出约定 + 上下文补两键 + 基线换发 | `core/nodes/coding.py` | **T-S8-1a-3**（门须先建好）+ T-S8-1a-2 | 中 | 中（首次改 coding 冻结区） |
| **T-S8-1a-5** | S8-03 | execution 只读工具接入（`read_code_file` / `list_dir`） | `core/nodes/execution.py` | T-S8-1a-2 | 低 | 中（两个闸不许合并，Q-S8-03） |
| **T-S8-1a-6** | S8-10 + 前置① | execution 上下文补 `baseline_results` + `success_criteria`（非空才注入） | `core/nodes/execution.py` | T-S8-1a-5（同文件串行） | 中 | 中（字节零扰动是命门） |
| **T-S8-1a-7** | 收口 | 既有断言同步 + 批次 1a 自测收口门（**要求全绿**） | `tests/`（同步面） | T-S8-1a-4 + T-S8-1a-6 | 中 | 中 |
...
| **T-S8-2-2** | 前置 | 实现前核实 + **冻结表 / 三处 prompt 旧哈希先记后改** | `/tmp` + §15 回填 | T-S8-2-1 | 低 | 低（不做等于门白建） |
...
| **T-S8-2-10** | S8-06 + **BUG-S8-01** | `_split_reported_metrics`：撞名两条都丢弃 + WARNING；**非 str `group` 跳过 + WARNING** | `core/nodes/execution.py` | T-S8-2-9 | 中 | 中 |
...
| **T-S8-3-9** | S8-07 | `result_report.py` 数据源改 `metrics_groups` + 档位读 `conclusion.level` + `_FORM_CARD_SPEC` / `_conclusion_card_key` 跟改 | `ui/pages/result_report.py` | T-S8-3-3/8 | 中 | 中（P-S8-4 下半） |
...
**任务总数**：**39 个**（1a×**7** + 1b×5 + 2×**12** + 3×10 + 4×3 + 5×2）
**批次数**：**6 个**（1a / 1b / 2 / 3 / 4 / 5）
**检查点总数**：**296 个**（分布见 §13 CP 索引；收口门 CP 为各批总闸门。逐批小计：1a=47 / 1b=41 / 2=107 / 3=74 / 4=15 / 5=12）
**落点勘误**：**11 条**（§15，其中 **P-S8-3 / P-S8-4 / P-S8-5 / P-S8-9 四条为实质性**）
...
|---|---|---|---|---|---|
| **1a** | 能力接入（通道不退场） | T-S8-1a-1 → 1a-7（**串行**，**7 个任务**） | Q-S8-06 已裁 + Q-S8-02 字段名已裁（架构 §2.5 ⇒ 依赖即刻解除） | AC-S8-04/05/15/21② | 🔴 **T-S8-1a-3 必须早于 T-S8-1a-4**（先建后改）；✅ **本批落盘后系统仍可真跑、可演示**（拍板 2）；**收口门要求全绿**；`core/state.py` **本批一次收口，此后全 Sprint 零改动** |
...
| **2** | **通道退场** + 验钞 + 四档判定 + 维度不坍缩 | T-S8-2-1 → 2-12（**串行，`execution.py` 单收口窗口**，**12 个任务**） | 批次 1a + 1b 全部收口 | **AC-S8-03** + AC-S8-06/07/08/09/10/11/12/16/18/19/22 | 🔴 **PRD 明令内部不得拆分**（R-S8-06）；🔴 **不可用窗口整体落在本批内部**：T-S8-2-1 起、**T-S8-2-11 是可用性恢复点**；🔴 **T-S8-2-1（解绑）与 T-S8-2-8（四档判据取代旧判据）是天然配对，中间不得留缝** |
...
```mermaid
graph TD
    subgraph B1a["批次 1a · 能力接入（通道不退场）"]
        A1[T-S8-1a-1 实现前核实 + 基线重测]
        A2[T-S8-1a-2 state 两键一次收口]
        AG["T-S8-1a-3 ★新建 coding 字节门（先建后改）"]
        A3[T-S8-1a-4 coding 三处清除 + 产出约定 + 基线换发]
        A4[T-S8-1a-5 execution 只读工具接入]
        A5[T-S8-1a-6 baseline_results + success_criteria 注入]
        A7[T-S8-1a-7 断言同步 + 1a 收口门（要求全绿）]
    end
...
**关键路径**：`T-S8-1a-2`（state 两键）→ 批次 1a 收口（**此时系统仍可用**）→ `T-S8-2-1`（解绑，**不可用窗口起点**）→ 批次 2 十二任务串行 → `T-S8-2-11`（**可用性恢复**）→ `T-S8-2-12`（批次 2 收口）→ 批次 3 → 批次 4 → 批次 5。
...
**为什么批次 2 十二个任务全部串行**：全部落在 `core/nodes/execution.py` 一个文件，且后一个任务的输入是前一个任务的产物（解绑 → 档名常量 → 判定函数 → 改判 → 结果构造 → prompt → 接线）。sprint7 批次 1 的教训（§5 单收口窗口令）在这里加倍成立。
...
| `_apply_no_metrics` | **删除** | **整行移出冻结表**（函数不存在了） | T-S8-2-7 / 2-11 |
| `_apply_incomplete_execution` | **否** | 一字不动，哈希不变 | — |
| `_build_execution_result` | **是**（+`conclusion` 形参、`success` 改派生） | **换发哈希 + 写明原因** | T-S8-2-8 / 2-11 |
| `_reconcile_steps` | **否** | 一字不动 | — |
| `_audit_declared_steps` | **否** | 一字不动 | — |
| `_extract_metrics_block` | **默认否**（只解绑不改体） | 哈希不变；**若 §16.1 改判为删除则整行移出** | T-S8-2-1 |
| `_parse_metrics` | **是**（清死参数 `plan`） | **换发哈希 + 写明原因** | T-S8-2-1 / T-S8-2-12 |
| `_collect_grouped_metrics` | **否**（保留为兜底，架构 §13） | 一字不动 | — |
| `_regex_scan_metrics` | **默认否**（只解绑不改体） | 同 `_extract_metrics_block` | T-S8-2-1 |

⇒ **默认取值下：2 行换发（`_build_execution_result` / `_parse_metrics`）+ 1 行移出（`_apply_no_metrics`）= 3 行动**。
⚠ **PRD §4.2 的"至少 6 行必红"是按"三档函数一并删除"估的**，与 AC-S8-03 的判定方式（"不再被**判定链路调用**"）口径不同。见 §15 **P-S8-9** + §16 **16.1**。**账目以本表逐行为准，不拿"至少 6 行"当靶。**
...
| **coding** | 🔴 **零字节门**（sprint7 P-64 + `docs/TODO.md:633` 开放遗留，本计划复核仍成立） | 🔴 **新建**（**单列任务 T-S8-1a-3**，先建后改） | 建门 **T-S8-1a-3** / 换发 **T-S8-1a-4** | AC-S8-21② 说“换发”，**实为新建**（P-S8-5）。唯一沾边的 `test_sprint5_t13_coding_prompt.py:180-183` 是**自锁定形态、零守门效力**。走三件套：旧哈希先记（T-S8-1a-1）→ 建门并验红（T-S8-1a-3）→ 改后重算写死字面量 + §15.1 留档（T-S8-1a-4） |
...
> **文件边界**：`core/state.py`（T-S8-1a-2 **一次收口**）+ `tests/`（T-S8-1a-3 建门，**零生产改动**）+ `core/nodes/coding.py`（T-S8-1a-4）+ `core/nodes/execution.py`（T-S8-1a-5/1a-6，**单收口窗口，串行**）+ `tests/`（T-S8-1a-7）。**不碰** `planning.py` / `plan_checks.py` / `reporting.py` / `ui/` / `graph.py` / `config.py`。
> **红线**：`_EXECUTION_SYSTEM_PROMPT_BODY` **本批一字不动**（改写整体在 T-S8-2-9，架构 §6.2④「哈希基线只换发一次」的直接推论，见 §16.2）；工具层 `code_fs_tools.py` 一字不动；`_collect_grouped_metrics` 一字不动。
...
5. **记下 coding prompt 主体的旧哈希**（三件套第①件，不做等于 T-S8-1a-4 的门白建）：`sha256(_CODING_SYSTEM_PROMPT_BODY.encode())[:16]` + 字符长度。sprint7 §60.2 事实 15 记的是 `37ec6ee2b1606715` / 3052 字符，**须现算复核**。
...
5. **§15.1 留档**（三件套第③件）：旧哈希 + 长度 + 建门时点 + 建门原因。**T-S8-1a-4 改完 prompt 后回来把新哈希补上同一行。**
...
- [ ] **CP-1a.3-8** §15.1 留档已写（旧哈希 + 长度 + 时点 + 原因；新哈希留空待 T-S8-1a-4 回填）

> **⚠ 顺序纪律（本批最容易做反的一件事）**：**T-S8-1a-3 必须在 T-S8-1a-4 之前完成并绿**。若先改了 `coding.py` 再建门，建出来的就是"照着改后字节写死的基线"——它**永远绿**，与自锁定形态等价，**这道门等于没建**。

### 任务 T-S8-1a-4：coding 侧三处 `<METRICS>` 清除 + 产出约定 + 上下文补两键

- **产出文件**：`core/nodes/coding.py`
- **依赖项**：**T-S8-1a-3（字节门必须先建好并绿）** + T-S8-1a-2（`success_criteria` 键名）
- **预计复杂度**：中
...
**自测检查点**：
- [ ] **CP-1a.4-1** **三处清除逐处验**（AC-S8-03 负向）：`grep -n "METRICS" core/nodes/coding.py` **零命中**；且分别断言 `:113` description、`:181-186` 段、`:191` 那句**三处各自不存在**（**逐处断言，不用一条全局 grep 代替**——全局 grep 过了不代表三处都清了，可能是清了一处另两处被改写成别的写法）
- [ ] **CP-1a.4-2** 产出约定三要素齐全（AC-S8-04 提示词面）：主体含 `summary.json` 字面量 + "顶层"对象约束措辞 + 至少一个 JSON 例子 + "落在计划声明的位置"这层边界
- [ ] **CP-1a.4-3** **负向：不得规定字段名**（防复发"代码猜产物"路线）：新增段落中不出现任何具体指标字段名的强制要求措辞（人工审 + 关键词负向断言）
- [ ] **CP-1a.4-4** 上下文补两键（AC-S8-04 上下文面）：计划含 `expected_results` / `success_criteria` 时，`_build_coding_context`（或等价上下文构造点）产出的 payload **含这两键**
- [ ] **CP-1a.4-5** ★**字节零扰动**：两键均缺 / 均为空 / 一有一无 三种形态下，payload 与改前基线**逐字节相同**（`json.dumps(sort_keys=True)` 比对）
- [ ] **CP-1a.4-6** **Prompt Cache 幂等**（已知 bug 模式 #4）：正则 `\d{4}\.\d{4,5}` 在主体零命中；两次不同 state 下 `_build_coding_system_prompt(...)` 的**主体部分逐字节相同**（沿 `test_paper_analysis_e2e.py` 的"截 SystemMessage 去尾部段落后比较"范式）
- [ ] **CP-1a.4-7** ★★**先建后改的活体证明**：改 prompt **之前**先跑一次 T-S8-1a-3 建的门 → **实测变红**并打出新旧两个哈希，**红的证据落档**。🔴 **没红就停手回查 T-S8-1a-3**——那说明门是假的（自锁定或守错了对象），此时继续改 prompt 等于门白建
- [ ] **CP-1a.4-8** 字节门基线换发闭合（三件套第②③件）：新哈希 + 新长度算出并**写死为字面量**；断言右侧仍是硬编码字面量（元检查复跑 CP-1a.3-2）；门**复绿**；§15.1 那一行补齐新哈希
- [ ] **CP-1a.4-9** **逐条验红**（`cp` 备份 → 改坏 → 记录 → `cp` 还原 → `sha256sum -c` → 复绿）：①主体再插一个空格 → 字节门必红并打出新旧哈希；②删掉新增产出约定段 → CP-1a.4-2 红；③把例子换成含 `arxiv_id` 的动态形态 → CP-1a.4-6 红；④删掉 `success_criteria` 注入 → CP-1a.4-4 红

### 任务 T-S8-1a-5：execution 只读工具接入（S8-03）
...
**自测检查点**：
- [ ] **CP-1a.5-1** 工具列 **3 → 5**，且新增的恰是 `read_code_file` / `list_dir` 两个（按 `tool.name` 断言，**不按下标**）
- [ ] **CP-1a.5-2** 既有三个工具的**装配顺序与构造实参一字不动**（`prepare_environment` / `run_in_sandbox` / `request_user_input`）
- [ ] **CP-1a.5-3** ★**正向**：执行环节**能读参考仓库**（构造 `selected_repo.local_path` 下的文件 → `read_code_file` 成功返回内容）
- [ ] **CP-1a.5-4** ★**负向**：工作区之外的路径**被拒**（`_is_within_workspace` 既有拦截用例全绿，`code_fs_tools.py` 逐字节未改，`git diff` 为空自证）
- [ ] **CP-1a.5-5** **不得写代码的硬防线零改动**（AC-S8-05）：`execution.py:995` 与 `:1010` 两处逐字节未改；既有内联写码拒绝用例 + 管道拒绝用例全绿
- [ ] **CP-1a.5-6 逐条验红**：①去掉 `read_code_file` 装配 → CP-1a.5-1 红；②把 `_is_within_workspace` 改成永远返回 True → CP-1a.5-4 红（**验后立即 `cp` 还原 + `sha256sum -c`**）

### 任务 T-S8-1a-6：execution 上下文补 `baseline_results` + `success_criteria`（S8-10 + 架构 §11 前置①）
...
5. ⚠ **配套提示词约束（「论文没报这个数也是合法结论」）不在本任务**——它属 prompt 主体，随 T-S8-2-9 一次落（本批红线 + §16.2）。**本任务只做通道，不做措辞。**
6. 🔴 **既有 payload 键的构造顺序与取值一字不动**（架构 §12）。

**自测检查点**：
- [ ] **CP-1a.6-1** 有 `baseline_results` 时 payload 含该键，值与 state 中的**逐字相同**（原样透传，不加工）
- [ ] **CP-1a.6-2** ★**命门·字节零扰动**（AC-S8-15②）：`baseline_results` 缺失 / 为 `None` / 为 `{}` / 为非 dict **四种形态**下，payload 与 sp7 基线**逐字节相同**（⇒ **既有 HumanMessage 字节基线不换发**，只新增"有该值"一条基线）
- [ ] **CP-1a.6-3** `success_criteria` 同款四形态字节零扰动；有值时注入且**原文照传**（不摘要、不截断）
- [ ] **CP-1a.6-4** ★**系统提示词主体零改动**（AC-S8-15③）：`_EXECUTION_SYSTEM_PROMPT_BODY` 本任务前后**逐字节相同**（`sha256` 比对，值与 T-S8-2-2 记录的旧基线一致）
- [ ] **CP-1a.6-5** 字节幂等：同一 state 连跑 3 次 `_build_execution_agent_context` 的 `json.dumps(sort_keys=True)` **逐字节相同**；`baseline_results` 键序打乱后重建，序列化结果仍相同（证明 `sort_keys` 生效）
- [ ] **CP-1a.6-6 逐条验红**：①去掉 `baseline_results` 注入 → CP-1a.6-1 红；②把"非空才注入"改成无条件注入 → CP-1a.6-2 红；③去掉 `sort_keys=True` → CP-1a.6-5 后半红

### 任务 T-S8-1a-7：既有断言同步 + 批次 1a 自测收口门

- **产出文件**：`tests/`（同步面，只换不弱化）
- **依赖项**：T-S8-1a-4 + T-S8-2-1
- **预计复杂度**：中
...
3. **本批只同步"归属明确且在本批就该改"的断言**：
   ...
   - 🔴 **coding prompt 字节门基线**（T-S8-1a-3 建、T-S8-1a-4 换发）→ 确认门**当前是绿的**且断言右侧仍是硬编码字面量；确认 `test_sprint5_t13_coding_prompt.py:180-183` 那条自锁定断言**仍在且仍绿**（它守的是"稳定前缀 == 两常量拼接"的结构关系，本批未改该结构）。
...
**自测检查点**：
- [ ] **CP-1a.7-1** ★**全量非 e2e 回归零失败**（含时间点）—— 拍板 2 之后本批**不允许带红收口**
- [ ] **CP-1a.7-2** ★**可用性自证**：本批落盘后系统**仍可判出 `success is True`**（构造 exit 全 0 + 步骤跑完 + `<METRICS>` stdout → `metrics` 非空、`success is True`）⇒ **证明不可用窗口确实没有在本批开启**
- [ ] **CP-1a.7-3** 本批该同步的断言已同步（coding 三处负向 / coding 字节门基线已换发且绿）；🔴 **`_parse_metrics` 相关与冻结表逐行保持原样**（负向：本批 `git diff` 中冻结表零改动）
- [ ] **CP-1a.7-3b** ★**三件套闭合自证**：coding 侧的旧哈希（CP-1a.1-5）、建门时的红（CP-1a.4-7）、换发后的新哈希（CP-1a.4-8）**三个证据齐全并落在 §15.1 同一行**——缺任一件，这道门在交付说明里**不得声称"已建"**
- [ ] **CP-1a.7-4** 禁弱化自查通过（`git diff tests/` 四类零新增；删除的断言逐条能说出等强或更严的替换关系）
- [ ] **CP-1a.7-5** **文件边界自查**：`git status --porcelain` 生产侧**只有** `core/state.py` / `core/nodes/coding.py` / `core/nodes/execution.py` 三个文件；`planning.py` / `plan_checks.py` / `reporting.py` / `ui/` / `graph.py` / `config.py` **逐一零改动**
- [ ] **CP-1a.7-6** `mypy` 清缓存后零错误（`rm -rf .mypy_cache` 再跑）

> **批次 1a 收口门**：CP-1a.1-\* ~ CP-1a.7-\* 全绿 + ✅ **全量非 e2e 回归零失败**。
> 🔴 **停手等 Maria 确认再开下一批。**
...
> **产出**：**`<METRICS>` 执行侧通道退场** + `_resolve_agent_report` + `_verify_evidence` 五重验钞 + `_decide_conclusion` 四档判定 + 三条封顶 + 新错误类别 + `success` 由 `level` 派生 + 撞名两条都丢弃 + **BUG-S8-01**。
> **文件边界**：`core/nodes/execution.py` **独占**（**十二任务串行**，**单收口窗口，主控收口令**）+ `tests/`（T-S8-2-12）。**不碰** `state.py`（1a 已收口）/ `coding.py` / `planning.py` / `plan_checks.py` / `reporting.py` / `ui/` / `graph.py` / `config.py`。
> 🔴 **PRD 明令批次 2 内部不得拆分**（R-S8-06）：档位判定与本篇标准**互为前提**——没有标准无从判档，没有判档标准写了也没人用。**十二个任务是一个交付单元，不得只交前几个。**
>
...
### 任务 T-S8-2-1：🔴 三档判定链路解绑 + `_parse_metrics` 死参 + `llm_calls_used` 归零 —— ★**不可用窗口起点**

> **📌 迁移留痕**：本任务在本计划 v1.0 中编号为 **`T-S8-1a-7`**（批次 1a 第 7 位）。**Maria 2026-08-04 拍板 2** 把它挪到**批次 2 首位**，以把「一律判失败」的不可用窗口**压缩进批次 2 内部**，让批次 1a 落盘后系统仍可真跑、可演示。**旧号 `T-S8-1a-7` 现已改指批次 1a 的收口任务，全文不再有第二套口径**（见 §14 编号速查的迁移表）。

- **产出文件**：`core/nodes/execution.py`（`execution()` 主体 `:2935` 调用点 + `_parse_metrics` 签名 `:517-521`）
- **依赖项**：**批次 1a + 批次 1b 全部收口**（本任务是批次 2 的首位任务）
- **预计复杂度**：中（改动小，**但影响面是全 Sprint 最广的一次**）
- **架构参考**：§12 `execution.py` 第 2 条 + PRD §4.2 第 4/5 条 + AC-S8-03
...
0. 🔴 **开工第一步（零生产改动，因本任务被前移到批次 2 首位而随之前移）**：先记下 **`_parse_metrics` 的改前源码哈希**与**冻结表 10 行的当前值**。理由：本任务会改 `_parse_metrics` 签名 ⇒ 其冻结哈希必变；而记录冻结表基线的 T-S8-2-2 现在排在本任务**之后**，若不前移这一步，T-S8-2-2 记到的就是**改后**的值，T-S8-2-12 的逐行对账当场失去参照。⚠ **其余基线（execution prompt 两处旧哈希等）不受本任务影响，仍由 T-S8-2-2 记录。**

1. **判定链路解绑**（AC-S8-03 的判定方式是"**调用链断言**"，不是"函数已删除"）：`execution()` 主体 `:2935` 不再调 `_parse_metrics`；`metrics` 的来源改为**只剩 agent 自报**（本任务落盘时门控还在，所以恒空——这正是中间态）。
2. **`llm_calls_used` 归零**（⚠ **架构 §12 未提，本计划实测补出，见 §15 P-S8-6**）：原 `metrics, llm_calls_used = _parse_metrics(...)` 的第二返回值流向 `_map_execution_result(..., llm_calls_used=llm_calls_used)`（`:3016`）→ 预算扣减 `total_calls = react_rounds_used + llm_calls_used`（`:2533`）。解绑后档 3 LLM 抽取不再发生 ⇒ **该支路恒 0**。
...
3. **`_parse_metrics` 清死参数**（PRD §4.2 第 5 条）：删掉从未被函数体使用的 `plan` 形参（§1.5 事实 6 已实测属实）。⇒ 该函数源码字节变 ⇒ **冻结表该行必换发**（§3.4 A 表）。
4. **三档函数本体的去留**：**默认保留 + 只解绑**（严格照架构 §12 的"判定链路解绑"字面）。⇒ `_extract_metrics_block` / `_regex_scan_metrics` / `_llm_extract_metrics` **函数体一字不动**，冻结表对应行**哈希不变**。
   > ⚠ 这会留下**死代码**（解绑后 `_parse_metrics` 及其三档无生产调用者）。是否一并删除 ⇒ **§16.1**，**默认不删**。理由：①AC-S8-03 的判定方式只要求"不再被判定链路调用"；②PRD §4.2 第 5 条要"清 `_parse_metrics` 的死参数"这个动作本身就预设了它还在；③删除会把冻结表必红行数从 3 推到 6，账目面扩大而收益为零。
5. **在解绑处写一条注释**，逐字写明：`<METRICS>` 通道退场（Maria 决策 3，是废掉不是收窄）、`metrics` 现在的唯一来源、**以及"在 T-S8-2-8 装上新判据之前 `success` 会恒假、这是计划内的"**（**给后人看的**，防止有人在窗口期看到全判失败就来"修"）。

**红线**：
- 🔴 **不得顺手把 `success` 的 `len(metrics) >= 1` 合取项一起改掉**——那是 T-S8-2-8 的事。批次 2 内部不得拆分，反过来说批次 1a 也不得提前吃批次 2 的活。
- 🔴 **不得为了让回归变绿而回滚本任务**（§0.0 第 4 条）。
- 🔴 `_collect_grouped_metrics` 一字不动（架构 §13：它是 agent 完全不服从时唯一还剩的数据来源）。

**自测检查点**：
- [ ] **CP-2.1-0** ★**改前基线前移记录**（零生产改动）：`_parse_metrics` 改前源码哈希 + 冻结表 10 行当前值**现算落档**（T-S8-2-12 逐行对账的参照）
- [ ] **CP-2.1-1** ★**调用链断言**（AC-S8-03 执行侧）：`execution()` 主体中 `_parse_metrics` **零调用**；`_extract_metrics_block` / `_regex_scan_metrics` / `_llm_extract_metrics` 在**判定链路上零可达**（用 `ast` 扫 `execution()` 函数体 + 其直接被调函数，断言三者不出现）
- [ ] **CP-2.1-2** `_parse_metrics` 签名不再含 `plan`；旧签名调用（三参）**会报 TypeError**（证明真的改了签名，不是留了个兼容形参）
- [ ] **CP-2.1-3** ★`llm_calls_used` 支路归零：解绑后 `_map_execution_result` 收到的 `llm_calls_used` 恒为 `0`；预算扣减 `total_calls == react_rounds_used`（**这条是本计划实测补出的，架构没写，必须验**）
- [ ] **CP-2.1-4** 三档函数**本体逐字节未改**（`ast.get_source_segment` + `sha256`，与 **CP-2.1-0** 前移记录的冻结表基线逐一相同）；`_collect_grouped_metrics` 同
- [ ] **CP-2.1-5** ★**不可用窗口如实开启**（不是"验证它坏了"，是**验证它按预期地坏**）：构造"exit 全 0 + 步骤跑完 + agent 自报 1 个主实验指标"→ `metrics == {}`（门控不采信）+ `success is False` + WARNING 已打。**这条 CP 在 T-S8-2-11 之后期望值会翻转**，届时换发（沿 sprint7 §49.0「删原内容→换发新 CP」先例）
- [ ] **CP-2.1-6** 解绑处注释含中间态告示的指路文字（元检查，防后人误"修"）
- [ ] **CP-2.1-7 逐条验红**：①把 `_parse_metrics` 调用加回去 → CP-2.1-1 红；②给 `_parse_metrics` 留一个 `plan=None` 兼容形参 → CP-2.1-2 红
...
### 任务 T-S8-2-5：`_verify_evidence` 五重验钞（S8-04，AC-S8-06 命门）
...
`_verify_evidence(evidence_item, code_output_dir, extra_commands) -> (ok, reason)`，**五重全过才采信；任一不过，该条判断不成立**：
...
1. **第④重的实现 = 4 行自写**，与 `reporting._resolve_report_path`（`:371-372`）、`code_fs_tools._is_within_base`（`:82-91`）**同一判定路径**：
   ...
   - 🔴 **两个闸物理分处两文件，不可能被合成一个**（架构 §3.3，与 T-S8-1a-5 表同一份，须逐字进交接文档）。
...
- [ ] **CP-2.5-9** ★★**AC-S8-06 验红（逐一放宽每一重）**：逐一把每一重改成恒真 → **对应用例各自必红**（**五次验红，一次都不能省**）。⚠ 沿 sprint7 P-72 的教训：**首轮没变红的，当场加固 fixture 后复验，并如实登记"首轮无牙"**
...
- [ ] **CP-2.8-6** 降级构造点（`:2908-2917`）已补 `conclusion={}`；该路径产出的 `ExecutionResult` **键数为 12**
...
> 🔴 **本任务是"第一层"的唯一物理落点之一**（另一处是 T-S8-2-3 的模块常量）。**四档语义段必须写在系统提示词主体里**（稳定前缀，进哈希基线），**达标线必须在 HumanMessage 里**（T-S8-1a-6 已建通道）。**两者不得混在同一段文本。**
...
4. **配套约束（`baseline_results` 那条）**：明说「**论文没报这个数也是合法结论，不得硬凑一个"对上了"**」（PRD §4.10 第 4 条 / R-S8-04）。
   > ⚠ 这条本属批次 1a 的 S8-10，但**按架构 §6.2④「哈希基线只换发一次」的推论合并到本次改写**（§16.2 已登记）。
...
6. **工具说明段同步**：介绍 T-S8-1a-5 新绑的两个只读工具，并写明 **`code_output_dir` 之外的路径可以读、但不能当物证**（第④重的口语化表述）。
...
**自测检查点**：
- [ ] **CP-2.9-1** ★**先改前验红**：改 prompt 前先跑两处门 → **必红**并打出新旧哈希（沿 T-S8-1a-4 同款"活体证明"）
...
### 任务 T-S8-2-10：`_split_reported_metrics` 撞名两条都丢弃 + 🔴 **BUG-S8-01 非 str `group`**

- **产出文件**：`core/nodes/execution.py`（`_split_reported_metrics` `:1781`，docstring `:1796-1799` + 代码 `:1826-1831`）
- **依赖项**：T-S8-2-9
- **预计复杂度**：中
- **架构参考**：AR-S8-08 + PRD §4.6 + 本计划 **§1.6 BUG-S8-01**
...
**自测检查点**：
- [ ] **CP-2.10-1（AC-S8-19）★** 真跑夹具重放：`tests/fixtures/s713_realrun_20260802/` 的 24 条**零撞名全装下**
- [ ] **CP-2.10-2** ★**撞名且值不同 → 两条都丢弃 + WARNING**；**值相同的重复 → 按一条收，不误报**
- [ ] **CP-2.10-3** ★**容器类型契约零改动**：`ExecutionResult.metrics_groups` 类型签名逐字未变
- [ ] **CP-2.10-4** **docstring 与代码同步**：`:1796-1797` 的"先到先得"表述已改；代码行为与 docstring **逐条一致**（元检查：把 docstring 里列的每条规则各构造一个用例）
- [ ] **CP-2.10-5（DA-S8-1）★★BUG-S8-01** `group` 为 `int` / `list` / `dict` **三形态**：该条目**不进主实验桶、不进任何分组桶、且打 WARNING**
- [ ] **CP-2.10-6（DA-S8-1）** `group` 为 `None` / 缺键 / 全空白字符串 **三形态**：**仍归主实验桶**（既有行为一字不变）
- [ ] **CP-2.10-7** ★**负向：不做强转**：`group={"a":1}` 时**不产生** `"{'a': 1}"` 这类 repr 形态的组名（负向断言）
- [ ] **CP-2.10-8** docstring 畸形清单**已补上 group 类型这一条**（元检查：`inspect.getdoc` 子串断言）
- [ ] **CP-2.10-9** 既有行为不退化：标量收编 / `mask_value` 脱敏 / 120 上限 / `sorted` 输出 / 连跑逐字节相同 **五项逐条**
- [ ] **CP-2.10-10 逐条验红**：①撞名改回"先到先得" → CP-2.10-2 必红；②BUG-S8-01 改回 `else ""` → CP-2.10-5 必红；③把非 str 改成 `str()` 强转 → CP-2.10-7 必红；④去掉 group 畸形的 WARNING → CP-2.10-5 必红（**已知 bug 模式 #3：禁静默吞错**）
...
- [ ] **CP-2.11-2** ★**可用性恢复自证**：与 CP-1a.6-5 **同一份构造输入**（exit 全 0 + 步骤跑完 + agent 自报 1 个主实验指标）→ 现在 `metrics` **非空**、档位判出、`success` 按 `level` 派生。**CP-1a.6-5 在此换发（期望值翻转），并在 §15.1 留档"翻转时点"**
...
2. **冻结表逐行换发**（§3.4 A 表，AC-S8-21①）：
   - `_apply_no_metrics` → **整行移出**（函数不存在了），**并在测试文件注释写明原因 + 授权留档**；
   - `_build_execution_result` → **换发哈希 + 写明原因**；
   - `_parse_metrics` → 批次 1a 已换发，**本批复核仍对得上**；
   - 其余 7 行 → **哈希不变，逐行核对**。
   - 🔴 **红线：禁止整表删除、禁止改成"不少于"来规避**（AC-S8-21 原文）。
3. **execution prompt 两处哈希基线**已在 T-S8-2-9 换发，本批复核**两处一致**。
4. **`ExecutionResult` 键集合断言**：11 → 12，逐处同步。
5. **禁弱化自查**（§0.3 第 3 条）。
6. 🔴 **本批要求全量回归零失败**（可用性已恢复，不再允许带红收口）。

**自测检查点**：
- [ ] **CP-2.12-1** ★**全量非 e2e 回归零失败**（含时间点）；与批次 1a 的红名单逐条对照，**§3.3 五族全部复绿**
- [ ] **CP-2.12-2** ★**冻结表逐行对账**（AC-S8-21①）：10 行 → 9 行（移出 1）+ 2 行换发 + 7 行不变；**逐行写明原因**；表结构未被整表删除、断言未被改成"不少于"
...
### 任务 T-S8-3-2：reporting 四函数删除 + `_verify_expected_results` 退化 + 三态字面量换发
...
### 任务 T-S8-3-6：回验 / 对比表的文案与渲染（S8-07 / S8-08）
...
3. **`_FORM_CARD_SPEC`（`:85-118`）四张卡换发**：
   - 两张 `full_success_*` 卡 → **按四档制重排**（「复现成功」/「部分复现」各一张；档位现在从 `conclusion.level` 直接读，不再靠 `_determine_conclusion` 现算）；
   - `code_only` 卡 → 复核是否已是纯结构描述；
   - `degraded` 卡「⚠️ **未成功复现（降级）**」→ **降级为纯结构描述 + 档位由四档说**（Maria 拍板 2）。
4. **`_conclusion_card_key`（`:121-135`）**：T-S8-3-3 已做"不静默降级"这一层，本任务完成**四档 → 卡片 key 的完整映射**。
5. 🔴 **报告展示本篇成功标准：结果页本次不扩**（架构 §5.7 末行明裁）。**不要顺手加**。

**自测检查点**：
- [ ] **CP-3.9-1（AC-S8-20②）★** **结果页不再出现假空**：有 `metrics_groups` 时**不显示"无可对比指标"**
- [ ] **CP-3.9-2** 分组展开口径与 `reporting._render_metrics_comparison` **一致**（同一份 `execution_result` 喂两边，组名与指标名集合相同）
- [ ] **CP-3.9-3** 主实验指标与分组指标**并列展示**（不是二选一）
- [ ] **CP-3.9-4** 结论档位读 `conclusion.level`；旧快照（`conclusion` 缺失）→ 走兼容分支**不崩**
- [ ] **CP-3.9-5** ★**四档 → 卡片 key 完整映射**：四个档位**各自命中不同的卡**，无静默降级
- [ ] **CP-3.9-6** ★`degraded` 卡文案已降级为纯结构描述（负向：不含结论意味词）
- [ ] **CP-3.9-7** 🔴 **负向：结果页未新增成功标准展示**（架构 §5.7 明裁本次不扩）
- [ ] **CP-3.9-8 验红**：①数据源改回 `metrics` → CP-3.9-1 红；②四档映射漏一档 → CP-3.9-5 红
...
**自测检查点**：
- [ ] **CP-4.2-1** ★A 表逐行对平：**移出 1 行 + 换发 2 行 + 不变 7 行**（或按 §16.1 的实际裁决调整），**逐行原因可查**
...
| **R-S8-21** 🔴 | **coding 侧字节门"改完再建"**：AC-S8-21② 写"换发"，而**编码侧根本没有基线**（P-S8-5）。若先改 `coding.py` 再建门，建出来的基线照着改后字节写死、**永远绿**，与自锁定形态等价 —— **这道门等于没建，而账目上却记着"已建"** | **T-S8-1a-3 单列为独立任务且强制早于 T-S8-1a-4**（先建后改，沿 sprint7 T-S7-6-2 范式）+ CP-1a.4-7「改前必红」的活体证明 + CP-1a.7-3b 三件套闭合自证（**缺任一件不得声称"已建"**）+ §1.4 裁剪线明列"不可裁不可顺延" |
| **R-S8-22** | **三档函数解绑后成为死代码**（§16.1 未裁前）：`_parse_metrics` 及其三档无生产调用者，留着是技术债、删了则冻结表必红行数由 3 推到 6 | 默认取值 = **保留 + 只解绑**（严格照架构 §12 字面）；§3.4 A 表已按两种取值各给一版账目；§16.1 登记待裁 |
...
6. 🔴 **先建后改**：coding 字节门（T-S8-1a-3）必须早于 coding prompt 改动（T-S8-1a-4）；改前那一次红是"门有牙"的活体证明。execution 侧同理（CP-2.9-1）。
...
## 13. 检查点总览（CP 索引，共 **296** 个）

| 批次 | 任务 | CP 段 | 数量 | 命门 |
|---|---|---|---|---|
| **1a** | T-S8-1a-1 | CP-1a.1-1 ~ 5 | 5 | 基线重测标时间点 |
| | T-S8-1a-2 | CP-1a.2-1 ~ 6 | 6 | 既有键顺序零扰动 |
| | T-S8-1a-3 | CP-1a.3-1 ~ 8 | 8 | ★门验红 + 证否自锁定 |
| | T-S8-1a-4 | CP-1a.4-1 ~ 9 | 9 | ★★改前必红（先建后改活体证明）/ 字节零扰动 |
| | T-S8-1a-5 | CP-1a.5-1 ~ 6 | 6 | ★两个闸同时为真 |
| | T-S8-1a-6 | CP-1a.6-1 ~ 6 | 6 | ★字节零扰动 |
| | T-S8-1a-7 | CP-1a.7-1 ~ 6（含 3b） | 7 | ★**全绿收口** + 可用性自证 / 三件套闭合自证 |
| | **批次 1a 小计** | | **47** | |
...
| | T-S8-4-3 | CP-4.3-1 ~ 6 | 6 | ★五条留档 + 对外表述纪律 |
| | **批次 4 小计** | | **15** | |
...
| 任务 | `T-S8-1a-1` ~ `T-S8-5-2` | 共 **39** 个（1a=7 / 1b=5 / 2=**12** / 3=10 / 4=3 / 5=2）。**批次名从 1a 起，不接 sprint7 的批次 9** |
| 检查点 | `CP-2.1-0` ~ `CP-5.2-4` | 共 **296** 个（逐批：1a=47 / 1b=41 / 2=107 / 3=74 / 4=15 / 5=12） |
...
| v1.0 旧号 | v1.1 新号 | 说明 |
|---|---|---|
| `T-S8-1a-7`（三档解绑） | **`T-S8-2-1`** | 拍板 2 迁到批次 2 首位 |
| `T-S8-1a-8`（1a 收口） | **`T-S8-1a-7`** | 顺位前移一格 |
| `T-S8-2-1` ~ `T-S8-2-11` | **`T-S8-2-2` ~ `T-S8-2-12`** | 整体后移一格，给首位腾位 |
| `CP-1a.7-*`（解绑的 CP） | **`CP-2.1-*`**（另新增 `CP-2.1-0`） | 随任务迁移 |
| `CP-1a.8-*` | **`CP-1a.7-*`** | 随任务前移 |
| `CP-2.1-*` ~ `CP-2.11-*` | **`CP-2.2-*`** ~ **`CP-2.12-*`** | 整体后移一格 |

**未变**：批次 1b / 3 / 4 / 5 的全部任务号与 CP 号；`R-S8-*` / `P-S8-*` / `DA-S8-1` / `AC-S8-*` / `Q-S8-*` 全部号段。
| **裁定留痕 / 待确认** | **已裁定 A / B + 待确认 16.1 / 16.2** | 本文 §16（v1.0 的“待确认项 1 / 3”已由 Maria 2026-08-04 拍板，转入 §16.A / §16.B） |
...
| **P-S8-9 🔴 实质** | PRD §4.2「⚠ 回归面价签」写「`tests/test_sprint7_s713_reported_metrics.py:994-1005` 的**函数源码字节冻结表至少 6 行必红**」 | ⚠ **口径差**：AC-S8-03 的**判定方式**明写「执行侧三档解析**不再被判定链路调用**（**调用链断言**）」，架构 §12 也写「**判定链路解绑**」——**两者均非"删除函数"**。按"只解绑不删除"，冻结表实际只有 **3 行动**：`_apply_no_metrics` **移出**（函数删除）+ `_build_execution_result` **换发**（新形参 + `success` 改派生）+ `_parse_metrics` **换发**（清死参数）。「至少 6 行」是按**三档函数一并删除**估的 | 中：**照"至少 6 行"当靶会以为漏改了三行**，反过来可能促使开发去删本不该删的函数 | §3.4 A 表**逐行给账**（默认取值下 3 行动）；账目以 AC-S8-21① 的"**逐一换发并写明原因**"为准，**不拿"至少 6 行"当靶**；三档函数删不删列入 **§16.1**（默认**不删**），**两种取值下的账目 §3.4 A 表已各给一版**。⚠ **批次归属已随拍板 2 订正**：解绑与 `_parse_metrics` 清死参数落在 **T-S8-2-1**（批次 2 首位，原 `T-S8-1a-7`），逐行换发对账落在 **T-S8-2-12**——**三行动全部发生在批次 2 内部，批次 1a 不碰冻结表** |
...
|---|---|---|---|---|---|---|
| **coding** | 🔴 **本 Sprint 新建**（P-S8-5） | 待 **CP-1a.1-5** 现算（sprint7 §60.2 事实 15 记为 `37ec6ee2b1606715` / 3052 字符，**须复核**） | 待 **CP-1a.4-8** 补 | S8-02：三处 `<METRICS>` 教学文本清除 + 补产出约定 | 建门 T-S8-1a-3 / 换发 T-S8-1a-4 | ⬜ 待开工 |
...
|---|---|---|---|---|
| **CP-2.1-5** | 同一构造输入下 `metrics == {}`（门控不采信）+ `success is False` | 同一构造输入下 `metrics` **非空** + 档位判出 + `success` 按 `level` 派生 | **T-S8-2-11** | 沿 sprint7 §49.0「删原内容 → 换发新 CP」先例。**翻转时点须在此表落档**，否则后人看到两条互斥的 CP 会以为其中一条写错了。对应新 CP = **CP-2.11-2** |

---

## 16. 开放项与裁定留痕

> **纪律**：本节的每一项都是**本计划落盘时在磁盘上发现、但两份定稿文档未覆盖或存在张力**的点。按开发代理定义的触发条件，**已给默认取值但不自行拍板改设计**；若 Maria 或架构师要改，**改的都是单点**，不影响批次结构。

### 16.0 状态总览

| 项 | 议题 | 状态 | 裁定 |
|---|---|---|---|
| **已裁定 A** | AC-S8-14「可编辑」的可执行口径 | ✅ **已裁定（Maria 2026-08-04 拍板 1）** | **按只读展示验收**，「可改」走既有多轮对话修订通道，**不新增交互控件种类** |
| **已裁定 B** | 解绑任务要不要挪到批次 2 首位以压缩不可用窗口 | ✅ **已裁定（Maria 2026-08-04 拍板 2）** | **挪**。现 `T-S8-2-1`，窗口整体压进批次 2 内部；批次 1a 落盘后系统仍可真跑可演示 |
| **待确认 1** | 三档函数解绑后**删不删** | ⬜ **待架构师** | 默认：**解绑但保留**（§16.1） |
| **待确认 2** | execution prompt 配套约束落哪一批 | ⬜ **待架构师** | 默认：**全 Sprint 只在批次 2 改写一次**（§16.2） |
...
- **验收落点**：CP-1b.4-1（独立可见）+ CP-1b.4-2（原文照登）+ **CP-1b.4-2b**（改后取值 + **负向：交互控件计数与改前逐一相同**）。

### 16.B ✅ 已裁定：解绑任务挪到批次 2 首位（**Maria 2026-08-04 拍板 2**）

- **落点**：完整裁定留痕表在 **§0.0**（含原方案 / 拍板意图 / 代价 / 连带同步六项 / 裁定人日期）。
- **一句话**：原 `T-S8-1a-7`（三档解绑）**挪到批次 2 首位，现编号 `T-S8-2-1`** ⇒ 「一律判失败」的不可用窗口**整体落进批次 2 内部**，**批次 1a 落盘后系统仍可真跑、可演示**。
- **连带同步已完成**：批次 1a **7 个任务** / 批次 2 **12 个任务**；批次 1a 收口门**恢复要求全绿**；§3.3 预期变红面整体后移；依赖图、任务清单总表、CP 索引、编号速查、§15 P-S8-9 批次归属**逐处已改**；编号迁移留痕见 §14。
- **配对约束（写进批次 2 头部与 T-S8-2-1 任务体）**：`T-S8-2-1`（撤旧判据分子）与 `T-S8-2-8`（装新判据）**是天然配对、中间不得留缝**；批次 2 **本就受 R-S8-06「内部不得拆分」约束**。

---

### 16.1 / 16.2 —— 以下两项仍待确认（**均不阻塞批次 1a 开工**）

### 16.1 待确认（→ 架构师）：三档函数解绑后**删不删**

- **当前模块**：`core/nodes/execution.py`，T-S8-2-1。
- **遇到的问题**：三份表述指向两种不同做法。
  - PRD §4.2 第 4 条写「三档**退场**」；PRD §4.2「回归面价签」按**删除**估出"冻结表至少 6 行必红"；
  - 但 **AC-S8-03 的判定方式**写「执行侧三档解析**不再被判定链路调用**（**调用链断言**）」，**架构 §12** 也写「**判定链路解绑**」——两者均非"删除"；
  - 且 PRD §4.2 第 5 条要"清 `_parse_metrics` 的死参数 `plan`"这个动作本身，就**预设了它还在**。
- **两种取值的代价**：
  | 取值 | 冻结表账目 | 代价 |
  |---|---|---|
  | **A（默认）解绑但保留** | 3 行动（移出 1 + 换发 2） | 留下**死代码**（`_parse_metrics` 及其三档无生产调用者） |
  | B 一并删除 | 6 行动（移出 4 + 换发 2） | 账目面翻倍；且 PRD §4.2 第 5 条那个动作变得无意义 |
- **默认取值**：**A（解绑但保留，严格照架构 §12 的"判定链路解绑"字面）**。§3.4 A 表已按两种取值各给一版账目。
- **请确认（2 项）**：①取 A 还是 B？②若取 A，死代码是**登记不治**（本 Sprint）还是**登记进 TODO 交后续批次**？

### 16.2 待确认（→ 架构师）：execution prompt 的「论文没报这个数也是合法结论」这条配套约束落在哪一批

- **当前模块**：`core/nodes/execution.py` 的 `_EXECUTION_SYSTEM_PROMPT_BODY`。
- **遇到的问题**：
  - **架构 §6.1 配套约束**要求：`baseline_results` 注入（S8-10，**批次 1a**）须配一句「论文没报这个数也是合法结论，不得硬凑一个"对上了"」；
  - 但**架构 §6.2④**又明令：系统提示词哈希基线「**只换发一次**」，且那一次的原因是 S8-04/05 的判定纪律段改写（**批次 2**），两处属「**同一次改写**」。
  - ⇒ 若批次 1a 就改这句，哈希会换发**两次**，违反 §6.2④。
- **默认取值**：**`_EXECUTION_SYSTEM_PROMPT_BODY` 全 Sprint 只在批次 2（T-S8-2-9）改写一次**，批次 1a 对它一字不动，只做 HumanMessage 通道（payload 键）。
  - **风险为零的理由**：批次 1a 落盘后到批次 2 之间**本来就不得真跑**（§0.0 AR-S8-01）⇒ 这期间 agent 拿到 `baseline_results` 却没被告知"论文没报也合法"，**不会产生任何实际后果**。
  - 本计划判断这属于**忠实落地架构 §6.2④ 的推论，而非新决策**（§6.2④ 说"只换发一次"已隐含了这个结论），故**登记但不阻塞**。
- **请确认（1 项）**：这个推论成立吗？若架构师认为 1a 就该改 prompt，则须同意 execution 哈希基线**换发两次**，并在 §15.1 记两行。

---

*（Sprint 8 开发计划 **v1.1**（v1.0 → v1.1 = Maria 2026-08-04 两条拍板的连带同步）完。**6 个批次 / 39 个任务 / 296 个检查点 / 11 条落点勘误 / 7 条新登记风险（R-S8-18~24）/ 1 条新登记缺陷（BUG-S8-01，验收点 DA-S8-1）/ **2 项已裁定（§16.A / §16.B）+ 2 项待确认（§16.1 / §16.2）**。** 本计划只写规格与检查点，**未写任何生产代码、未跑任何测试、未改 `docs/TODO.md`**。）*
