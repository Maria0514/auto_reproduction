...
> 　⇒ 🔴 **现行汇总账（2026-08-08；数字取自 §13 实算表，逐行加总非沿用）：检查点 `331` 个**（逐批 **25 / 45 / `150` / 83 / 16 / 12**）、**任务 40 个**（批次 2 = **15**）、**落点勘误 ~~`38`~~ ~~`41`~~ ~~`44`~~ ~~`47`~~ ~~`49`~~ ~~`50`~~ ~~`54`~~ ~~`59`~~ ~~`62`~~ `63` 条**（`P-S8-1` ~ **`P-S8-63`**，其中**实质性 ~~`16`~~ ~~`17`~~ ~~`18`~~ ~~`20`~~ ~~`22`~~ ~~`26`~~ ~~`31`~~ ~~`33`~~ `34` 条**；
🔴 **2026-08-10 ~ 08-11 批次 2 执行期新增 ~~6~~ ~~9~~ ~~11~~ ~~12~~ ~~16~~ ~~21~~ ~~24~~ `25` 条 `P-S8-39 ~ ~~P-S8-44~~ `P-S8-63``，见 §15.0e** —— `39~41` 由 `T-S8-2-2` 实现前核实挖出、`42~44` 由 `T-S8-2-1` 真跑全量回归暴露、**`45~47` 由 `T-S8-2-1b` 实做时挖出**（`45` 收网抓出第四处 `<METRICS>`；`46` 是两条各自正确的约束互相拉扯；`47` 超范围登记不治））、**冻结表 10 行 → `5` 行**。~~（作废旧账：勘误 `35` 条）~~⚠ **上一行（`:6`，2026-08-07 那轮）记的 `329` / `148` / `20 条` / 「10→4」全部是当时的账、已被本轮作废** —— 保留作留痕，**不要照它对账**。
...
- [x] **CP-2.5-10（B18）★★命门·须验红** **把对照基准编低 → 判不成功**（AR-S8-14）：同一份产物 + 达标线「数值与论文报告对上」+ `baseline_results = {"knn_accuracy": 0.95}`，agent 报 `{metric: "knn_accuracy", value: "0.61"}` 并自称「印证上了」 ⇒ 该条 `ok=false` ⇒ 逐条结论落「**无法核实**」⇒ **拿不到「复现成功」**；报告中性标注。**验红：让论文值物证无条件通过 → 本用例必红** ⇒ ✅ **[2026-08-11] ★★命门**：同一份产物 + `baseline_results = {"knn_accuracy": 0.95}`、agent 报 `{metric: "knn_accuracy", value: "0.61"}` 并自称「印证上了」⇒ 该条 **`ok=false`**、落台账后 reason 为中性文案；**验红**：让论文值物证无条件通过（第⑦重恒真）→ **本用例必红**（已实测）。🔴 **如实登记：本条只验到了链条的前半段**——「⇒ 逐条结论落「无法核实」⇒ 拿不到「复现成功」」那两跳的落地物（`_decide_conclusion` / 逐条结论收编）分别在 `T-S8-2-6` 与 `T-S8-2-11`，**本任务里还不存在**。~~⇒ **AR-S8-14 的端到端闭合须在 `T-S8-2-12` 收口门补一条贯通用例**~~ 🔴 **2026-08-11 同日订正（`P-S8-63`）：那个贯通用例在现行设计下写不出来** —— `T-S8-2-6` 交付后拿三个构造实跑全链，实测**链条第三跳没有落地物**（封顶③ 判据是「一条都不成立」、判档函数按 `AR-S8-04` 又不读逐条结论）⇒ 编低基准的 agent 只要还留一条过验的真产物，**照样拿得到「复现成功」**。⇒ **本条现状 = 只标注、未堵死**，全裁与三条候选路见 §15.0e `P-S8-63`（**待 Maria 拍板**）。不得因本条已打勾就当成 `AR-S8-14` 已堵死
...
- **产出文件**：`core/nodes/execution.py`（新纯函数，~~**放 `_split_reported_metrics`（`:1781`）附近**~~ ⇒ **放 `_build_evidence_ledger` 之后（步骤 4.75 一节内）**）⟦🔴 **2026-08-11 主控订正（`P-S8-62`，非实质·落点指错）**：`_split_reported_metrics` 由 `T-S8-2-10` **整体删除**，把本函数放它旁边等于放进一片正要拆的区域；且架构 §12 把那个腾空的位置**明确给了 `_collect_result_blocks`**、对本函数**没写落点** ⇒ 原文疑似与前者串了。现落点与架构 §1.5 的步骤骨架一致（验钞 → 建账 → 判档同处 4.75，数据流相邻）⟧
- **依赖项**：T-S8-2-5
- **预计复杂度**：**高**
...
8. ~~**产出结构** = `state.py` docstring 里写的那份（`{level, goal_checks, evidence}`）。~~ 🔴 **2026-08-11 主控换发（`P-S8-61`，本条双重失真）**：①`core/state.py` **现在根本没有 `conclusion` 这个键**（由 `T-S8-2-8` 才加，本任务是它的前置）⇒ 原文指向一份**尚不存在**的 docstring；②那个三键形态是 **v1.0 的内联证据形态**，架构 §2 已明写它"已被 PRD v4.0 §4.6.4 修订为**唯一证据台账 + 引用**"（v2.2）。⇒ **现行产出结构** = `{level, goal_checks, evidence_ledger, level_evidence_ids}`，其中 `goal_checks[]` **只持 `evidence_ids`、不内联任何 `path` / `metric` / `value` / `ok` / `reason`**（架构 §2 四条形态纪律第 1 条）；`result_blocks` / `report_caveats` 两键由 `T-S8-2-10b` 产出、在 `T-S8-2-11` 接线处并进同一个 dict。**照原文实现会把证据内联回逐条结论，正是"漂移成两套"要防的那件事。**

**自测检查点**（AC-S8-07 / AC-S8-08 / AC-S8-09 三条命门都在这）：
- [x] **CP-2.6-1（AC-S8-09①）** `exit_ok` 为假 + **agent 判「复现成功」** → 最终「**失败**」 ⇒ ✅ **[2026-08-11]** `exit_ok=False` + agent 判「复现成功」+ 物证全过验 → 最终「**失败**」。封顶①与汇报无关，客观事实直接压低
- [x] **CP-2.6-2（AC-S8-09②）** 步骤没跑完 + agent 判「复现成功」 → 最终「**仅代码跑通**」 ⇒ ✅ **[2026-08-11]** 步骤对账 `completed=1 / planned_actionable=2` + agent 判「复现成功」→ 最终「**仅代码跑通**」。判据走既有单点谓词 `_completion_insufficient`，**未另写一遍比较**（S7-11 单点谓词红线）
- [x] **CP-2.6-3（AC-S8-09③）** 支撑物证**一条都不成立** + agent 判「复现成功」 → 最终「**仅代码跑通**」 ⇒ ✅ **[2026-08-11]** 档位的支撑物证只有一条且不过验（路径不存在）+ agent 判「复现成功」→ 最终「**仅代码跑通**」；台账如实记 `ok=false`、`level_evidence_ids` 仍指向它（不隐藏证据）
- [x] **CP-2.6-4（AC-S8-09④）★** **agent 报低档但客观事实良好** → 档位**不得被抬高**（agent 报「仅代码跑通」，exit 全 0 + 步骤跑完 + 物证全过 → 仍是「仅代码跑通」） ⇒ ✅ **[2026-08-11] ★** agent 报「仅代码跑通」+ exit 全 0 + 步骤跑完 + 物证全过 ⇒ **仍是「仅代码跑通」**；另测 agent 报最低档「失败」+ 同样良好的客观事实 ⇒ **仍是「失败」**。🔴 **这不是另加的一条规则，是「按下标取 max」的数学结论** —— 取 max 只会往低走
- [x] **CP-2.6-5** 汇报缺失（`report={}`）+ `exit_ok=True` + 步骤跑完 → 「仅代码跑通」；+ `exit_ok=False` → 「失败」（架构 §1.4 表逐行） ⇒ ✅ **[2026-08-11]** 架构 §1.4 表**逐行**实测：①汇报缺失 + exit 全 0 + 步骤跑完 →「仅代码跑通」；②汇报缺失 + 命令跑挂 →「失败」；③汇报缺失 + 步骤没跑完 →「仅代码跑通」；④汇报缺失时逐条结论与台账**均为空**（不臆造）。🔴 **实现取向要点**：档位拿不到时**起点取最高档、全交给封顶决定** —— 此时封顶③必然命中（一条支撑物证都没有）⇒ 落「仅代码跑通」。⇒ **「跑通了却因为没读到汇报被判失败」这条失真路径在算术上不可能发生**，不是靠一条 if 挡住的
- [x] **CP-2.6-6** agent 报了一个**不在四档内**的档名 → 视同缺失走封顶，**不做模糊匹配**（负向：`_decide_conclusion` 中无归一化/子串匹配逻辑） ⇒ ✅ **[2026-08-11]** **8 种畸形档名**逐个实测视同缺失走封顶：`"复现成功 "`（仅多一个尾空格）/ `"复现 成功"` / `"SUCCESS"` / `"部分复现！"` / `""` / `None` / `3` / `["复现成功"]` —— **没有一个被匹配到某一档**，且每次都打 WARNING 留痕（禁静默吞错）。**负向静态审查**：函数体内 `ast` 层 **零** `lower/upper/strip/sub/startswith/endswith/replace/casefold` 调用 ⇒ 没有任何归一化或子串匹配的余地
- [x] **CP-2.6-7（AC-S8-10）★** **部分复现档可达**：三条预期**印证两条** → 判「部分复现」，且**与"一条都没印证"结果不同**（治病②） ⇒ ✅ **[2026-08-11] ★治病②**：三条预期**印证两条** → 判「**部分复现**」，逐条结论实测 = `['印证上了', '印证上了', '没印证上']`；**对照组**——同样报「部分复现」但一条都没印证上（物证全不过验）→「**仅代码跑通**」，三条逐条结论全部落「无法核实」。⇒ **两者结果确实不同**，「部分复现」这一档判得出来
- [x] **CP-2.6-8（AC-S8-11）★** **无对照组的定量论文可达最高档**：单组实验、无对照结构、达标线为"与论文报告对上" → **可判「复现成功」**（治病③） ⇒ ✅ **[2026-08-11] ★治病③**：单组实验、**无对照结构**、达标线为「与论文报告对上」、物证为单个产出文件里的一个数 → 判「**复现成功**」。⇒ 没有对照组**不构成任何结构性障碍**
- [x] **CP-2.6-9（AC-S8-12）★★主断言** **纯定性论文可达最高档**：跑通 + **零数值** + 计划声明的图产物已落地且物证过验 + 达标线为定性表述 → 判「**复现成功**」 ⇒ ✅ **[2026-08-11] ★★主断言**：跑通 + **零数值**（台账里 `value` 全为 `None`，已断言）+ 计划声明的图产物已落地且过验（**真二进制 png，解不出 utf-8**）+ 达标线为定性表述 → 判「**复现成功**」，逐条结论「印证上了」。⇒ **纯定性论文能拿到最高档**，病③的正面证否
- [x] **CP-2.6-10（AC-S8-08②）★★命门·负向静态审查** `_decide_conclusion` 函数体内：①`success_criteria` **零出现**；②**无证据类型枚举**；③**无按证据形态的 if/elif 分支**（`ast` 扫分支条件，人工复核每一个 `if` 的判据都只涉及 `level` / `exit_ok` / `step_reconciliation` / 物证 `ok` 布尔） ⇒ ✅ **[2026-08-11] ★★命门·四项负向静态审查**：①函数体（3898 字符，含 docstring 与全部注释）内该达标线字段名**零出现**；②函数体内**零类定义** ⇒ 无证据类型枚举；③🔴 **逐个分支扫过并逐个打印出来供人工复核**（共 10 个判据：`agent_level in _LEVELS` / `not exit_ok` / `_completion_insufficient(recon)` / `not any(ok_by_id...)` / 三个 `isinstance` 结构守卫 / `data` / `eid is not None...` / `reported in _VERDICTS`）—— **没有一个**触及证据的内容、形态或出处（禁词表 `value/path/metric/reason/trend/numeric/qualitative/…` 逐个比对名字与字面量）；④🔴 **结果块连形参都不收** ⇒「块不参与判定」**是结构性的**，不靠行为用例去证（B17 的行为面复核也一并做了：块数 1 vs 7 ⇒ 档位逐字相同）
- [x] **CP-2.6-11** ★**封顶实现形态**：确认是**按 `_LEVELS` 下标取 max**，**不是 if 链**（`ast` 静态审查 + 人工复核） ⇒ ✅ **[2026-08-11] ★** `ast` 三向核实：`max(...)` 调用在、`_LEVELS[...]` 下标取值 1 处、**零个 `if` 拿四档常量做判据**（if 链形态命中 0）。⇒ 封顶确为"按元组下标取更低档"，不是 if 链
- [x] **CP-2.6-12** 三态字面量为**新三态**（印证上了 / 没印证上 / 无法核实）；**旧三态词零出现**；物证不过验 → 落「无法核实」 ⇒ ✅ **[2026-08-11]** 三态字面量 = `('印证上了', '没印证上', '无法核实')`，均已提为模块级具名常量（进守门候选，账目交 T-S8-3-10）；函数体内**旧三态词零出现**。**五条混合用例逐条实测**：物证过验且 agent 说印证上了 → 保留；**物证不过验** → 落「无法核实」；**干脆没附物证** → 落「无法核实」；agent 自己说没印证上 → 保留；**agent 编了个三态外的词**（"大概对上了"）→ 落「无法核实」。⇒ 保守出口四条入口全部收敛到同一态
- [x] **CP-2.6-13** ★★**逐条验红**：①去掉封顶 1 → CP-2.6-1 必红；②去掉封顶 2 → CP-2.6-2 必红；③去掉封顶 3 → CP-2.6-3 必红；④~~把 `max` 下标改成"取 agent 报的" → CP-2.6-4 必红~~ ⇒ **改为「加一条『客观事实良好就抬档』的分支」→ CP-2.6-4 必红**⟦🔴 **2026-08-11 主控换发（`P-S8-60`）**：原变异**在结构上不可能**打红 `CP-2.6-4` —— 该 CP 的构造里**三条封顶一条都不触发**，`indices` 只有一个元素，**取 max 与取第一个恒等**。已照原文实测：打红 8 条，**独独没有 `CP-2.6-4`**。`AC-S8-09④` 真正要防的是**抬高**，故换发为注入一条抬档分支，实测必红⟧；⑤**引入一个按形态的 if 分支** → CP-2.6-10③ 必红（**这条是守病③根因的活体证明**） ⇒ ✅ **[2026-08-11] ★★六次变异逐条必红、首轮全部有牙**（`cp` 备份 + sha256 逐次还原核验，**零 `git checkout` / `restore` / `stash`**；基线与终值均 `2b2dbbab7166a791`）：①封顶 1 架空 ⇒ `CP-2.6-1` 红；②封顶 2 架空 ⇒ `CP-2.6-2` 红；③封顶 3 架空 ⇒ `CP-2.6-3` 红；④′ 注入「客观事实良好就抬档」分支 ⇒ **`CP-2.6-4` 红**；⑤注入「按这条物证有没有数值岔开判定」⇒ **`CP-2.6-10③` 红**（守病③根因的活体证明）。🔴 **计划原写的④「把 max 下标改成取 agent 报的」打不红 `CP-2.6-4`，已照原文跑一遍留证**（登记 **`P-S8-60`**）：实测它打红 8 条、**独独没有 `CP-2.6-4`** —— 因为该 CP 的构造里三条封顶一条都不触发、`indices` 只有一个元素，**取 max 与取第一个恒等**。`AC-S8-09④` 真正要防的是**抬高**，故补做 ④′。本条 CP 原文已同批换发
...
| **落点勘误** | ~~**`P-S8-1` ~ `P-S8-11`**~~ ~~**`P-S8-1` ~ `P-S8-20`**（共 **20** 条）~~ ~~**`P-S8-1` ~ `P-S8-25`**（共 **25** 条）~~ ~~**`P-S8-1` ~ `P-S8-34`**（共 **34** 条）~~ ~~**`P-S8-1` ~ `P-S8-35`**（共 **`35`** 条）~~ ~~⇒ **`P-S8-1` ~ `P-S8-38`**~~ ⇒ ~~**`P-S8-1` ~ `P-S8-54`**~~ ~~⇒ **`P-S8-1` ~ `P-S8-59`**~~ ~~⇒ **`P-S8-1` ~ `P-S8-62`**~~ ⇒ **`P-S8-1` ~ `P-S8-63`**（🔴 **总数见文首「现行汇总账」权威行，本行 2026-08-09 起不复述**；构成：§15 本表 11 + §15.0 四条 + §15.0b 五条 + §15.0c 五条 + §15.0d 十三条 + ~~**§15.0e 六条**~~ ~~**§15.0e `16` 条**~~ ~~**§15.0e `21` 条**~~ ~~**§15.0e `24` 条**~~ **§15.0e `25` 条**⟦🔴 **2026-08-11 主控订正（`P-S8-54`）**：本处的「六条」**三轮推数（6→9→11→12）一次都没跟改**，而同一括号里的编号上界每轮都改了 ⇒ **同一个抄写口里的两个数独立老化**。加总现为 ~~11+4+5+5+13+16 = **54**~~ ~~**11+4+5+5+13+21 = `59`**~~ ~~**11+4+5+5+13+24 = `62`**~~ **11+4+5+5+13+25 = `63`**，与文首 `:10` 权威行一致⟧） | 本文 §15，**与 sprint7 的裸 `P-N`（已用到 P-73）显式分段** |
...
### 15.0e 🔴 批次 2 执行期新增勘误（~~**6 条**~~ ~~**9 条**~~ ~~**11 条**~~ ~~**12 条**~~ ~~**16 条**~~ ~~**21 条**~~ ~~**24 条**~~ **`25` 条**：`P-S8-39 ~ ~~P-S8-44~~ `P-S8-63``，2026-08-10 ~ **08-11**，起点 `HEAD=ac60379`；**`P-S8-39~41` 由 `T-S8-2-2` 实现前核实挖出，`P-S8-42~44` 由 `T-S8-2-1` 落盘后真跑全量回归暴露，`P-S8-45~47` 由 `T-S8-2-1b` 实做时挖出**）

> **纪律**（沿 §15.0b / §15.0d 先例）：实做时凡与文档记载对不上、或按文档做会漏做/做错的，一律落进本表并写清磁盘实测值，**不默默按文档写的做**。
> 🔴 **本表 ~~3~~ ~~6~~ ~~9~~ ~~11~~ ~~12~~ ~~16~~ ~~21~~ ~~24~~ `25` 条里有 ~~2~~ ~~3~~ ~~5~~ ~~10~~ ~~11~~ `12` 条（`P-S8-39` / `P-S8-40` / **`P-S8-43`** / **`P-S8-51`** / **`P-S8-54`** / **`P-S8-55`** / **`P-S8-56`** / **`P-S8-57`** / **`P-S8-58`** / **`P-S8-59`** / **`P-S8-60`** / **`P-S8-63`**）是同一份文档内部两处直接打架** —— 不是"文档与磁盘不符"，是"**文档与自己不符**"。⚠ **这类失真比对不上磁盘更危险**：执行者读到哪一处全凭运气，而两处都在同一份 dev-plan 里、都带着权威口吻。⇒ **`T-S8-2-2` 这类"实现前核实"任务的射程必须包含"同文档内互证"，不能只做"文档 vs 磁盘"。**
...
| **`P-S8-60` 🔴 实质**（同文档内互斥**第十一例**；`BUG-S8-G8` / `P-S8-52` 同族——「**验红项与它所指的检查点对不上**」本 Sprint **第三次发作**；`T-S8-2-6` 逐条验红时撞出） | `CP-2.6-13④` 写「把 `max` 下标改成『取 agent 报的』→ **`CP-2.6-4` 必红**」 | ❌ **该变异在结构上不可能打红 `CP-2.6-4`**（已照原文实测留证）：`CP-2.6-4` 的构造是「agent 报低档 + **客观事实良好**」⇒ **三条封顶一条都不触发** ⇒ 下标列表只有一个元素 ⇒ **取 max 与取第一个恒等**。实测该变异打红 **8 条**（`CP-2.6-1/2/3/5a/5b/5c/7b/11`），**独独没有 `CP-2.6-4`** —— 它架空的是封顶，而 `CP-2.6-4` 守的是**抬高** | **中高**：①按原文做，会得到「变异跑了、红了一片、目标 CP 却始终绿」，而执行者极可能只看「有没有红」就判通过 ⇒ **`AC-S8-09④` 实际零验红**；②`AC-S8-09④`（只压低不抬高）恰是三条封顶之外**唯一**约束档位方向的验收条 | ✅ **`CP-2.6-13④` 已就地换发**为「**加一条『客观事实良好就抬档』的分支** → `CP-2.6-4` 必红」，实测必红（连带 `CP-2.6-4b/5a/7a`）。🔴 **计划原文的变异也照样跑了一遍并留证**（本轮验红脚本里标为「留证项、不计入成败」）—— **不能只写一句「原文不对」就换掉，得把它不对的样子留在证据里** |
| **`P-S8-61` 🔴 实质·双重失真**（`T-S8-2-6` 落产出结构时撞出） | `T-S8-2-6` 第 8 条写「**产出结构** = `state.py` docstring 里写的那份（`{level, goal_checks, evidence}`）」 | ❌ **两处都不成立**：①`core/state.py` **现在根本没有 `conclusion` 这个键**（`grep` 实测零命中）—— 它由 **`T-S8-2-8`** 才加，而 `T-S8-2-8` 依赖本任务 ⇒ **原文指向一份尚不存在、且要等到本任务之后才存在的 docstring**；②那个三键形态是 **v1.0 的内联证据形态**，架构 `§2` 逐字写着它「已被 PRD v4.0 §4.6.4 修订为**唯一证据台账 + 引用**」（v2.2 改写，v1.0 原文只留在折叠留痕里） | **中高**：照第 8 条实现会**把证据内联回 `goal_checks`**，正撞架构 `§2` 四条形态纪律第 1 条（「落盘后的 `goal_checks` 与 `result_blocks` **只允许出现 `evidence_ids`**」）—— 而那条纪律要防的正是 PRD §4.6.4 说的「**漂移成两套证据**」 | ✅ **第 8 条已就地换发**：现行产出结构 = `{level, goal_checks, evidence_ledger, level_evidence_ids}`，`goal_checks[]` **只持 `evidence_ids`**；`result_blocks` / `report_caveats` 由 `T-S8-2-10b` 产出、在 `T-S8-2-11` 接线处并进同一个 dict。代码已按换发后的形态交付并有断言守着 |
| **`P-S8-62`**（**非实质·落点指错**；`T-S8-2-6` 选放置位置时撞出） | `T-S8-2-6` 的「产出文件」行写「新纯函数，**放 `_split_reported_metrics`（`:1781`）附近**」 | ⚠ `_split_reported_metrics` 由 **`T-S8-2-10` 整体删除**（本任务之后第四个任务）⇒ 把新函数放它旁边等于**放进一片正要拆的区域**；且架构 `§12` 把那个腾空的位置**明确给了 `_collect_result_blocks`**、对 `_decide_conclusion` **根本没写落点** ⇒ 原文疑似与前者串了 | **低**（不影响行为，但会让 `T-S8-2-10` 删除时多一次"这段能不能删"的判断） | ✅ **已就地订正为「放 `_build_evidence_ledger` 之后（步骤 4.75 一节内）」** —— 与架构 `§1.5` 的步骤骨架一致：验钞 → 建账 → 判档三者同处 4.75、数据流相邻，且与 `T-S8-2-10` 的删除面**零重叠** |
| **`P-S8-63` 🔴🔴 实质·防作弊命门的因果链断了一跳**（同文档内互斥**第十二例**；`P-S8-46`「两条各自正确的约束互相拉扯」同族；`T-S8-2-6` 交付后**拿三个构造实跑 `AR-S8-14` 全链**才现形。🔴 **须 Maria 裁定，不自行接线**） | 架构 `§16.3.2` 第 2 条给出 `AR-S8-14` 的完整因果链并断言「**零新机制**」：「把论文值编低 ⇒ 论文值物证不成立 ⇒「数值与论文报告对上」这条预期判不成「印证上了」⇒ **拿不到它想要的那一档**」；`CP-2.5-10（B18）★★命门` 逐字照抄这条链，期望写死为「⇒ **拿不到「复现成功」**」 | ❌ **第三个 ⇒ 一段代码都没有**（三个构造已实跑，`baseline_results={"knn_accuracy": 0.95}`、agent 报 `{metric: knn_accuracy, value: 0.61}` 并自称「印证上了」）：<br>**丙**·档位物证**只有**那条编低的论文值 ⇒ 「仅代码跑通」✅（封顶③ 命中）<br>**甲**·档位物证 = 编低的论文值 **+ 一个过验的真产物** ⇒ **「复现成功」** ❌<br>**乙**·把编低的值只藏进逐条预期、档位物证只放过验产物 ⇒ **「复现成功」** ❌<br>两条各自正确的裁定合起来把链切断了：①**封顶③ 的判据是「一条都不成立」**（PRD `§4.5.3` 逐字，实现即 `not any(ok)`）⇒ **只要还剩一条过验就不触发**；②**`AR-S8-04` 写死判档函数只读档位 + 数封顶、不读逐条结论** ⇒ 「一条预期落无法核实」**到**「档位降下来」之间**没有任何机制**。⇒ 逐条结论那一层确实落了「无法核实」（前两跳成立），但它**不影响档位** | 🔴🔴 **高，且方向是安全面**：`AR-S8-14` 是「防 agent 把对照基准往低了编」的**唯一**挡板，而「基准是论文说的」**没人会去质疑**。现状 = **只标注、未堵死** ⇒ 报告会印出「复现成功」+ 一条「无法核实」+ 台账里一条 `ok=false`，**三者并存、自相矛盾，全靠人眼发现**。⚠ 更要紧的是**对外表述**：`R-S8-01` 明令不得夸大能力，而现行文档写的是「拿不到它想要的那一档」/「零新机制」，**这是一句不成立的强口径** | ⏸ **本轮只登记 + 收窄表述，不动实现**（`§16.3.2` 第 2 条明令「**不新增第四条封顶**、**开发不得另写一条论文值对不上则降档的分支**」⇒ 自行接线必与该红线正面冲突）。**三条候选路，交 Maria 拍板**：<br>**甲·收窄表述**（零改动）—— 承认 `AR-S8-14` 是**标注级**防线不是**封顶级**，把架构与 `CP-2.5-10` 的「拿不到那一档」改为「该条落无法核实并在报告中显著标注」，并在 `R-S8-01` 口径表里记一条局限。**代价**：编低基准的 agent 仍能拿到「复现成功」。<br>**乙·改封顶③ 判据** —— `not any` ⇒ `not all`（任一支撑物证不成立即封顶「仅代码跑通」）。**代价**：口径比 PRD `§4.5.3` 写的「一条都不成立」严得多，属**产品语义变更**，且一条笔误路径就会把整轮压档，误伤面须评估。<br>**丙·加第四条封顶**（有预期落「无法核实」⇒ 封顶「部分复现」）—— **架构已明令否决**，且会让判档函数读逐条结论、正面撞 `AR-S8-04`。<br>⇒ **主控倾向甲**：`AR-S8-04`（不按证据形态/语义分支）与 `AR-S8-14`（防编基准）本就是两个量级的红线，为后者去松前者不划算；且甲**立刻消除一句不成立的对外口径**，这一条本身就有价值。🔴 **`CP-2.5-10` 的证据栏已同批订正**：原写「端到端闭合须在 `T-S8-2-12` 收口门补一条贯通用例」——**那个用例在现行设计下写不出来**，已改为指向本条 |
