...
> 　⇒ 🔴 **现行汇总账（2026-08-08）：检查点 `331` 个**（逐批 **25 / 45 / `150` / 83 / 16 / 12**）、**任务 40 个**（批次 2 = **15**）、**落点勘误 ~~`35`~~ `38` 条**（`P-S8-1` ~ `P-S8-38`）、**冻结表 10 行 → `5` 行**。⚠ **上一行（`:6`，2026-08-07 那轮）记的 `329` / `148` / `20 条` / 「10→4」全部是当时的账、已被本轮作废** —— 保留作留痕，**不要照它对账**。
> 🔴 **2026-08-09 批次 2 开工前置（三组失真收口，经 Maria 授权直接落盘；本轮零生产改动）**：①**验钞口径全文统一到七重**（`P-S8-36`，10 处，其中 3 处是 grep 收网挖出的、原先无人点名）；②**`T-S8-2-8` 正文补"第 0 条"+ 三行实体复选框 + `R-S8-42` 验红项**（`P-S8-37`，`:593` 那条挂了两天的"唯一未做"结清）；③**§3.4 C 表与 `T-S8-3-1` 术语守门现值订正为 18 / 43 / 61**（`P-S8-30` 结清，目标下限随基数由 ≥19 改为 **≥22**）。**新登记勘误 3 条**（`P-S8-36 ~ P-S8-38`，§15.0d）⇒ 总数 35 → **`38`**，实质性 14 → **`16`**。🔴 **检查点总数 `331` 与任务数 40 均未变**（三条 CP 本就已计入 `T-S8-2-8` 的 19 条，本轮只是把它们的实体补上）。
...
**检查点总数**：~~328 个~~ ~~329 个~~ **331 个**（分布见 §13 CP 索引。逐批小计：~~1a=32 / 1b=41 / 2=144 / 3=83 / 4=16 / 5=12~~ ~~1a=25 / 1b=45 / 2=148 / 3=83 / 4=16 / 5=12~~ ⇒ **1a=25 / 1b=45 / 2=`150` / 3=83 / 4=16 / 5=12**）
> 🔴 **本轮实算算式（2026-08-08 主控收口，逐行加总，非沿用旧数）**：`25 + 45 + 150 + 83 + 16 + 12 = 331`。~~（2026-08-07：`25 + 45 + 148 + 83 + 16 + 12 = 329`）~~
...
> ⚠ **还有 1 条待定**：`CP-2.8-18`（旧快照 `result.get("conclusion") or {}` → `{}`）按原文是「批次 2 开工时先核对 `CP-2.8-1 ~ 16` 是否已覆盖：**有则登记吸收、不新设；无则新设**」⇒ ~~**若新设，批次 2 = 149、总数 = 330**。本轮按"尚未新设"记 329~~ ⇒ 🔴 **2026-08-09 订正（`P-S8-37` 收网）：该条件数是 2026-08-07 的账、已过期两轮**（批次 2 此后由 148 → 150、总数 329 → 331）⇒ **若新设，批次 2 = `151`、总数 = `332`**（与 §13 末合计行一致）。现按"尚未新设"记 **331**，**待批次 2 开工核对后回填**。

**落点勘误**：~~15 条~~ ~~20 条~~ ~~25 条~~ ~~34 条~~ ~~`35` 条~~ **`38` 条**（§15；v1.0~v1.2 的 **P-S8-1 ~ P-S8-11**（11 条）+ v1.3 新增 **P-S8-12 ~ P-S8-15**（4 条，§15.0）+ 批次 1a 执行期 **P-S8-16 ~ P-S8-20**（5 条，§15.0b）+ 批次 1b 开工前置期 **`P-S8-21 ~ P-S8-25`**（5 条，§15.0c）+ **批次 1b 执行期 + 批次 2 开工前置期 `P-S8-26` ~ ~~`P-S8-35`~~ `P-S8-38`**（~~10 条~~ **`13` 条**，**§15.0d**，2026-08-08 ~ 08-09，含主控收口挖出的 `P-S8-35` 与批次 2 开工前置三组失真收口的 **`P-S8-36 ~ P-S8-38`**）。实质性 ~~`14`~~ **`16` 条**：P-S8-3 / P-S8-4 / P-S8-5 / P-S8-9 / 🔴 **P-S8-12（架构未覆盖）** / 🔴 **P-S8-16（`AR-S8-15` 触发条）** / P-S8-18 / `P-S8-21` / `P-S8-22` / `P-S8-23` / `P-S8-24` / **`P-S8-27`** / **`P-S8-31`** / **`P-S8-35`** / 🔴 **`P-S8-36`（验钞口径，安全面）** / 🔴 **`P-S8-37`（`T-S8-2-8` 缺实现步骤）**）⟦🔴 **2026-08-08 订正**：本行**上一轮（20 → 25）整个漏跟**，四个同源汇总口只改了另外三处 ⇒ 同一份文档里两个数并存了半天。已登记 **`P-S8-32`**（`P-S8-22` 同族复发）⟧⟦🔴 **2026-08-09 补充实测**：`P-S8-32` 说勘误总数有"**4 个**同源汇总口"，**实为 5 个** —— 它漏了**文首 `:9` 那行「现行汇总账」**（该行同样写着"落点勘误 N 条"）。本轮已**五处一并**推到 `38` 条 / 实质性 `16` 条⟧
...
| 冻结表 `_parse_metrics` 一行 | ~~死参数被清 ⇒ 源码字节变~~ ⇒ 🔴 **v1.3 改判：函数被整体删除 ⇒ `inspect.getsource` 直接抛异常**（不是"字节变了"，是**取不到源码**） | **T-S8-2-12**（**整行移出冻结表**，不是逐行换发） |
...
- [ ] **CP-2.1b-10** ★**三件套闭合自证**（**由批次 1a 的 `CP-1a.5-3b` 改判迁入，2026-08-07，§15 P-S8-19**）：coding 侧的旧哈希（`CP-1a.1-5`，**批次 1a 已备**：`37ec6ee2b1606715` / 3052 字符，已落 §15.1）、建门时的红（`CP-2.1b-7`）、换发后的新哈希（`CP-2.1b-8`）**三个证据齐全并落在 §15.1 同一行**——**缺任一件，这道门在交付说明里不得声称"已建"**
...
- [ ] **CP-2.1b-11** **旧快照兼容·coding 注入侧**（AC-S8-02④，**由批次 1b 的 `CP-1b.2-7` 改判迁入，2026-08-08，§15.0d `P-S8-33`**）：缺 `success_criteria` 的旧计划 dict 经 coding 上下文构造点 `.get(...) or ""` 读**不崩**、按空处理
...
**自测检查点**（AC-S8-06 **须验红**）：
- [ ] **CP-2.5-1** ~~五重~~ **产物侧五重**（下列①~⑤）**逐重构造用例**（🔴 **论文值侧⑥⑦不在本条内，由 `CP-2.5-11` / `CP-2.5-12` 覆盖**；本条原写"五重"未限定出处，`P-S8-36` 收网时补限定）：①路径不存在 → 不成立；②不可读（权限/目录）→ 不成立；③数值查不到 → 不成立；④路径越出 `code_output_dir` → 不成立；⑤路径出现在计划外命令参数里 → 不成立
- [ ] **CP-2.5-2** ~~五重~~ **该出处对应的各重全过** → 成立（正向路径存在，不是只会说不）。🔴 **两种出处各构造一条正向**：产物物证（①~⑤全过）与论文值物证（⑥⑦全过）**各自成立**——只做产物侧那一条，论文值侧的正向路径就无人守（`P-S8-36`）
...
## 13. 检查点总览（CP 索引，共 ~~328~~ ~~329~~ **331** 个）
...
| | **合计** | | ~~328~~ ~~329~~ **331** | 🔴 **实算：25 + 45 + `150` + 83 + 16 + 12 = 331**（2026-08-08 逐行加总）。~~（2026-08-07：25+45+148+83+16+12 = 329）~~ ⚠ **`CP-2.8-18` 若在批次 2 开工核对后新设 ⇒ 批次 2 = `151`、合计 = `332`**，届时须同步 §2 摘要行（**含其下算式与"净 +N 从哪来"注**）/ 本节标题 / 批次 2 小计行 / 本行 / §14 编号速查 / **结语行** —— 🔴 **共 6 处，不是原文写的"四处"**（2026-08-08 实数；原文漏列批次 2 小计行与结语行）。**别只改一处**，`P-S8-22` / `P-S8-32` 都是这么发作的。<br>🔴 **2026-08-09 补记（`P-S8-37` 收网）**：**这个"若新设则 = 多少"的条件数本身也有 4 个落点** —— ①§2 概述「还有 1 条待定」注 / ②本节 `T-S8-2-8` 索引行 / ③`T-S8-2-8` 第 0 条 / ④`CP-2.8-20` 下方待核注。**①②两处一直停在 2026-08-07 的「149 / 330」、已过期两轮**（批次 2 此后 148→150、总数 329→331），本轮已四处对平到 **`151` / `332`**。⇒ **连"条件数"都会独立失真** —— 它平时不被对账，正因为如此更没人发现 |
...
| 检查点 | `CP-1a.1-1` ~ `CP-5.2-4` | ~~共 300 个（逐批：1a=32 / 1b=41 / 2=126 / 3=74 / 4=15 / 5=12）~~ ~~共 329 个（2=148）~~ ⇒ 共 **`331`** 个（逐批：**1a=25 / 1b=45 / 2=`150` / 3=83 / 4=16 / 5=12**，实算 `25+45+150+83+16+12=331`）。⚠ **本行原数同为 v1.2 时期的账**，与 §2 摘要行、§13 索引长期打架，2026-08-07 三处一次对平；**2026-08-08 因 `P-S8-33` 跨批次换发（`CP-2.1b-11` / `CP-2.8b-7`）再对平一次，本轮共 6 个汇总口同批跟改** |
| **落点勘误** | ~~**`P-S8-1` ~ `P-S8-11`**~~ ~~**`P-S8-1` ~ `P-S8-20`**（共 **20** 条）~~ ~~**`P-S8-1` ~ `P-S8-25`**（共 **25** 条）~~ ~~**`P-S8-1` ~ `P-S8-34`**（共 **34** 条）~~ ~~**`P-S8-1` ~ `P-S8-35`**（共 **`35`** 条）~~ ⇒ **`P-S8-1` ~ `P-S8-38`**（共 **`38`** 条：§15 本表 11 + §15.0 四条 + §15.0b 五条 + §15.0c 五条 + **§15.0d `13` 条**，2026-08-08 批次 1b 执行期 + 主控收口 + **2026-08-09 批次 2 开工前置三组失真收口**） | 本文 §15，**与 sprint7 的裸 `P-N`（已用到 P-73）显式分段** |
...
> ~~🔴 **口径订正（2026-08-07）：勘误现共 20 条，不是 11 条。**~~ ~~⇒ 🔴 **再订正（2026-08-08）：勘误现共 `25` 条。**~~ ~~⇒ 🔴 **三订正（2026-08-08 批次 1b 执行期）：勘误现共 `34` 条。**~~ ~~⇒ 🔴 **四订正（2026-08-08 主控收口）：勘误现共 `35` 条。**~~ ⇒ 🔴 **五订正（2026-08-09 批次 2 开工前置，三组失真收口）：勘误现共 `38` 条。** 本表 **11 条**（`P-S8-1 ~ P-S8-11`，v1.0~v1.2 落盘期）+ **§15.0 四条**（`P-S8-12 ~ P-S8-15`，v1.3 跟改期）+ **§15.0b 五条**（`P-S8-16 ~ P-S8-20`，批次 1a 执行期）+ **§15.0c 五条**（`P-S8-21 ~ P-S8-25`，批次 1b 开工前置期）+ ~~**`§15.0d` 十条**（`P-S8-26 ~ P-S8-35`）~~ ⇒ **`§15.0d` `13` 条**（**`P-S8-26 ~ P-S8-38`，批次 1b 执行期 + 主控收口 + 批次 2 开工前置期**）= **`38` 条**，实质性 ~~`14`~~ **`16` 条**：P-S8-3 / P-S8-4 / P-S8-5 / P-S8-9 / **P-S8-12** / **P-S8-16** / P-S8-18 / **`P-S8-21`** / **`P-S8-22`** / **`P-S8-23`** / **`P-S8-24`** / **`P-S8-27`** / **`P-S8-31`** / **`P-S8-35`** / **`P-S8-36`** / **`P-S8-37`**。
...
|---|---|---|---|---|---|---|
| **coding** | 🔴 **本 Sprint 新建**（P-S8-5）⇒ `tests/test_sprint8_s8_1a3_coding_prompt_guard.py`：主体门 `EXPECTED_BODY_HASH`（`:81`）+ 稳定前缀门 `EXPECTED_PREFIX_HASH`（`:87`） | ✅ **CP-1a.1-5 已现算（2026-08-06，HEAD=`0e250fb`）**：主体 **`37ec6ee2b1606715` / 3052 字符** —— **与 sprint7 §60.2 事实 15 记载逐字相同，复核通过**；稳定前缀（主体+诚实红线段）**`2973ea0f0ad17502` / 3485 字符**（`_CODING_HONESTY_SECTION` 单体 `55f3740622b23807` / 433 字符） | ⬜ 待 **CP-2.1b-8** 补（**两个值都要补**） | S8-02：三处 `<METRICS>` 教学文本清除 + 补产出约定 | 建门 **T-S8-1a-3 ✅ 已完成**（2026-08-07；7 用例全绿，已验红）/ 换发 T-S8-2-1b | 🟡 **建门已闭合，换发待批次 2**（⚠ 三件套整体**未**闭合，见 ~~CP-1a.5-3b~~ → **`CP-2.1b-10`**（2026-08-07 改判迁入批次 2）/ P-S8-19） |
...
*（Sprint 8 开发计划 ~~**v1.2**~~ **v1.3（+ 2026-08-07 收口订正 + 2026-08-08 批次 1b 交付与主控收口）** 完。~~**6 个批次 / 39 个任务 / 300 个检查点 / 11 条落点勘误 / 7 条新登记风险（R-S8-18~24）**~~ ⇒ **6 个批次 / 40 个任务 / ~~329~~ `331` 个检查点 / ~~20 条落点勘误（P-S8-1~20）~~ ~~`25` 条落点勘误（`P-S8-1~25`）~~ ~~`34` 条落点勘误（`P-S8-1~34`）~~ ~~`35` 条落点勘误（`P-S8-1~35`，含 §15.0d 批次 1b 执行期十条；2026-08-08 勘误总数 4 个 + 检查点总数 6 个同源汇总口各一次同批跟改）~~ ⇒ **`38` 条落点勘误（`P-S8-1~38`，含 §15.0d `13` 条 = 批次 1b 执行期十条 + **2026-08-09 批次 2 开工前置三条** `P-S8-36`（验钞五重→七重全文统一，10 处）/ `P-S8-37`（`T-S8-2-8` 缺"第 0 条"+ 三行复选框 + `R-S8-42` 验红项，三件全补）/ `P-S8-38`（批次 1a/1b 交付后行号整体位移）；⚠ **勘误总数的同源汇总口实测为 `5` 个不是 4 个**，本轮五处同批跟改，见 §15 表头）**/ 7 条新登记风险（R-S8-18~24）+ 1 条架构升格纪律（`R-S8-42`）** / 1 条新登记缺陷（BUG-S8-01，验收点 DA-S8-1）/ **4 项裁定全部结案（§16.A~D，🟢 零开放项）+ 架构文档同批跟改至 ~~v2.1~~ **v2.5**（§16.E + `AR-S8-15` / §17）**。** 本计划只写规格与检查点，**未写任何生产代码、未改 `docs/TODO.md`**（批次 1a / 1b 的实测数字由开发代理回填）。*
