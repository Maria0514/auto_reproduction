...
> 　⇒ 🔴 **现行汇总账（2026-08-08；数字取自 §13 实算表，逐行加总非沿用）：检查点 `331` 个**（逐批 **25 / 45 / `150` / 83 / 16 / 12**）、**任务 40 个**（批次 2 = **15**）、**落点勘误 ~~`38`~~ ~~`41`~~ ~~`44`~~ `47` 条**（`P-S8-1` ~ **`P-S8-47`**，其中**实质性 ~~`16`~~ ~~`17`~~ ~~`18`~~ `20` 条**；
🔴 **2026-08-10 ~ 08-11 批次 2 执行期新增 ~~6~~ `9` 条 `P-S8-39 ~ ~~P-S8-44~~ `P-S8-47``，见 §15.0e** —— `39~41` 由 `T-S8-2-2` 实现前核实挖出、`42~44` 由 `T-S8-2-1` 真跑全量回归暴露、**`45~47` 由 `T-S8-2-1b` 实做时挖出**（`45` 收网抓出第四处 `<METRICS>`；`46` 是两条各自正确的约束互相拉扯；`47` 超范围登记不治））、**冻结表 10 行 → `5` 行**。~~（作废旧账：勘误 `35` 条）~~⚠ **上一行（`:6`，2026-08-07 那轮）记的 `329` / `148` / `20 条` / 「10→4」全部是当时的账、已被本轮作废** —— 保留作留痕，**不要照它对账**。
...
**落点勘误**：🔴 **总数见文首「现行汇总账」权威行（本处 2026-08-09 起不复述数字）**；下列为编号构成。~~（作废旧账：15 / 20 / 25 / 34 / `35` 条）~~（§15；v1.0~v1.2 的 **P-S8-1 ~ P-S8-11**（11 条）+ v1.3 新增 **P-S8-12 ~ P-S8-15**（4 条，§15.0）+ 批次 1a 执行期 **P-S8-16 ~ P-S8-20**（5 条，§15.0b）+ 批次 1b 开工前置期 **`P-S8-21 ~ P-S8-25`**（5 条，§15.0c）+ **批次 1b 执行期 + 批次 2 开工前置期 `P-S8-26` ~ ~~`P-S8-35`~~ `P-S8-38`**（~~10 条~~ **`13` 条**，**§15.0d**，2026-08-08 ~ 08-09，含主控收口挖出的 `P-S8-35` 与批次 2 开工前置三组失真收口的 **`P-S8-36 ~ P-S8-38`**）+ 🔴 **批次 2 执行期 `P-S8-39 ~ ~~P-S8-44~~ `P-S8-47``**（~~6 条~~ **`9` 条**，**§15.0e**，2026-08-10 ~ **08-11**；`39~41` 由 `T-S8-2-2` 实现前核实挖出、`42~44` 由 `T-S8-2-1` 真跑全量回归暴露、**`45~47` 由 `T-S8-2-1b` 实做时挖出**；**其中 3 条是同一份文档内部两处直接打架**）。实质性 ~~`14`~~ ~~`16`~~ ~~`17`~~ ~~`18`~~ **`20` 条**⟦🔴 **2026-08-11 主控订正（第一次）**：本行原写 `17`，而**同一行的编号列表实为 18 个**（下列逐个可数），文首 `:10` 权威行写的也是 `18` ⇒ 上一轮新增两条实质性（`P-S8-40` / `P-S8-42`）时**编号加进来了、数字只推了一格**。🔴 **`P-S8-32` 同族又一次发作，且这次最刺眼——两个对不上的数就在同一行里**，连"跨章节漏跟"这个借口都没有。⟧⟦**同日第二次推数**：`T-S8-2-1b` 交付新增 `P-S8-45` / `P-S8-46` 两条实质性（`P-S8-47` 属超范围登记不治、不计入）⇒ `18` → **`20`**，编号列表同批补入，**本轮已按 §3.10 全文 grep 五个同源汇总口一并推**⟧：P-S8-3 / P-S8-4 / P-S8-5 / P-S8-9 / 🔴 **P-S8-12（架构未覆盖）** / 🔴 **P-S8-16（`AR-S8-15` 触发条）** / P-S8-18 / `P-S8-21` / `P-S8-22` / `P-S8-23` / `P-S8-24` / **`P-S8-27`** / **`P-S8-31`** / **`P-S8-35`** / 🔴 **`P-S8-36`（验钞口径，安全面）** / 🔴 **`P-S8-37`（`T-S8-2-8` 缺实现步骤）** / 🔴 **`P-S8-40`（`_collect_grouped_metrics` 存废，三处仍写「不删」）** / 🔴🔴 **`P-S8-42`（§3.3 变红面表漏三族，收口门自身失效）** / 🔴 **`P-S8-45`（三处 `<METRICS>` 实为四处，两道验收口都判绿）** / 🔴 **`P-S8-46`（「例子要具体」与「主体禁含论文级字面量」互相拉扯，初版当场被判红）**）⟦🔴 **2026-08-08 订正**：本行**上一轮（20 → 25）整个漏跟**，四个同源汇总口只改了另外三处 ⇒ 同一份文档里两个数并存了半天。已登记 **`P-S8-32`**（`P-S8-22` 同族复发）⟧⟦🔴 **2026-08-09 补充实测**：`P-S8-32` 说勘误总数有"**4 个**同源汇总口"，**实为 5 个** —— 它漏了**文首 `:9` 那行「现行汇总账」**（该行同样写着"落点勘误 N 条"）。本轮已**五处一并**推到 `38` 条 / 实质性 `16` 条⟧
...
| **落点勘误** | ~~**`P-S8-1` ~ `P-S8-11`**~~ ~~**`P-S8-1` ~ `P-S8-20`**（共 **20** 条）~~ ~~**`P-S8-1` ~ `P-S8-25`**（共 **25** 条）~~ ~~**`P-S8-1` ~ `P-S8-34`**（共 **34** 条）~~ ~~**`P-S8-1` ~ `P-S8-35`**（共 **`35`** 条）~~ ~~⇒ **`P-S8-1` ~ `P-S8-38`**~~ ⇒ **`P-S8-1` ~ `P-S8-47`**（🔴 **总数见文首「现行汇总账」权威行，本行 2026-08-09 起不复述**；构成：§15 本表 11 + §15.0 四条 + §15.0b 五条 + §15.0c 五条 + §15.0d 十三条 + **§15.0e 六条**） | 本文 §15，**与 sprint7 的裸 `P-N`（已用到 P-73）显式分段** |
...
### 15.0e 🔴 批次 2 执行期新增勘误（~~**6 条**~~ **`9` 条**：`P-S8-39 ~ ~~P-S8-44~~ `P-S8-47``，2026-08-10 ~ **08-11**，起点 `HEAD=ac60379`；**`P-S8-39~41` 由 `T-S8-2-2` 实现前核实挖出，`P-S8-42~44` 由 `T-S8-2-1` 落盘后真跑全量回归暴露，`P-S8-45~47` 由 `T-S8-2-1b` 实做时挖出**）

> **纪律**（沿 §15.0b / §15.0d 先例）：实做时凡与文档记载对不上、或按文档做会漏做/做错的，一律落进本表并写清磁盘实测值，**不默默按文档写的做**。
> 🔴 **本表 ~~3~~ ~~6~~ `9` 条里有 ~~2~~ `3` 条（`P-S8-39` / `P-S8-40` / **`P-S8-43`**）是同一份文档内部两处直接打架** —— 不是"文档与磁盘不符"，是"**文档与自己不符**"。⚠ **这类失真比对不上磁盘更危险**：执行者读到哪一处全凭运气，而两处都在同一份 dev-plan 里、都带着权威口吻。⇒ **`T-S8-2-2` 这类"实现前核实"任务的射程必须包含"同文档内互证"，不能只做"文档 vs 磁盘"。**
