...
> 　⇒ 🔴 **现行汇总账（2026-08-08；数字取自 §13 实算表，逐行加总非沿用）：检查点 `331` 个**（逐批 **25 / 45 / `150` / 83 / 16 / 12**）、**任务 40 个**（批次 2 = **15**）、**落点勘误 ~~`38`~~ ~~`41`~~ `44` 条**（`P-S8-1` ~ **`P-S8-44`**，其中**实质性 ~~`16`~~ ~~`17`~~ `18` 条**；
🔴 **2026-08-10 批次 2 执行期新增 6 条 `P-S8-39 ~ P-S8-44`，见 §15.0e** —— 前 3 条由 `T-S8-2-2` 实现前核实挖出、后 3 条由 `T-S8-2-1` 真跑全量回归暴露）、**冻结表 10 行 → `5` 行**。~~（作废旧账：勘误 `35` 条）~~⚠ **上一行（`:6`，2026-08-07 那轮）记的 `329` / `148` / `20 条` / 「10→4」全部是当时的账、已被本轮作废** —— 保留作留痕，**不要照它对账**。
...
**落点勘误**：🔴 **总数见文首「现行汇总账」权威行（本处 2026-08-09 起不复述数字）**；下列为编号构成。~~（作废旧账：15 / 20 / 25 / 34 / `35` 条）~~（§15；v1.0~v1.2 的 **P-S8-1 ~ P-S8-11**（11 条）+ v1.3 新增 **P-S8-12 ~ P-S8-15**（4 条，§15.0）+ 批次 1a 执行期 **P-S8-16 ~ P-S8-20**（5 条，§15.0b）+ 批次 1b 开工前置期 **`P-S8-21 ~ P-S8-25`**（5 条，§15.0c）+ **批次 1b 执行期 + 批次 2 开工前置期 `P-S8-26` ~ ~~`P-S8-35`~~ `P-S8-38`**（~~10 条~~ **`13` 条**，**§15.0d**，2026-08-08 ~ 08-09，含主控收口挖出的 `P-S8-35` 与批次 2 开工前置三组失真收口的 **`P-S8-36 ~ P-S8-38`**）+ 🔴 **批次 2 执行期 `P-S8-39 ~ P-S8-44`**（**6 条**，**§15.0e**，2026-08-10；前 3 条 `T-S8-2-2` 实现前核实挖出、后 3 条 `T-S8-2-1` 真跑全量回归暴露；**其中 3 条是同一份文档内部两处直接打架**）。实质性 ~~`14`~~ ~~`16`~~ **`17` 条**：P-S8-3 / P-S8-4 / P-S8-5 / P-S8-9 / 🔴 **P-S8-12（架构未覆盖）** / 🔴 **P-S8-16（`AR-S8-15` 触发条）** / P-S8-18 / `P-S8-21` / `P-S8-22` / `P-S8-23` / `P-S8-24` / **`P-S8-27`** / **`P-S8-31`** / **`P-S8-35`** / 🔴 **`P-S8-36`（验钞口径，安全面）** / 🔴 **`P-S8-37`（`T-S8-2-8` 缺实现步骤）** / 🔴 **`P-S8-40`（`_collect_grouped_metrics` 存废，三处仍写「不删」）** / 🔴🔴 **`P-S8-42`（§3.3 变红面表漏三族，收口门自身失效）**）⟦🔴 **2026-08-08 订正**：本行**上一轮（20 → 25）整个漏跟**，四个同源汇总口只改了另外三处 ⇒ 同一份文档里两个数并存了半天。已登记 **`P-S8-32`**（`P-S8-22` 同族复发）⟧⟦🔴 **2026-08-09 补充实测**：`P-S8-32` 说勘误总数有"**4 个**同源汇总口"，**实为 5 个** —— 它漏了**文首 `:9` 那行「现行汇总账」**（该行同样写着"落点勘误 N 条"）。本轮已**五处一并**推到 `38` 条 / 实质性 `16` 条⟧
...
| **落点勘误** | ~~**`P-S8-1` ~ `P-S8-11`**~~ ~~**`P-S8-1` ~ `P-S8-20`**（共 **20** 条）~~ ~~**`P-S8-1` ~ `P-S8-25`**（共 **25** 条）~~ ~~**`P-S8-1` ~ `P-S8-34`**（共 **34** 条）~~ ~~**`P-S8-1` ~ `P-S8-35`**（共 **`35`** 条）~~ ~~⇒ **`P-S8-1` ~ `P-S8-38`**~~ ⇒ **`P-S8-1` ~ `P-S8-44`**（🔴 **总数见文首「现行汇总账」权威行，本行 2026-08-09 起不复述**；构成：§15 本表 11 + §15.0 四条 + §15.0b 五条 + §15.0c 五条 + §15.0d 十三条 + **§15.0e 六条**） | 本文 §15，**与 sprint7 的裸 `P-N`（已用到 P-73）显式分段** |
...
### 15.0e 🔴 批次 2 执行期新增勘误（**6 条**：`P-S8-39 ~ P-S8-44`，2026-08-10，起点 `HEAD=ac60379`；**前 3 条由 `T-S8-2-2` 实现前核实挖出，后 3 条由 `T-S8-2-1` 落盘后真跑全量回归暴露**）

> **纪律**（沿 §15.0b / §15.0d 先例）：实做时凡与文档记载对不上、或按文档做会漏做/做错的，一律落进本表并写清磁盘实测值，**不默默按文档写的做**。
> 🔴 **本表 3 条里有 2 条（`P-S8-39` / `P-S8-40`）是同一份文档内部两处直接打架** —— 不是"文档与磁盘不符"，是"**文档与自己不符**"。⚠ **这类失真比对不上磁盘更危险**：执行者读到哪一处全凭运气，而两处都在同一份 dev-plan 里、都带着权威口吻。⇒ **`T-S8-2-2` 这类"实现前核实"任务的射程必须包含"同文档内互证"，不能只做"文档 vs 磁盘"。**
...
| **`P-S8-43`**（同文档内互斥，`P-S8-39` 同型第二例） | `CP-2.1-1③` 要求「**全仓**（`core/` + `ui/` + `app.py`）四名**零引用**」；同任务 `CP-2.1-6` 要求「删除处注释含窗口告示的指路文字」，而 `T-S8-2-1` 正文第 5 条明令那条注释要写明「`<METRICS>` 通道退场 / `metrics` 现在的唯一来源 / 窗口期 `success` 恒假」 | ❌ **两条按字面无法同时满足**：把删除留痕写清楚（`CP-2.1-6`）就必然在注释里**点名这四个函数**，而"零引用"若按 `grep` 字符串命中判，`CP-2.1-1③` **当场红**。本轮初版自测正是这么红的 —— **命中 11 处，全部是本次为满足 `CP-2.1-6` 写下的注释与 docstring** | 中：**两条都是本任务自己的检查点**，执行者会在"写清楚留痕"与"零命中"之间反复横跳；更坏的情况是**为了让 `CP-2.1-1③` 绿而把窗口告示删掉** —— 那恰好毁掉 `CP-2.1-6` 要防的东西（后人误"修"） | ✅ **口径收窄为「零*代码*引用」并落地为 `ast` 断言**：扫 `ast.Name` / `ast.Attribute` / `ast.ImportFrom`，**注释与 docstring 不计**。实测**零命中**。⇒ **通用一句**：「零引用」这类断言必须写明**在哪一层零** —— 字符串层 / 代码层 / 调用链层是三件事，**留痕纪律天然要求字符串层非零** |
