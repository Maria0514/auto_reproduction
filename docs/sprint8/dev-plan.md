...
**落点勘误**：🔴 **总数见文首「现行汇总账」权威行（本处 2026-08-09 起不复述数字）**；下列为编号构成。~~（作废旧账：15 / 20 / 25 / 34 / `35` 条）~~（§15；v1.0~v1.2 的 **P-S8-1 ~ P-S8-11**（11 条）+ v1.3 新增 **P-S8-12 ~ P-S8-15**（4 条，§15.0）+ 批次 1a 执行期 **P-S8-16 ~ P-S8-20**（5 条，§15.0b）+ 批次 1b 开工前置期 **`P-S8-21 ~ P-S8-25`**（5 条，§15.0c）+ **批次 1b 执行期 + 批次 2 开工前置期 `P-S8-26` ~ ~~`P-S8-35`~~ `P-S8-38`**（~~10 条~~ **`13` 条**，**§15.0d**，2026-08-08 ~ 08-09，含主控收口挖出的 `P-S8-35` 与批次 2 开工前置三组失真收口的 **`P-S8-36 ~ P-S8-38`**）+ 🔴 **批次 2 执行期 `P-S8-39 ~ P-S8-44`**（**6 条**，**§15.0e**，2026-08-10；前 3 条 `T-S8-2-2` 实现前核实挖出、后 3 条 `T-S8-2-1` 真跑全量回归暴露；**其中 3 条是同一份文档内部两处直接打架**）。实质性 ~~`14`~~ ~~`16`~~ ~~`17`~~ **`18` 条**⟦🔴 **2026-08-11 主控订正**：本行原写 `17`，而**同一行的编号列表实为 18 个**（下列逐个可数），文首 `:10` 权威行写的也是 `18` ⇒ 本轮新增两条实质性（`P-S8-40` / `P-S8-42`）时**编号加进来了、数字只推了一格**。🔴 **`P-S8-32` 同族又一次发作，且这次最刺眼——两个对不上的数就在同一行里**，连"跨章节漏跟"这个借口都没有⟧：P-S8-3 / P-S8-4 / P-S8-5 / P-S8-9 / 🔴 **P-S8-12（架构未覆盖）** / 🔴 **P-S8-16（`AR-S8-15` 触发条）** / P-S8-18 / `P-S8-21` / `P-S8-22` / `P-S8-23` / `P-S8-24` / **`P-S8-27`** / **`P-S8-31`** / **`P-S8-35`** / 🔴 **`P-S8-36`（验钞口径，安全面）** / 🔴 **`P-S8-37`（`T-S8-2-8` 缺实现步骤）** / 🔴 **`P-S8-40`（`_collect_grouped_metrics` 存废，三处仍写「不删」）** / 🔴🔴 **`P-S8-42`（§3.3 变红面表漏三族，收口门自身失效）**）⟦🔴 **2026-08-08 订正**：本行**上一轮（20 → 25）整个漏跟**，四个同源汇总口只改了另外三处 ⇒ 同一份文档里两个数并存了半天。已登记 **`P-S8-32`**（`P-S8-22` 同族复发）⟧⟦🔴 **2026-08-09 补充实测**：`P-S8-32` 说勘误总数有"**4 个**同源汇总口"，**实为 5 个** —— 它漏了**文首 `:9` 那行「现行汇总账」**（该行同样写着"落点勘误 N 条"）。本轮已**五处一并**推到 `38` 条 / 实质性 `16` 条⟧
...
> 🔴 **本表 ~~3~~ `6` 条里有 ~~2~~ `3` 条（`P-S8-39` / `P-S8-40` / **`P-S8-43`**）是同一份文档内部两处直接打架** —— 不是"文档与磁盘不符"，是"**文档与自己不符**"。⚠ **这类失真比对不上磁盘更危险**：执行者读到哪一处全凭运气，而两处都在同一份 dev-plan 里、都带着权威口吻。⇒ **`T-S8-2-2` 这类"实现前核实"任务的射程必须包含"同文档内互证"，不能只做"文档 vs 磁盘"。**
