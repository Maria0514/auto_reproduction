...
| V8 ⚠真跑 | 端到端（~~AC-S8-21~~ **`AC-S8-24`** 内⟦🔴 **2026-08-11 主控订正（`P-S8-53`）**：`AC-S8-21` 是「两类基线账目显式对平」，与端到端真跑无关；本行的构造与期望（「最终档位非失败」「物证可溯源到本次代码目录下的真实文件」）**逐字就是 PRD `:494` 的 `AC-S8-24`「端到端真值」**⟧） | 跑到一半触发一次 `request_user_input` 后 resume | 最终档位非「失败」，物证路径可溯源到本次代码目录下的真实文件 | 现场证据 |

V3 建议直接用既有真跑夹具 `tests/fixtures/s713_realrun_20260802/` 重放，与 ~~AC-S8-16~~ **`AC-S8-19`** 共用夹具、不新建现场⟦🔴 **2026-08-11 主控订正（`P-S8-53`）**：`AC-S8-16` 是「逐条回验三态」，不涉夹具；**点名这份 24 条真跑夹具的是 PRD `:489` 的 `AC-S8-19①`**。⚠ `dev-plan.md:1396`（`CP-2.4-3`）写的一直是 `AC-S8-19`（**对的**）⇒ 又一次「两份文档对同一句话引两个号，错的是被引用方」⟧。
...
- `ui/term_map.py`：`conclusion_level:science/engineering/none` **三条整体删除**，换发为四条 **恒等映射**（key 的 value 部分与 label **逐字相同**）：
  ...
  这不是"两套值"——它是同一个值，`humanize` 调用点因此**一个都不用改**，术语守门扫描面与计数保持"相等断言"闭合（~~AC-S8-17⑤~~ **`AC-S8-20⑤`**⟦🔴 **2026-08-11 主控订正（`P-S8-53`）**：`AC-S8-17` 是「造假审计已改为证据输入」、**只有 ①②③④**；「四档档名文案已换发且进术语守门、计数精确闭合（保持相等断言，禁止放宽为不少于）」是 PRD `:490` 的 **`AC-S8-20⑤`**⟧）。**恒等映射的存在理由是保住守门通道，不是做转换**，须在 term_map 里写一行注释说明，防后人当冗余删掉。
...
⇒ agent 读参考仓库里的结果表 **不被拒绝**，但**拿它当物证一律不成立**（R-S8-03 的落地形态：堵的是"从官方仓库抄一个对得上的数"）。**测试须有一条正向用例证明"读参考仓库成功"与一条负向用例证明"引用参考仓库路径作物证不成立"同时为真**——两条一起才叫验完（~~AC-S8-05④ + AC-S8-04~~ **`AC-S8-06④` + `AC-S8-05`**⟦🔴 **2026-08-11 主控订正（`P-S8-53`）**：**两个号同时错了一位**。`AC-S8-05` 无子项（「执行环节能读文件」，正身就是这里说的**正向**那条）；**负向**那条「路径越出本次代码目录一律不成立」是 PRD `:476` 的 **`AC-S8-06④`**；而 `AC-S8-04` 是「编码环节知道要产出什么」，与两个闸无关。🔴 **`dev-plan.md:1461`（`CP-2.5-4`，下一个任务）原样抄了这一对错号，已同批订正**⟧。
...
**~~AC-S8-06③~~ PRD `§4.5.1`「判定落点搬迁」⟦🔴 **2026-08-11 主控订正（`P-S8-53`）**：`AC-S8-06③` 是验钞第三重「数值前缀匹配可查」，与本句无关；而**全 PRD 没有任何一条 AC 写「报告环节不再自行判定档位」** —— 该规则的真实出处是 `§4.5.1`（判定落点搬迁）+ 本文档 `§1.5`（报告环节读已落盘的 `conclusion`，不重算、不重读盘）。⇒ 本条属**无正确目标的悬空引用**，不硬凑编号⟧的负向断言落点**：`_assemble_conclusion` 函数体内**不得出现任何 `level = ...` 的条件赋值**（今天 `:313-322` 那段 if/elif/else 整段删除）。静态审查即可断言。
...
- **绝不为了让旧报告好看而在报告侧重新判定**——那正是 ~~AC-S8-06③~~ **PRD `§4.5.1`「判定落点搬迁」**要禁的（同上，`P-S8-53`）。
...
1. **无 `baseline_results` 的路径**：payload 与 sp7 基线**字节零扰动** ⇒ **既有 HumanMessage 字节基线不换发**（~~AC-S8-12②~~ **`AC-S8-15②`** 的断言对象就是这一条⟦🔴 **2026-08-11 主控订正（`P-S8-53`）**：`AC-S8-12` 是「纯定性论文可达最高档」、**无子项**；「无该数据时 payload 与基线字节零扰动」是 PRD `:485` 的 **`AC-S8-15②`**⟧）。
...
4. ⚠ **但系统提示词哈希基线本批必须换发一次**——原因**不是**本项，而是 S8-04/05 要求改写"成功判定纪律（强约束）"三句（`:1159-1162`，现文明写"你不判定复现是否成功"）+ "输出要求"段（新增档位/逐条结论/物证字段）。这两处属**同一次改写**，哈希基线**只换发一次**，须在开发计划里预先列为预期改动（~~AC-S8-18②~~ **`AC-S8-21②`**⟦🔴 **2026-08-11 主控订正（`P-S8-53`）**：`AC-S8-18` 是「成功布尔与路由由档位派生」、**无子项**；「提示词哈希基线换发，**在开发计划里预先列为预期改动**」逐字是 PRD `:491` 的 **`AC-S8-21②`**⟧），**禁止事后补记**。
...
⚠ 这条同时是 ~~AC-S8-15~~ **`AC-S8-18`**「`_apply_no_metrics` 已删除且无残留引用」的**边界澄清**⟦🔴 **2026-08-11 主控订正（`P-S8-53`）**：该句**逐字在 PRD `:488` 的 `AC-S8-18` 里**，`AC-S8-15` 是「论文报告值确已送达执行环节」。⚠ `dev-plan.md:1357` 写的一直是 `AC-S8-18`（**对的**）⟧：清零断言的对象是**函数与其调用点**，**不是枚举成员**。写测试时若把枚举成员一并清零，会当场把旧快照兼容打掉。
...
⇒ **判别式一句话：`Enum` 有运行时成员查找，`TypedDict` 没有。** 前者删成员会炸，后者删声明不会。**~~AC-S8-15~~ `AC-S8-18` 那条"`_apply_no_metrics` 已删除且无残留引用"的清零断言，射程仍然是"函数与其调用点"，既不含枚举成员、也不含状态键。**
