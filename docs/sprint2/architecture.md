...
| §10 LLM 配置策略 | `ui/components/llm_config_form.py` | UI 注入 LLMConfig（不持久化 api_key）；api_key 可留空，由 create_llm 回退 .env（方案 A，见 §2.7.2） |
...
**api_key 回退点裁定（方案 A，Maria 拍板 2026-06-07）**：

- **回退发生在 `core/llm_client.py::create_llm`**（消费层最末端，紧邻 `ChatOpenAI` 构造）：当 `LLMConfig["api_key"]` 为空（strip 后为 ""）时，回退 `config.get_llm_api_key()`（读 `.env` 的 `LLM_API_KEY`）。
- **为什么放这层而非表单层 / `_refresh_llm_config_set` 层**：表单层与 `_refresh` 层之后都要经 `create_initial_state → SqliteSaver` 落盘；在这两层回退会把真实 key 写进 `LLMConfigSet → checkpoint`，违背"真实 key 不进 UI/session_state/checkpoint"的安全目标。**明确否决在表单层 / `_refresh` 层回退**。
- **为什么是 `create_llm` 而非 `resolve_llm_config`**：`resolve_llm_config` 是纯选路函数（不创建 `ChatOpenAI`、不发起请求、无副作用），回退属于"消费前补值"，应留在 `create_llm`。
- **override 一致规则**：回退只在 `create_llm` 一处实现，对 default / override 无差别生效——`resolve_llm_config` 命中哪条 `LLMConfig`（default 或某 override），该条空 api_key 就在 `create_llm` 统一回退同一个 `.env` 源。不在 default 层"预回退后让 override 继承"。
- **链路上 api_key 状态**：表单(`""`) → `LLMConfigSet.default.api_key=""` → `_refresh_llm_config_set`(`""`) → `create_initial_state`(`""`) → SqliteSaver checkpoint(`""`，恒空可断言) → `resolve_llm_config`(`""`) → **`create_llm` 回退 `.env` 真实 key（仅进程内存，不回写 state）**。
- **app.py 须调 `load_dotenv`**（与 `tests/conftest.py` 一致），否则 `os.environ` 无 `LLM_API_KEY`，回退取到 None 仍会 404；兜底校验（§2.8.2）在表单提交时即拦截此情况。
...
def render_llm_config_form(
    ...
    - base_url（text_input；**仅 default panel** 预填 config.get_llm_base_url()
                作 value=（默认 https://inference-api.nvidia.com/v1）；override 卡片不预填）
    - model（text_input；**仅 default panel** 预填 config.get_llm_model() 作 value=
             （默认 azure/openai/gpt-5.4）；override 卡片不预填）
    - api_key（text_input，type="password"，**每条独立 mask**，4+1 条互不影响；
               **允许留空**——留空时由 worker 消费层 create_llm 回退 .env 的 LLM_API_KEY，
               真实 key 永不进 UI/session_state/checkpoint，见 §2.7.2 回退点裁定）
    - temperature（slider 0.0~1.0，默认 0.3）
    - max_tokens（slider；min=512 / max=16384 / step=512 / 默认=8192，
                  与 config.DEFAULT_LLM_MAX_TOKENS=8192 对齐，
                  修正历史 _MAX_TOKENS_DEFAULT=4096 的不一致）

    预填策略（OBS-D1-01 约束）：default panel 的 base_url/model 预填值仅通过
    value= 注入，**不预写 st.session_state**；override 卡片维持空（保持"全空=不覆写"
    语义，不动 _panel_is_blank）。

    校验规则（每条 LLMConfig 独立校验）：
    - 全局默认 panel：base_url / model 非空、范围合法（必填）；**api_key 允许留空**
                     （留空时由 create_llm 回退 .env，故 _validate_panel 不再对 api_key
                     报"不能为空"）；
    - override 卡片：若用户**填了 base_url 或 model**则视为"开启覆写"（_panel_is_blank
                     仍按 base_url/model/api_key 三字段全空判定，不变），开启后必须
                     base_url/model 合法填齐、范围合法；api_key 同样允许留空回退；
                     若**所有字段全空**则视为"不覆写"，该节点不写入 overrides 字典；
    - **最终兜底校验（提交成功路径末端）**：组装出 default.api_key 为空 **且**
                     config.get_llm_api_key() 也为空（.env/环境均无 LLM_API_KEY）时，
                     st.error("未提供 api_key 且环境变量 LLM_API_KEY 为空，无法调用 LLM")
                     并返回 None——早失败，避免静默走到 worker 才 404；
    ...
    **不持久化到磁盘**；浏览器刷新（F5）后需重新输入。
    ...
    方案 A（Maria 拍板 2026-06-07）：推荐用户在表单 api_key 框留空，由 create_llm
    在消费层回退 .env 的 LLM_API_KEY。此时 LLMConfigSet.default/overrides 的 api_key
    恒为 ""，经 start_task → create_initial_state → SqliteSaver 落盘的也恒为 ""，
    真实 key 永不进 UI/session_state/checkpoint（回退点裁定见 §2.7.2）。
    app.py 需调 load_dotenv（与 tests/conftest.py 一致）以保证 .env 注入进程环境。
    """
```
