...
- [ ] [2026-05-06] 实现 `core/nodes/planning.py`——节点4：复现规划 + interrupt 人在回路（**不实现 revise 上限**，决策详见 Sprint 2 PRD AC-S2-06 / Q-S2-03 RESOLVED 2026-05-18：revise/switch_repo 无次数上限，依赖 `MAX_TOTAL_LLM_CALLS = 50` 总预算自然兜底；新增 `cancel` 决策分支，写入 `current_step = "cancelled_by_user"`，见 AC-S2-13）
...
- [ ] [2026-05-06] 实现 `app.py`——Streamlit 应用入口 + 工作线程 + 轮询机制（**Sprint 2 范围内新增 `cancel_task(thread_id)` 接口**，作为 planning revise 无上限的主动出口；详见 PRD §5.5 + AC-S2-13）
...
- [ ] [2026-05-27] @测试工程师代理 [A3-watchlist] A3 任务验收时核对 4 处守门：`core/react_base.py:825` 改读 `llm_config_set` / `tests/test_sprint1_smoke.py:229` 直读 `llm_config` 位点改读 / `tests/test_graph_e2e.py:254` 直读 `llm_config` 位点改读 / `tests/test_sprint2_a1.py::test_cp_a1_aux_6_llm_config_mirror_invariant_both_paths` 升级为 `llm_config not in state` 反向断言 + CP-A1-5 docstring 镜像段落删除；触发条件 = A3 任务交付后
