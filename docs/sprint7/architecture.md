# Sprint 7 核心架构设计文档：修复循环失控治理族（S7-01~03）+ 修复循环记忆增强（S7-05）+ 资源探索只读环境探测（S7-06）+ planning 平台感知规划（S7-08）+ 计划与编码/执行落点对齐（S7-10）+ 执行完整度进判定（S7-11）

**文档版本**：v1.6（v1.0 = Q-S7-1~6 六项裁决 + S7-01~03 三需求方案；v1.1 增补 §13 = S7-05 修复循环记忆增强档 B 方案；v1.2 增补 §14 = S7-06 只读环境探测的安全底座，仅裁 Q-S7-7/Q-S7-8 两项；**v1.3 增补 §15/§16/§17 = S7-06 剩余四项裁决 Q-S7-9~12 全部收口 + 主控跨节合并裁定**，Maria 2026-07-28 授权；**v1.4 增补 §18 = S7-08 平台感知规划 Q-S7-13~15 三问全裁**；**v1.5 增补 §19 = S7-10 计划与编码/执行落点对齐 Q-S7-17~24 八问补落章**；**v1.6 = 2026-08-04 交付后回填：增补 §20 = S7-11 执行完整度进判定 Q-S7-25~31 两轮裁决补落章 + §21 = §18/§19/§20 三章的交付状态逐条核对表**）
**日期**：2026-07-19（v1.0）／2026-07-20（v1.1 §13）／2026-07-28（v1.2 §14、v1.3 §15~§17）／2026-07-29（v1.4 §18）／2026-07-31（v1.5 §19）／2026-08-04（v1.6 §20~§21）
**作者**：架构师代理（§15 与 §16 由两位架构师并行独立裁决，§17 为主控跨节收口；§18~§19 由架构师代理裁决、主控逐条上磁盘核实；§20~§21 为 2026-08-04 交付后回填）

> **⚠ 头部与正文曾长期不一致（2026-08-04 订正留痕）**：本文件正文自 2026-07-29 起已含 **§18**（S7-08）、自 2026-07-31 起已含 **§19**（S7-10），而**标题、`文档版本`、`日期` 三行一直停在 v1.3 / 只列到 S7-06**。成因是两章各自只在**章内**写了自己的版本号（§18 前言写"架构 v1.3 → v1.4"、§19 章末写"v1.5 增补完"），**没有一个人回头改头部** —— 于是"这份文档写到哪了"这个问题，读头部得到的答案是错的。本次一并订正，形态记在 §21.2。
...
## 20. S7-11 执行完整度进判定（Q-S7-25 ~ Q-S7-31 两轮裁决补落章）

> **补落章说明**：本章七问由架构师代理于 2026-08-01 分两轮裁决——**第一轮 Q-S7-25 ~ Q-S7-27**（完成度算法 / 并存关系 / 死锁护栏）、**第二轮 Q-S7-28 ~ Q-S7-31**（改判落点 / 分类取值 / 叠加优先级 / 对外口径），含附则 **Q-S7-29a**（不吃 ledger）与 **Q-S7-29b**（文案约束）。此前**只记在 `dev-plan.md` §49.2、从未落本文件**——`dev-plan` §55.2 自己白纸黑字登记了这条缺口（原文："均尚未落 `architecture.md`"），但**没有任何 TODO 条目在跟**，于是它和 §19 补落章前的形态完全一样。本次按「代码交付后回填」制补齐。
> **本批不走 PRD**（Maria 2026-08-01 明确：修法 A/B 是纯缺陷修复，修法 C 的产品决策她当场拍板）⇒ 验收点用 `DA-S7-11-N`，**不占用 `AC-S7-*` 号段**（与 S7-10 同款处置）。
> **交付**：`f5a68d7`（S7-11 + S7-12 两批因 `execution.py` 交叉施工合并提交）+ `717ccb6`（BUG-S7-11-01 阻塞级分母缺陷修复）。
> **⚠ 本章有两条裁决在交付过程中被推翻 / 撤回**，逐条记在 §20.8，**原裁决原文不改写**。

### 20.0 立项事实：判定口径为什么必须改

2026-08-01 第三次真跑（arXiv:1802.03426）：**计划 9 步只完成 2 步、执行总耗时 0.243 秒，却判 `success=true`**，报告形态出 `full_success`；同一份报告顶部又如实印着「⚠️ 计划步骤未全部执行完成（已完成 2/9 步）」——**判定逻辑压根没看这个数**。

铁证在两轮日志里并排摆着：round_0 跑了 **17 条命令、5 条非零退出** ⇒ 判失败、回修复循环；round_1 只跑 **2 条、全 0** ⇒ 判**成功**。**做了 17 件事失败，做了 2 件事成功。** 更糟的是 round_1 的指标是收尾脚本**汇总 round_0 遗留产物**得来的——"成功"的指标源自上一轮的残留。

**定性：这不是 bug，是"成功"这个词的定义写窄了。** 原式 `success = exit_ok and len(metrics) >= 1` 只问"做过的有没有做错"，不问"该做的做完没有"，而 `run_results` 逐轮重置 ⇒ **构成反向激励：agent 做得越少越容易被判成功。** 本批把定义改成三合取项：**该跑的跑完没有 ∧ 跑过的没跑错 ∧ 有指标。**

**根因分四层**（前三层是主控识别的，第四层是裁决时新挖出的、也是修法 C 能否成立的命门）：

| 层 | 内容 | 落点 |
|---|---|---|
| 一（最根本，信息缺失） | 修复轮上下文里**没有任何字段告诉执行节点"代码已经被改过了"**。编码节点的自述 `last_fix_note` / `last_files_written` 早已存在于 `GlobalState`，但在 execution 侧**只被 `_append_fix_record` 消费写进 `fix_loop_history` 供下一回合 coder 读、从未送到 execution agent 眼前** ⇒ agent 在它的认知里"那些命令还是坏的" | 修法 A |
| 二（提示词措辞） | 纪律里两次出现"如实收尾"＝明确的提前结束授权；"不要重复执行同一条命令空转"**没有区分"无意义空转"与"重跑验证修复"**，而后者恰是唯一必须重复执行同一条命令的场景；输出契约字段语义是"实际执行条数"/"已执行命令是否全 0"；连代码注释都把修复轮上下文的意图写成"帮助 agent **避开**上一轮已知错误"——**设计意图就是绕开，不是回验** | 修法 B |
| 三（判定口径） | `success = exit_ok and len(metrics) >= 1`，见上 | 修法 C |
| **四（新挖出，命门）** | **`success=False` 并不等于"回修复循环"**。回 coding 的唯一准入判据是 `feedback.auto_fixable`；而"全部 exit 0 + 有指标"这条路径上 feedback 恒为 `ErrorCategory.NONE` 且 `auto_fixable=False`（`NONE` 不在 `AUTO_FIXABLE` 里）⇒ **只做 A/B/C 而不配套改判，C 会把"交修复循环补跑"落成"直接打断用户"，设计意图落反** | 修法 D |

⇒ **A/B/C/D 四条必须同批生效，禁止拆批**：只上 C 不上 D ⇒ 打断用户；只上 C 不上 B ⇒ agent 只补跑缺失那一步，判定层**必然死锁**；只上 A/B 不上 C ⇒ 反向激励原封不动。任何"先上 C 观察一轮"的提议一律驳回。

### 20.1 Q-S7-25 完成度算法：原裁决 + Maria 复审推翻 + 保留生效的 Q-S7-25(0)

**原裁决（2026-08-01 第一轮）**：新写一个确定性纯函数 `_deterministic_completion`——按命令字符串归一后比对计划步骤与实跑命令，**不采信 agent 自报的 `step_index`**，理由是自报理论上可被绕过（agent 可以给任意命令打任意下标把完成数刷满）。

**⛔ Maria 同日复审推翻（已决，不再议）**：**砍掉整条确定性算法路线，改为直接采信 agent 自报的 `step_index`**，即复用既有 `_reconcile_steps` 已经产出的 `completed / planned`。

推翻的依据是**实测证据方向相反**：立项那次真跑的存档实测——round_0 `executed=8 / completed=3`、round_1 `executed=2 / completed=2`，**agent 首轮诚实声明了 8 步**（9 步里只有 1 步没声明）。**没有虚报。** 原裁决把"理论上可被绕过"当成了"实际正在被绕过"。⇒ **问题从来不在自报可信度，而在"exit_code 全 0 就算成功"这个口径。**

推翻带来一个**大坑自动消失**：`python scripts/x.py` 与 `python -m scripts.x` 的写法不匹配（原登记为一条系统性假红风险）**不再构成问题**——不比对命令字符串就不会误伤，agent 用哪种写法重跑、只要声明了下标就归属得上。该风险条目随之作废。

**代价与对冲**：自报可信度由此成为一条**产品级假设**（被明确接受，不是遗漏）。三重对冲：① `exit_ok` 与 `≥1 指标` 两个合取项仍是硬的；② 新增只读留痕（见 §20.5 附则）把"换了命令还自报同一下标"打成 WARNING；③ 真跑实测无虚报。

> **★ Q-S7-25(0)：判定口径为"单轮全量"，跨轮不取并集——本条是阻塞级前置裁决，未被推翻、现仍生效。**
>
> `run_results` / `step_ledger` 逐轮重置是**正确设计**。合取项上线后，若 agent 在修复轮只重跑失败那一步，本轮完成数必然小于分母 ⇒ **永远判不成功**。这不是边缘死锁，是**必然死锁**。两条出路只能选一：
>
> - **采纳（本批取此）**：**提示层要求每个执行回合从头按计划全量重跑，判定层坚持单轮全量。** 理由是产品定义——"复现成功"的证据必须是**一次干净的端到端跑通**，跨轮拼凑出的"累计完成 9 步"在**代码已被修改**的前提下根本不成立。
> - **否决**：在 state 层跨轮取已完成步骤下标的并集。它把"上一轮代码下步骤 3 通过"当成"当前代码下步骤 3 通过"，是另一种假绿，**与本次修复的初衷同型**。
>
> ⇒ **修法 B 的 prompt 约束是本批交付项而非"顺带"**，判定层的正确性硬依赖它。这是**新增耦合**，登记为阻塞级风险（R-S7-59）。Maria 独立提出同一结论，与本裁决一致；主控此前曾建议"完成度跨轮累计"——**那是错的，已撤回**。

### 20.2 Q-S7-26 并存关系：随 Q-S7-25 主体一并作废

原问：新写的确定性算法与既有 `step_reconciliation` 如何**并存分层**（谁供判定、谁供展示、两个完成数同时对外怎么办）。

**Q-S7-25 改为采信自报后，本问的前提消失**：全系统**只有一个完成数**——`step_reconciliation`。判定、报告横幅、步骤对账节**全部取它**，不存在口径分叉，**报告内不可能再自相矛盾**。`_reconcile_steps` 函数体一行不改（本批只把它的**调用位置前移**，因为改判步骤需要它的产出）。

**连带解除**：S7-10 立的红线「验收断言不得引 `step_reconciliation` 作证」**本批解除**——既然产品决定采信自报，它就是**判定的正式数据源**，验收自然可以引它。

### 20.3 Q-S7-27 死锁护栏：早停计数器压到第二批

原问：要不要加"连续 N 轮完成数无增长即早停"的计数器，防止判定收严后每次真跑都烧满 `MAX_FIX_LOOP_COUNT=20` 轮。

**裁决：不加，压到第二批。** 理由是**没有证据前它就是过度工程**——修复循环本就有轮次上限与预算两道兜底，撞顶后走既有 interrupt#2 面板让用户决策，链路是通的；再叠一个计数器是在猜一个还没发生的失败形态。

**2026-08-02 真跑数据支持了这条裁决**：`fix_loop_count = 2`，远低于上限 20，且每轮完成数有实质增长（失败数 8 → 2 → 0）。⇒ **早停仍属过度工程**，第二批也不必自动启动，等观测到"烧满轮次"的真实现场再议。

### 20.4 Q-S7-28 改判落点：走 feedback 通道，**不走路由旁路**

**裁决形态**：新增纯函数 `_apply_incomplete_execution(feedback, recon, exit_ok) -> ExecutionFeedback`，插在 `_apply_no_metrics` 的**紧邻上游**；**路由条件 `feedback.auto_fixable` 一字不改**。

**明确否决**"改路由条件、加一个 `or completion_insufficient`"，三条独立理由：

1. **会污染刚建好的反馈链路**：路由分支里由 `_append_fix_record` 拿 `feedback.category.value` 写 `fix_loop_history`。走旁路则历史里记的是 `error_category="none"` + 空 `fix_strategy`——**直接污染 S7-02 / S7-05 刚建起来的 coder 反馈与跨回合记忆链路**。
2. **会造出第二个真值源**：`auto_fixable` 目前是"是否回 coding"的**唯一准入判据**，旁路等于再造一个。
3. **guard 重入路径能零改动自动正确**：`_feedback_from_committed_result` 从落盘的 `errors[0]` 的 `[error_category=...]` 前缀重建 feedback；走 feedback 通道则 interrupt#2 两段式重入路径**一行不改**就正确。

> **附则 Q-S7-29a「不吃 ledger」**：`dev-plan` §55.2 只留下了这四个字的标题、正文从未落盘。**本节按落地形态如实描述、不追认不存在的原文**：改判链路**不直接消费 `step_ledger`** —— `_apply_incomplete_execution` 的入参只有 `(feedback, recon, exit_ok)`，台账只被 `_reconcile_steps`（归属）与只读的留痕函数消费。这维持了"对账事实源单点"这条既有纪律。

### 20.5 Q-S7-29 分类取值：新增 `INCOMPLETE_EXECUTION`，**不复用 `NO_METRICS`**

**裁决**：新增 `ErrorCategory.INCOMPLETE_EXECUTION = "incomplete_execution"` 并入 `AUTO_FIXABLE`。

**不复用 `NO_METRICS` 的三条理由**（第三条是决定性的）：

1. **会被早停误伤**：`_no_metrics_stalled`（连续 2 轮无进展即提前打断）显式 gate 在 `NO_METRICS` 上，复用会把"正在补跑"误判成"无进展"提前打断。
2. **修复提示指错方向**：fix_hint 会让 coder 去查"为什么没产出指标"，而真相是"步骤没跑完"。
3. **★ 会对用户撒谎**：`fix_loop_history.error_category` 是**面向用户的修复历程折叠条标题**（经 `ui/term_map.py` 的 `humanize` 转中文渲染在执行监控页）。复用会让界面连续印"未产出指标"，而真相是"步骤没跑完"。**对用户撒谎，比技术债更贵。**

**代价盘点（架构侧实测，比预估低——三处映射点全部零改动）**：
- `_map_category_to_error_type` 是 `if category in AUTO_FIXABLE: return "transient"` ⇒ **自动 transient，零改动**；
- `_feedback_from_committed_result` 是 `ErrorCategory(raw)` + `category in AUTO_FIXABLE` ⇒ **round-trip 零改动**；
- `_no_metrics_stalled` 显式 gate 在 `NO_METRICS` ⇒ **新类别天然不进早停**。

真实成本只有 `ui/term_map.py` 加一条 + 术语守门的三个计数。⚠ 术语措辞**刻意与 `annotation:incomplete_execution`（"执行不完整"）用不同的词**——两处同词会让用户分不清"报告标注"和"错误分类"。

> **附则 Q-S7-29b 文案约束**：改判产生的 summary 经 `_append_fix_record` 进 UI ⇒ **必须通俗中文、零内部标识符**；缺失步骤清单**必须截断**（前 5 条 + "等共 N 个"），步骤名走 `mask_value`（与对账同口径）；fix_hint 指向"把剩余步骤按计划跑完 / 排查这些步骤为何没跑（前置步骤失败？命令写法与计划不一致？）"，**不得出现"检查指标输出"字样**（那是 `NO_METRICS` 的话术，指错方向）。⚠ 归属不可用时未执行清单恒空 ⇒ 文案走**无清单**分支（只说"已跑完 X/Y 步"），**不得凭空编造步骤名**。

> **附则：防伪留痕（Maria 明令设计判据，替代被砍掉的确定性算法）**——新增只读纯函数 `_audit_declared_steps(plan_steps, run_results, step_ledger)`，**只打 WARNING、不进任何返回值、不进 state、不影响判定**。
> - **判据**：对每条有效 run，若它带**合法自报下标 `i`**，取该 run 的归一 key 与**计划第 i 步自身的归一 key 集合**比对；集合非空 ∧ run 的 key 不在集合内 ⇒ 记一条"自报与实际执行不符"。计划第 i 步无命令（纯 `cd` / 空串）时**不判**（无从比对，避免噪声）。
> - **为什么这个判据是对的**：它捕的正是"换了命令还自报同一下标"这一种虚报形态；而"写法变通"（`-m` / 相对路径差异）也会命中——**但那正是我们想看见的观测量**，所以是 WARNING 不是错误。
> - **日志形态**：命中时打**一条**汇总 WARNING（不逐条刷屏），命令与步骤名一律过 `mask_value`；零命中不打（无目标不制造噪声）。
> - **红线**：该函数**返回值不得被任何判定 / 渲染 / state 消费**。守门方式是"打桩使其恒报大量不符 ⇒ `success` 与 feedback 一字不变"。

### 20.6 Q-S7-30 叠加优先级：`INCOMPLETE` 优先于 `NO_METRICS`，且 `_apply_no_metrics` 一行不改

**裁决**：**优先级靠调用顺序拿，不靠加参数。** `_apply_no_metrics` 已有前置守卫 `feedback.category == ErrorCategory.NONE`——把 `_apply_incomplete_execution` 排在它上游，改判后 category 不再是 `NONE`，它**自动原样返回**。**零改动拿到优先级**，比给它加一个 `completion_ok` 入参好得多（加参会让 sp6 既有测试面全部改签名）。
...
**★ 单点谓词红线**：`success` 判定与 `_apply_incomplete_execution` 用的是同一个"完成度不足"判断，**必须抽成唯一小谓词 `_completion_insufficient(recon)`，两处都调它**。否则两处各写一遍比较，日后必漂移出"**改判了但 `success` 还是 True**"这种最隐蔽的假绿。

**主流程插槽顺序（关键）**：`_collect_grouped_metrics`（不动）→ **`_reconcile_steps` 调用位置前移**（函数体一行不改，只是从 `_apply_no_metrics` 之后挪到之前，因为下一步需要它的产出）→ **`_audit_declared_steps`（只打 WARNING）** → **`_apply_incomplete_execution`** → `_apply_no_metrics`（函数体零改动，仅位置后移）→ `_build_execution_result`。`exit_ok` 仍是**单点计算**、供两个改判函数共用，**不新增第二处 `exit_ok` 口径**。

⚠ **`ExecutionResult` 不加新键**——`step_reconciliation` 本就在里面。**全批零新增 state / schema 顶层字段。**

### 20.7 Q-S7-31 对外口径订正：判定口径改了，说明文字必须同批改

判定口径一改，报告里那句"判定口径"的说明文字就与实现不符 ⇒ 本批必做，否则又是一条"文档与代码不符"。

- **(a) 报告里的判定口径说明必须改，且顺手提为具名常量。** 原文两个问题：① 口径本身已过时（只写了退出码与指标两条）；② 它写着「判定口径（**B 档**）」——**"B 档"是内部分档术语裸露给用户**，本就违反产品红线，只是因为它是**内联 f-string、不在受控常量清单的覆盖面内**才一直没被扫到（与 S7-06 那次"扫 0 条却 passed"是同款失效模式）。⇒ 提为模块级常量 `_SUCCESS_CRITERIA_NOTE` 并登记进术语守门；文案重写为**三条件**、**去掉"B 档"**。"指标对比表仅供参考、不做硬性判定"那半句**逐字保留**。
- **(b)（⚠ 本条在实施时被撤回，见 §20.8.2）** 原裁决要求给报告的 `incomplete_execution` 标注条件补一条析取项，以免"归属不可用"时出现"结论说没跑完、横幅不印"的反向矛盾。
- **(c) 术语守门账目**：新增 `error_category:incomplete_execution` 一条中文映射，守门的三个精确计数同步 +1。**`==` 形态禁止放宽为 `>=`**——被撞一次**正是它的设计目的**（新增用户可见文案必须过一次评审）。

### 20.8 交付过程中被推翻 / 撤回的两条（原裁决原文不改写，此处如实登记）

#### 20.8.1 ★ BUG-S7-11-01（阻塞级）：本章原裁决的完成度**分母写错了**

**原裁决（§20.6 单点谓词那条）给的语义是 `planned > 0 and completed < planned`——用的是"原始步数"当分母。这是错的。**

后果链（测试工程师独立验收在 mock 层连跑 20 轮钉死）：计划里只要有**一条 agent 无从执行的步骤**（无 `command` 键 / 空串 / 纯 `cd`），它**永远进不了分子** ⇒ 即便 agent 完全照做、全 exit 0、指标齐全、诚实自报下标，也**恒判未完成、烧满 20 轮修复、每次真跑都被推到 interrupt#2**；而下一轮 coding 也变不出"人工查看图表"这种命令 ⇒ **这个循环解不开**。

⚠ 值得记一笔的是：**`dev-plan` §49.2 第 6 条与风险条正文写的本来就是"可执行步数"——设计写对了、本文件的裁决写窄了、实现照着窄的落了。** 这条订正也顺带推翻了"该风险只能由真跑证伪"的一半（mock 层就证出来了）。

**修法（比"把 `planned` 换成 `planned_actionable`"更根本）**：把"进得了分母"做成**归属规则②那套解析的布尔投影**，而不是另写一份判据——**"进得了分母"与"进得了分子"从此共用同一把尺子**，结构上杜绝同类缺陷再生。分母取数函数落在 `core/state.py`（放 `execution` 会撞 `reporting` 的纯度红线：reporting 不得 import 任何带 LLM 的模块），**判定层与展示层共用同一个取数点**——两份实现必然漂移出"判定说成功、报告说没跑完"的自相矛盾报告。

**"自然语言 command"的取舍：判为可执行、算进分母、不做识别。** 理由：`人工查看 outputs/figures 下的图` 与 `pyhton train.py`（真命令拼错）在字符串层**没有可靠判据可分**；三种启发式（英文词表 / 可执行文件存在性 / 非 ASCII 头 token）**都会把真步骤误剔出分母**——那是往**假绿**方向退，正是本批要修的东西。取舍原则：**宁可误红不可误绿。** 根治出口在 planning 侧强制每步 `command` 可执行（属跨节点契约，**另行立项**）。

**展示层配套**：原始步数键保留（"计划共 N 步"的陈述 + 自报下标的合法区间仍需要它），但"已完成 N/M"的 M 跟判定走；两者不等时**显式补一句**"其中 X 步没有可执行的命令、不计入完成度"，**不让用户自己猜**。不可执行步骤**也不进未执行清单**——否则报告的未完成标注会在 `success=True` 时点火，制造自相矛盾的报告，且等于给 coder 一个它变不出命令的伪修复目标。

#### 20.8.2 Q-S7-31(b) 的析取项在实施时**撤回**

原裁决要求给 `incomplete_execution` 标注条件补第三条析取项。实施时判定：**标注条件一字不改**。理由是该析取项唯一新增的只有"归属不可用"那一格，而"归属不可用时不打未执行标注"是**既有产品契约**（保守语义，两处既有测试守着）——补上去等于推翻一条既有契约去救一个尚未观测到的矛盾。⇒ **本条撤回，报告标注条件与 sp5 逐字节一致。**

> **这两条的共同教训**：架构裁决写"语义"时，**分母 / 边界条件这类看似枝节的定义必须与 dev-plan 逐字对齐**——本次是 dev-plan 写对、架构写窄；只要两份文档在同一个量上给出不同措辞而没人对表，就一定有一份会被实现照抄。

### 20.9 同批交付但**没有架构问答号**的两条（S7-12 / S7-13）说明

`f5a68d7` 与 `4a7420f` / `1e2577d` 里还含两条需求，**均由 Maria 直接拍板、未走架构问答**，故本文件不为它们开 Q 号。此处只记两条**架构级不变量**，以免日后被当成"没人裁过"而重新发明：

- **S7-12（沙箱 shell 元字符显式拒绝）**：沿 S7-10 立下的落点纪律——**同一条不变量在计划期与执行期各查一次，一处定义两处调用**。谓词与拒绝文案同址落在 `core/plan_checks.py`（该模块位于 `core/` 顶层、零项目内依赖，被 `core.nodes.execution` import 不成环；**反向 import 明令禁止**）。处置是**识别后拒绝，不是识别后支持**——实现管道 / 重定向语义等于自己写一个完整 shell，正是当初禁 `shell=True` 要逃离的东西，且边界无穷。⚠ 拦截早退点必须在**结果并入台账之前**，放错位置这条硬防线会自己制造假绿（S7-10 已踩过一次）。
- **S7-13（execution agent 指标汇报出口）**：根因是**确定性代码在猜 LLM 的意思**，三处全猜错（哪块是主指标 / 产物文件长什么样 / 两个组名是不是一回事），而每一次 agent 都知道答案——目录是它建的、命令是它发的、产物是它写的。⇒ 改为「**agent 报 → 装进现成结构 → 判定仍全归代码**」。两条自律是架构级的、不可让：① **主通道零指标时不采信自报**，否则 `≥1 指标` 这个成功合取项的分子会变成 agent 自报，**代码一字未改而判定语义被悄悄换掉**；② **agent 汇报与磁盘扫描刻意不合并**（实测合并会因组名归一后同名而让匹配判歧义、把本能匹配上的一条退回"未验证"）。另**砍掉 `metrics[].source`** 一条：**没有消费点的字段本身就是过度工程**——不产生任何可验证约束、却每回合白占 schema 与 prompt 字节、还要模型多想一层；日后若真跑发现报数与磁盘对不上，**要加就连同磁盘核对一起加**，而不是先摆一个"看着像防线"的空壳。

---

## 21. §18 / §19 / §20 三章交付状态核对表（2026-08-04 回填时逐条上磁盘核实）

> **纪律**：判"已实现"一律附 **文件:行号**，**不拿文档证明文档、不照抄 commit message**。下表每一格都在 2026-08-04 由架构师代理亲自 `Read` / `Grep` 核实。

### 21.1 逐章交付状态

| 章 | 需求 | 交付 commit | 磁盘证据（抽样） | 状态 |
|---|---|---|---|---|
| §18 | S7-08 planning 平台感知规划 | `8b254e0`（2026-07-30） | `core/state.py:155-156` 两键；`core/nodes/planning.py:118-119` schema、`:482-483` 构造、`:696-697` 降级路径、`:988` interrupt payload 第 11 键、`:163-177` 三级优先级冻结区、`:221-240` 两键契约段；`core/nodes/reporting.py:307-308` 第 4 条标注、`:690-701` 声明块；`core/nodes/coding.py:92`/`:459` 与 `core/nodes/execution.py:119`/`:1368` 双侧缩规模指令；`ui/pages/plan_review.py:427` 只读展示块、`:192` 讨论助手第 4 键；`ui/term_map.py:91` | **已交付** |
| §18.3 | 探测摘要上限 400 → 2600 + 总长 8000 | `8b254e0` | `core/nodes/resource_scout.py:68` = **2600**、`:80` = **8000**、`:89` 截断说明常量、`:537` 单条截断、`:562-564` 总长截尾；另 `:136` 起 prompt 由"3~5 条"改为**6 项必探维度** | **已交付** |
| §19 | S7-10 计划与编码/执行落点对齐 | `c480990`（2026-07-31） | `core/plan_checks.py:90` 阈值 120、`:143` 载荷定位、`:217` 单一谓词、`:554`/`:563` W4/W5；`core/nodes/execution.py:995-1003` 工具层硬拦截（位置在并入台账之前）、`:790` 拒绝文案；`core/nodes/planning.py:187` 删 `cd` 授权、`:189-196` 执行落点段 | **已交付** |
| §20 | S7-11 执行完整度进判定 | `f5a68d7` + `717ccb6` | `core/nodes/execution.py:157`/`:168` 新错误类别、`:1264` 修复轮上下文补线、`:2023` 单点谓词、`:2073` 可执行步判定、`:2097` 防伪留痕、`:2189` 改判纯函数、`:2976`/`:2987-2988` 插槽顺序、`:1922-1923`/`:2000` 分母子键；`core/state.py:321` 跨模块单一取数点；`core/nodes/reporting.py:560` 判定口径常量、`:675`/`:776`/`:803-809` 展示层同口径 | **已交付**（含 §20.8.1 的分母订正） |
| —（无章） | S7-12 沙箱 shell 元字符显式拒绝 | `f5a68d7` | `core/plan_checks.py:295` 受限 token 集、`:319` 共用文案、`:329` 谓词；`core/nodes/execution.py:1010-1018`、`core/tools/run_command_tool.py:106-111` 两处工具层早退 | **已交付**，见 §20.9 |
| —（无章） | S7-13 agent 指标汇报出口 | `4a7420f` + `1e2577d` | `core/nodes/execution.py:1092` 输出契约（`metrics` 项现为 `name`/`value`/`group` **三属性**，`source` 已砍）、`:1218` 透传字段、`:1617` 补传 `result_schema`、`:1781` 拆分纯函数、`:2939-2952` 门控合并、`:2961` 汇报优先磁盘兜底 | **已交付**，见 §20.9 |

### 21.2 本次回填挖出的文档失真（形态记录，供日后防复发）

| # | 失真 | 形态 | 处置 |
|---|---|---|---|
| 1 | 本文件头部停在 v1.3 / 标题只列到 S7-06，而正文已含 §18 / §19 | **章内写版本号、没人回头改头部** ⇒ "这份文档写到哪了"读头部得到错误答案 | 本次订正（EDIT A/B），并在头部留痕 |
| 2 | Q-S7-25 ~ Q-S7-31 七问裁决只活在 `dev-plan.md` §49.2 | **dev-plan §55.2 自己登记了"尚未落 architecture.md"，但没有任何 TODO 在跟** ⇒ 登记 ≠ 有人跟 | 本次补 §20 |
| 3 | 本章原裁决的完成度分母写窄（`planned` 而非可执行步数），dev-plan 写对 | **两份文档在同一个量上措辞不同、没人对表** ⇒ 实现照抄了窄的那份，酿成阻塞级缺陷 | 见 §20.8.1；纪律已写进该节末 |
| 4 | 全局 `docs/technical-architecture.md` §7.5 把**已修**的探测摘要上限缺口写成"当前实现，未修"（称 `_PROBE_OUTPUT_MAX_CHARS=400`，磁盘实为 2600） | **"待交付后回填本节"的挂账条目无人认领** ⇒ 挂账在交付后变成假陈述，方向危险（把已修的说成未修） | 已于 2026-08-04 在全局文档 §7.5 订正（原文删除线保留 + 补记结构性原则） |

---

*（v1.6 增补完：§20 = S7-11 执行完整度进判定的完整架构裁决——Q-S7-25 完成度算法（原裁"新写确定性算法"，**Maria 复审推翻改为采信 agent 自报**）+ 未被推翻的 Q-S7-25(0) 单轮全量口径 / Q-S7-26 并存关系随之作废 / Q-S7-27 早停计数器压第二批（真跑数据事后支持）/ Q-S7-28 改判走 feedback 通道不走路由旁路（三条理由）+ 附则 Q-S7-29a / Q-S7-29 新增 `INCOMPLETE_EXECUTION` 不复用 `NO_METRICS`（第三条理由"对用户撒谎比技术债更贵"为决定性）+ 附则 Q-S7-29b 文案约束 + 防伪留痕纯观测红线 / Q-S7-30 优先级靠调用顺序拿 + 单点谓词红线 / Q-S7-31 对外口径订正。§20.8 如实登记交付中被推翻的两条：**BUG-S7-11-01 本章分母裁窄致必然死锁**（dev-plan 写对、架构写窄）与 Q-S7-31(b) 析取项撤回。§20.9 记 S7-12 / S7-13 两条无 Q 号需求的架构级不变量。§21 = 三章交付状态逐条核对表 + 四条文档失真形态。本增补不覆盖 §1~§19 任何既有内容。）*
