...
> **默认参数变更注记（2026-06-30，Maria 拍板）**：修复循环三常量默认值放大——`MAX_FIX_LOOP_COUNT` 3→**10**、`MAX_DEV_LOOP_LLM_CALLS` 20→**60**、`MAX_TOTAL_LLM_CALLS` 50→**120**（强约束 `MAX_DEV_LOOP_LLM_CALLS < MAX_TOTAL_LLM_CALLS` 不变，60 < 120）。本文档正文涉及这些数值处已就地更新；架构机制（路由三道闸、预算单点回写、入口预算门）不变，仅常量取值放大。
...
| **改动** | `config.py` | S3-08 | 新增 `MAX_DEV_LOOP_LLM_CALLS=60` + sandbox 护栏常量；`MAX_FIX_LOOP_COUNT=10` 首次接线（默认值 2026-06-30 放大，见顶部注记） |
...
> - `MAX_DEV_LOOP_LLM_CALLS=60 < MAX_TOTAL_LLM_CALLS=120`：满足 PRD §5.2 强约束（AC-S3-04 断言）。默认值 2026-06-30 由 20/50 放大（见顶部注记）。
...
> 关键字表（`_HARDWARE_KEYWORDS` / `_DATA_MISSING_KEYWORDS` 等）作为模块级静态常量，复用 git_tools `_TRANSIENT_STDERR_KEYWORDS` 的小写匹配范式。**分类不准的风险（R-S3-04）**：兜底归 RUNTIME（可修复，给一次机会），由 `MAX_FIX_LOOP_COUNT=10` 上限拦截，不会无限重试。
...
- **进度展示**：`poll_state` 读 `current_step`（coding/execution/reporting）+ `fix_loop_count`/`MAX_FIX_LOOP_COUNT`（"修复第 N / 10 轮"，上限读常量）+ `fix_loop_history` 每轮摘要（错了什么 + 修复策略）。
...
- **子预算触顶**：`_dev_loop_llm_calls >= MAX_DEV_LOOP_LLM_CALLS(60)` → 视同修复耗尽，走 interrupt#2。

> **回问点 4（§7）**：`_dev_loop_llm_calls` 是否必须新增 state 字段，还是可由 `fix_loop_history` 长度 × 平均回合成本估算？架构推荐显式字段（精确、AC-S3-04 可断言「累计达 `MAX_DEV_LOOP_LLM_CALLS`（默认 60）终止」）。
...
| `config.py` | **新增常量** | `SANDBOX_*`（5 个）+ `MAX_DEV_LOOP_LLM_CALLS=60` + `DEV_LOOP_MIN_CALLS_PER_ROUND=2` + `REACT_MAX_ROUNDS_CODING=12` + 2 个 UI 路由常量；`MAX_FIX_LOOP_COUNT=10` 首次接线引用（默认值 2026-06-30 放大） | 低 |
...
**回问点 4（子预算累计 `_dev_loop_llm_calls` 字段）**：AC-S3-04 要求断言「修复循环累计 LLM 调用达 `MAX_DEV_LOOP_LLM_CALLS`（默认 60）时终止」。精确记账需显式累计字段。备选是用 `MAX_TOTAL_LLM_CALLS - retry_budget_remaining`（含前序节点消耗，不纯）或 `fix_loop_history` 长度估算（不精确）。**推荐显式 `_dev_loop_llm_calls` 字段**（单值 int，无 reducer）。请 Maria 确认是否接受。
...
**P4 验收对齐（测试工程师协作）**
- [ ] 五条 e2e（happy path B 档 / 修复循环上限（取 MAX_FIX_LOOP_COUNT，默认 10）/ interrupt#2 三选一 / code_only / 降级）+ sandbox 4 护栏 mock + must-fix-1 grep 断言 + must-fix-2 预算回写断言；报告归档 `docs/sprint3/test-reports/`。
