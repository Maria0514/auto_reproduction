...
- **编排层**：`core/graph.py` 新增 `coding↔execution` 单 agent 修复回边（上限 `MAX_FIX_LOOP_COUNT=3`）、code_only 路由、第二个 interrupt（dev_loop 失败三选一），**严格保持 7 节点 DAG，不新增任何节点**；
- **配置层**：新增 `MAX_DEV_LOOP_LLM_CALLS=20` 子预算 + `DEV_LOOP_MIN_CALLS_PER_ROUND=2` 入口预算门 + `SANDBOX_*` 护栏常量；统一 `MAX_FIX_LOOP_COUNT=3` 首次接线；
...
- **must-fix-2（预算扣减回写，对应 AC-S3-04）**：coding 走 `_make_react_wrapper` 自动扣减（react_base.py L889-894，无需手写）；execution **仅在 metrics LLM 抽取兜底触发时**按实际调用次数单点回写 `retry_budget_remaining`（不触发则零扣减，正确语义）；新增 `_dev_loop_llm_calls` 累计字段 + 入口预算门（`retry_budget_remaining < DEV_LOOP_MIN_CALLS_PER_ROUND=2` 直接降级）+ 子预算触顶（`_dev_loop_llm_calls >= MAX_DEV_LOOP_LLM_CALLS=20` 视同修复耗尽）。
...
| `MAX_DEV_LOOP_LLM_CALLS` | `20` | 修复循环子预算天花板（**强约束 `< MAX_TOTAL_LLM_CALLS=50`**） |
...
**关键约束**：
- 不修改 sp1/sp2 既有常量（含 `MAX_TOTAL_LLM_CALLS=50` / `MAX_FIX_LOOP_COUNT=3` —— 后者**值不变**，sp3 首次在修复循环路由接线引用）；
...
- **`_maybe_interrupt_or_return`（修复循环边界，架构 §2.5.1）**：
  - 成功 → 返回（出边到 reporting）；
  - 可修复 + `fix_loop_count < MAX_FIX_LOOP_COUNT(3)` + `retry_budget_remaining >= DEV_LOOP_MIN_CALLS_PER_ROUND(2)` + `_dev_loop_llm_calls < MAX_DEV_LOOP_LLM_CALLS(20)` → **`fix_loop_count += 1`（单点，§2.5.2）** + `fix_loop_history` append 一条 `FixLoopRecord`（read-modify-write）+ `_dev_loop_route="retry_coding"` → 返回（出边回 coding）；
...
- **中风险**：错误分类关键字表准确率（R-S3-04）—— 兜底归 RUNTIME（可修复给一次机会），靠 `MAX_FIX_LOOP_COUNT=3` 上限拦截不会无限重试。
...
- **测试策略（沿用 sp2「凭证就绪后补跑」范式）**：五条核心场景 e2e（部分可用 mock sandbox + 真实 LLM，部分需真实链路）：
  ...
  2. **修复循环上限 3**（AC-S3-03）：mock execution 连续可修复失败 → fix_loop_count 自增至 3 拦截 → interrupt#2；
...
| R-S3-02 | 修复循环不收敛 / 预算失控 | 中 | 高 | `MAX_FIX_LOOP_COUNT=3` 上限（CP-C3-5）+ 子预算 20 + 入口预算门 + 实际调用回写（CP-C3-8/9/10）+ 错误分类分流（CP-C3-3） |
